# kaioken-plugin-github

GitHub issues and pull requests inside Kaioken, with one-click agent dispatch.

Install it from the BB Official catalog:

```sh
kaioken plugin install github
```
...
- **Send agent / Review with agent**: spawns a Kaioken worker thread on the issue
  (or a review thread on the PR) in the repo's Kaioken project. The issue/PR then
...
- **`kaioken github` CLI**: `repos`, `issues [repo]`, `prs [repo]`, `sync` — also
...
- Every Kaioken project source whose checkout has a GitHub `origin` remote
...
- Plus the `extraRepos` setting: comma-separated `owner/repo` list. Entries that
  ...
  tracked; `kaioken github repos` names them on stderr and the plugin log warns once
  per distinct set. Wildcards are not supported.
- `defaultProject` setting: where threads spawn for repos with no project.

```
kaioken plugin config github set extraRepos "owner/repo, owner/other"
kaioken plugin reload github
```

A background service refreshes the issue/PR cache every 5 minutes; the
panel's Refresh button (or `kaioken github sync`) forces it.
...
```sh
pnpm exec turbo run typecheck test --filter=kaioken-plugin-github
```
