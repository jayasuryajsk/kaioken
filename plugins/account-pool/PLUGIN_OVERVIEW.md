...
- Drag handles set the account order within each provider in settings (keyboard: Space to pick up, arrow keys to move, Space to drop, Escape to cancel), with the same operation available through `kaioken pool account reorder <claude|codex> <id>...`.
- Live limit windows per account and model family in the plugin's settings page, and the same numbers from `kaioken pool status`.
- A routing switch per provider and a bypass per thread, so one thread can go straight to its own credentials.

## How it works

The hub runs inside Kaioken and serves an Anthropic Messages endpoint and an OpenAI Responses endpoint. With routing on, Kaioken hands the Claude Code or Codex process a base URL that points at the hub and a token scoped to that machine, and the provider reports **Proxied** in its health row. An account is skipped for a request when it is at or above the switch threshold or in error. The threshold defaults to 98 percent of a window. Account secrets stay in the Kaioken data directory on the server machine, and the hub refreshes them in the background.
...
`kaioken pool account add|list|remove|enable|disable|priority|reorder`, `kaioken pool status`, `kaioken pool routing <claude|codex> [--off]`, `kaioken pool config`, `kaioken pool config set`, `kaioken pool token rotate`, and `kaioken pool bypass <thread-id>`. `list` and `status` take `--json`.
