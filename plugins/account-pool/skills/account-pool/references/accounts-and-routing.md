...
browser may be on a different machine from the kaioken server. Newly added or
...
`CODEX_POOL_AUTH_TOKEN`; kaioken applies them as in-memory app-server config.
...
`~/.codex/auth.json` on the kaioken server host. OAuth quota refreshes on add or
enable and every five minutes while an account is idle. Use
`kaioken pool account refresh <id>` to request an immediate refresh for one account.
...
UUID is aligned with the selected OAuth account. Use `kaioken pool config` to
inspect the full routing configuration and
`kaioken pool config set <key> <value>` to update one value. The upstream URL keys
