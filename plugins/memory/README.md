# kaioken-plugin-memory

Kaioken's official plugin for durable, progressively disclosed agent memory. It
provides:
...
duplicated or conflicting memories split between Codex, Claude Code, and kaioken.
...
- CLI-only agent access through `kaioken memory` (no native agent tools);
...
kaioken memory add \
  ...
  --details "Run pnpm exec turbo run typecheck --filter=@kaioken/<pkg>." \
  --kind procedure \
  --tag build \
  --tag testing \
  --importance 80 \
...
kaioken memory search "Turbo typecheck" --scope all --json
kaioken memory get <id> --scope all --json
```

Project writes take the invoking CLI's Kaioken project context. Global writes must
...
```bash
pnpm exec turbo run test typecheck --filter=kaioken-plugin-memory
kaioken plugin dev ./plugins/memory
```
