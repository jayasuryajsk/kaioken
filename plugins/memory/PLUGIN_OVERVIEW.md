...
- A summary catalog that kaioken adds to the agent's instructions on every turn. The catalog holds summaries only, up to 3,900 characters.
...
The agent uses the `kaioken memory` CLI. `kaioken memory search` finds summaries with keyword search. `kaioken memory get` reads one complete record. `kaioken memory add`, `kaioken memory update`, and `kaioken memory forget` change the store with version checks. `kaioken memory catalog` and `kaioken memory history` list summaries and past versions.
...
Disable provider-native memory in Settings under Providers to avoid duplicate or conflicting stores. The CLI connects to the local kaioken server. Some provider sandboxes ask for approval before they allow that connection.
