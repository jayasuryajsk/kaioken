# Worker guide — kaioken-plugin-tasks

You are one of several agents building this plugin in a SHARED worktree on branch `kaioken/tasks-plugin`. Read this whole file before writing code.
...
- Visual reference: `tasks-plugin-mock.html` in manager thread storage (Linear-like, kaioken-themed).
...
2. **Git**: commit only your owned paths on `kaioken/tasks-plugin` when your checks are green, message prefixed `tasks plugin: `. Never push, never switch branches, never merge.
3. **UI rules** (for any frontend work):
   ...
   - Theme: kaioken tokens only (`bg-background`, `text-muted-foreground`, `border-border`, semantic `--success`/`--attention`/`--timeline-accent`/`--warning`/`--destructive` via their utilities). NO hex/oklch literals, no `text-[Npx]` (AGENTS.md).
...
4. **Tests**: vitest in this package; plugin testing harness for backend (`@get-kaioken/plugin-sdk/testing`, see `server.test.ts`). High-value tests only; never mock sqlite (the harness gives a real one).
5. **Gates before you report done**:
   - `pnpm exec turbo run typecheck --filter=kaioken-plugin-tasks`
   - `pnpm exec turbo run test --filter=kaioken-plugin-tasks`
   - `pnpm exec turbo run build --filter=kaioken-plugin-tasks`
   - UI tasks: visual verification (below) with screenshots in your report.

## Dev instance + reload loop

A dev kaioken instance for THIS worktree is already running with the plugin installed (path install, `provenance: direct`):

- App: http://localhost:15943 — Server API: http://localhost:23943
- Plugin panel: http://localhost:15943/plugins/tasks/tasks
- CLI against the dev instance: `eval "$(scripts/kaioken-dev-app env)"` then `pnpm kaioken:dev tasks <subcommand>`.
- After changing plugin code: `pnpm exec turbo run build --filter=kaioken-plugin-tasks` then reload. NOTE: the `kaioken plugin reload` CLI hits the same pre-existing `displayName` validation bug as `plugin list` — reload via the API instead: `curl -s -X POST http://localhost:23943/api/v1/plugins/reload -H 'Content-Type: application/json' -d '{"id":"tasks"}'` (check the exact payload against `packages/server-contract` if it 400s; T3.2 used this route successfully).
- If the dev server itself died: `scripts/kaioken-dev-app status` / `scripts/kaioken-dev-app current`.
...
- `pnpm kaioken:dev plugin list` currently fails CLI response validation (pre-existing repo bug, missing `displayName`). Use `curl -s http://localhost:23943/api/v1/plugins` instead.
- Backend factory reloads must stay clean: never keep `kaioken` in module state; register everything inside the factory; `onDispose` for cleanup.
