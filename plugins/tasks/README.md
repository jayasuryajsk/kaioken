...
Tasks is a Linear-style tracker inside kaioken for planning work, delegating it to
...
Install Tasks from the official plugins that Kaioken includes:

```sh
kaioken plugin install tasks
```

The plugin adds the Tasks sidebar panel, the `kaioken tasks` command, and an agent
skill that teaches workers how to report progress back to tasks.

## Quick start

Install the plugin with `kaioken plugin install tasks`. Then use the `kaioken tasks` CLI
to create a tracker project. Link it to the kaioken project where delegated agents
will run:

```sh
kaioken tasks project create \
  --name "Product" \
  --prefix PROD \
  --link-kaioken-project proj_your_bb_project

kaioken tasks create \
  --project PROD \
  --title "Ship task delegation" \
  --description "Implement the flow and run focused validation." \
  --priority high

kaioken tasks list --project PROD
kaioken tasks show PROD-1
kaioken tasks preset list
kaioken tasks delegate PROD-1 --preset "GPT-5.6 · high"
```

When the CLI runs inside a linked kaioken project, `create` and `list` infer the
...
choose or create presets under **Manage → Presets**, and type `@` in the kaioken
...
`kaioken tasks comment PROD-1 --body "New context" --notify`.
...
Run `kaioken tasks --help` or `kaioken tasks <command> --help` for exact options. Add
...
| Command                                        | Purpose                                                                                                                                    |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `kaioken tasks status`                              | Show the installed Tasks plugin name and version.                                                                                          |
| `kaioken tasks project create\|list\|show\|update`  | Manage tracker projects, folders, colors, prefixes, and kaioken-project links.                                                                  |
| `kaioken tasks folder create\|list\|update\|delete` | Organize tracker projects into nested folders. Deleting a folder moves its projects and subfolders to the top level; no tasks are deleted. |
| `kaioken tasks create`                              | Create a task with description, priority, labels, due date, optional parent, and file attachments (repeatable `--attach <path>`).          |
| `kaioken tasks list`                                | Page/filter tasks by project, status, priority, label, active agents, or search text; supports `--sort`, `--limit`, and `--cursor`.        |
| `kaioken tasks show <key-or-id>`                    | Show the complete task record, including comments, attachments, subtasks, and attached threads.                                            |
| `kaioken tasks update <key-or-id>`                  | Update status, priority, title, description, due date, or labels.                                                                          |
| `kaioken tasks comment <key-or-id>`                 | Add a Markdown comment from inline text or a file; optionally notify the latest responding task agent.                                     |
| `kaioken tasks attachment add\|get\|list\|remove`   | Add, fetch, list, or remove attachments. Referenced attachments require `remove --remove-references`.                                      |
| `kaioken tasks preset list\|create\|update\|delete` | Manage reusable agent execution presets.                                                                                                   |
| `kaioken tasks delegate <key>`                      | Start and attach a new agent thread using a preset.                                                                                        |
| `kaioken tasks attach <key-or-id>`                  | Attach the current kaioken thread to a task when it was not delegated from Tasks.                                                               |
| `kaioken tasks detach <key-or-id>`                  | Detach the current kaioken thread (or `--thread <id>`) from a task, for example a dead predecessor after a respawn.                             |
| `kaioken tasks threads <key>`                       | List the kaioken threads attached to a task: live threads first, newest first.                                                                  |
| `kaioken tasks label create\|list\|delete`          | Manage project-scoped labels.                                                                                                              |
| `kaioken tasks seed-demo --yes`                     | Create sample folders, projects, labels, tasks, and comments for evaluation.                                                               |
...
Linking a Tasks project to a kaioken project enables delegation. Open a task, choose
...
Delegation creates a worker thread in the linked kaioken project, attaches that
...
thread with `kaioken tasks attach KEY`. The inverse is `kaioken tasks detach KEY
...
page and `kaioken tasks threads` list live threads before completed or failed ones,
newest first.

## Task mentions

Type `@` in the kaioken composer and select **Tasks** to search by task key or title.
...
action contract as context. Tasks linked to the current kaioken project rank first.
