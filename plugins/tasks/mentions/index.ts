import type { KaiokenPluginApi, PluginMentionItem } from "@get-kaioken/plugin-sdk";
...
function searchTasks(
  ...
  query: string,
  kaiokenProjectId: string | null,
): PluginMentionItem[] {
  const normalizedQuery = query.trim();
  const search = `%${escapeLike(normalizedQuery)}%`;
  const rows = database
    .prepare<{ kaiokenProjectId: string | null; search: string }, MentionTaskRow>(
      `
        SELECT
          t.id,
        ...
        ORDER BY
          CASE
            WHEN @kaiokenProjectId IS NOT NULL
              AND p.linked_bb_project_id = @kaiokenProjectId THEN 0
            ELSE 1
          END,
          t.updated_at DESC,
          t.id DESC
...
You can act on this task with the kaioken tasks CLI. If you begin working on it, first run: kaioken tasks attach ${task.key} (attaches THIS thread so the task shows you as working). Comment substantive updates via kaioken tasks comment ${task.key} --body ... and set status via kaioken tasks update ${task.key} --status ...
`;
}

export function registerMentions(bb: KaiokenPluginApi, store: TasksApiStore): void {
