Keep kaioken from starting more agent threads than your machines can handle. When a limit is reached, new turns wait in line and start as soon as a running thread goes idle.
...
Before kaioken sends a message to an idle thread, the plugin counts the threads that are running. If the count meets the limit for that host, or the overall limit, the message waits. The thread shows why it waits. A message to a thread that is already running is not held. When a thread goes idle, fails, or is archived, the plugin rechecks the line.

## For agents and scripts

Use the `kaioken concurrency-limit` command:

- `kaioken concurrency-limit status` shows the effective limits.
- `kaioken concurrency-limit global unlimited|<limit>` sets the overall limit.
- `kaioken concurrency-limit host <host-id> auto|<limit>` sets one host.
