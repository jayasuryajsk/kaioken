...
kaioken lists it as `plugin:<pluginId>:mine`. Theme Preview resolves the CSS through
the manifest, so chips and live reload work the same way. Install with
`kaioken plugin install path:<dir> --yes`, reload with `kaioken plugin reload <pluginId>`
after CSS edits.
