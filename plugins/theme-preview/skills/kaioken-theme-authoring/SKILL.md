---
name: kaioken-theme-authoring
description: "Create or edit Kaioken color themes and inspect them in the Theme Preview panel."
---

# Authoring a kaioken theme

A kaioken theme is one CSS file that overrides the app's CSS custom properties. kaioken
...
the separate agent thread; use the existing `kaioken theme` commands to locate,
...
```sh
kaioken theme dir          # the custom-theme directory, e.g. ~/.kaioken/theme
```
...
is open, and `kaioken theme list` shows it at once.
...
You only need to declare what you change; everything else derives from kaioken's
...
How kaioken uses them (from kaioken's own components, so you can predict the result):
...
(kaioken has no primary-filled button; `--primary` is links, focus and accents),
...
```sh
kaioken theme set <name>   # activate it app-wide
kaioken theme show         # what is active now
```
