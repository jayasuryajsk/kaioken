Start a thread, pick Codex, and let it write and review code in your repository from kaioken. The plugin drives the Codex CLI on the host machine. It streams the agent's work into the kaioken timeline.
...
- Codex skills from your home directory and project, listed next to kaioken skills.
- Health, usage, and install status on each host, with an install or update action.
- A Codex AI service for inference and voice that other kaioken features can use.

## Settings

- `Codex memory`: let Codex recall and create memories from kaioken threads.
- `Disable provider subagents`: stop native subagents so the agent delegates through kaioken.
