...
it("keeps the introduction and skill switches independent across reloads", async () => {
  ...
  try {
    ...
    expect(instructions()).toContain("You are working inside kaioken");
    ...
    expect(instructions()).toContain("kaioken status");
    expect(await skills()).toEqual([
      "kaioken-cli",
      "skill-creator",
      "submit-a-plugin",
...
describe("individual skill selection", () => {
  it.each([
    ["kaiokenCli", ["kaioken-plugin-authoring", "skill-creator", "submit-a-plugin"]],
    ["pluginAuthoring", ["kaioken-cli", "skill-creator", "submit-a-plugin"]],
    ["skillCreator", ["kaioken-cli", "kaioken-plugin-authoring", "submit-a-plugin"]],
    ["submitPlugin", ["kaioken-cli", "kaioken-plugin-authoring", "skill-creator"]],
