...
and counted in the plugin's handler stats (`kaioken plugin list`).
...
Core runs `.kaioken-env-setup.sh` on the environment's host after `create` returns
`ownsPath: true`, and `.kaioken-env-teardown.sh` before `remove`. Each has a separate
...
- `"local"` (default) — accepts no `Origin` header or a trusted Kaioken app origin.
  A non-GET mutation must use `application/json`. Use this mode for the Kaioken
  frontend.
- `"token"` — requires the per-plugin token (`kaioken plugin token <id>`;
...
- Semantics differ on throw: a service throwing `NeedsConfigurationError`
  ...
  in the schedule's `last_status`/`last_error` shown by `kaioken plugin list`.
...
if (!initial.apiKey)
  bb.status.needsConfiguration(
    "Set apiKey with `kaioken plugin config <id>`, then reload.",
  );
```
