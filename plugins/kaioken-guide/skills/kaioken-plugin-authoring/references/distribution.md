...
1. **`kaioken plugin types`**, run in the plugin directory (or given its path),
   syncs that plugin's SDK surface to the running kaioken — no server needed. For a
   plugin that depends on the npm package it repins the exact
   `@get-kaioken/plugin-sdk` devDependency to this kaioken's SDK version and brings the
   runtime-shimmed packages' type-only devDependencies (sonner, vaul, the
   portal radix families, ...) to the versions this kaioken ships — adding any an
...
2. **Read the bundled declarations** — the authoritative surface, ~13,000
   ...
   - plugins scaffolded by a current kaioken depend on the npm package, so after
     `npm install` read
     `node_modules/@get-kaioken/plugin-sdk/bundled-types/kaioken-plugin-sdk.d.ts`
     (`kaioken-plugin-sdk-app.d.ts` for frontend symbols and
     `kaioken-plugin-sdk-host.d.ts` for the host entry);
   - plugins scaffolded before that still carry the root declaration in
     `types/kaioken-plugin-sdk.d.ts` (plus `types/kaioken-plugin-sdk-app.d.ts` for an
     app), which the plugin's `tsconfig.json` maps
     `@get-kaioken/plugin-sdk` onto. Read whichever the plugin in front of you has.
     That layout still works for existing entries, but migrate before adding
     `bb.host` so the `/host` and `/testing/host` subpaths are present; `kaioken
...
```sh
kaioken plugin install ./kaioken-plugin-notes
kaioken plugin install npm:kaioken-plugin-notes@^1.0.0
kaioken plugin install https://github.com/acme/kaioken-plugin-notes
kaioken plugin install git:https://github.com/acme/kaioken-plugin-notes.git@main
kaioken plugin install git:https://github.com/acme/kaioken-plugin-notes.git@^1.2.0
```
...
kaioken reads the repository's tags, installs the highest release the range allows,
and `kaioken plugin update` moves them to later releases in the same range.
...
kaioken records the tag it installed together with the commit that tag pointed at,
...
the directories in a `.kaioken/plugins.json` collection manifest at the repository
root:
...
```sh
kaioken plugin install git:https://github.com/acme/kaioken-plugins.git@main --plugin notes
kaioken plugin install git:https://github.com/acme/kaioken-plugins.git@main --subdirectory plugins/notes
kaioken plugin install path:. --plugin notes
```
...
last catalog kaioken validated keeps serving. `name` is the marketplace's identity
...
manifest — kaioken fetches and validates them server-side and serves them from its
own origin.
...
kaioken marketplace refresh acme-plugins
kaioken marketplace remove acme-plugins
```
...
installing from a marketplace that is not `bb-community`, kaioken resolves and shows
...
Kaioken's own official plugins are separate: inclusion in the `bb-community`
marketplace is a Kaioken release decision, not part of the plugin authoring
...
app and on the public getbb.app marketplace. `kaioken plugin new` scaffolds one, the
...
`"overview": "./overview/<plugin-id>.md"`. A bundled Kaioken plugin uses the same
...
tool or `kaioken` command names in inline code.
