...
### bb.cli — an agent-facing `kaioken` subcommand

One top-level command per plugin; a second `register` in one factory
execution is rejected.
Users and agents run `kaioken <name> …` like any core command; the kaioken CLI
proxies it to the server, where `run` executes. Core collisions log an
activation warning and appear in `kaioken plugin list` as `kaioken plugin run <id>`.

```ts
bb.cli.register({
  name: "weather", // lowercase [a-z0-9-]+; core collisions use kaioken plugin run <id>
  ...
  commands: [
    // help/skill metadata only; parsing argv is yours
    {
      ...
      usage: "kaioken weather today <city>",
    },
  ],
  async run(argv, ctx) {
    // argv EXCLUDES the command name: `kaioken weather today sf` → argv = ["today", "sf"]
...
`PLUGIN_CLI_OUTPUT_MAX_BYTES` from `@get-kaioken/plugin-sdk` (1,048,576 UTF-8 bytes).
...
loopback, so `kaioken` CLI calls (including plugin commands) work sandboxed;
...
kaioken remotely — share tunnel URLs"), use `contributeInstructions`:
...
bb.agents.registerTool({
  ...
  // Optional row presentation (grammar v3). Without it, Kaioken shows its normal
...
// selects this plugin's own ids when Kaioken resolves a thread/session config.
...
bb.agents.contributeInstructions(({ threadId, projectId }) => {
  ...
  return "The user is viewing kaioken remotely — share tunnel URLs, not localhost.";
});
```
...
approval — falls back to Kaioken's standard `Running tool …` / `Ran tool …`
...
Ordering is standard Kaioken instructions, selected tools' static snippets,
...
takes effect only when the provider session is next started/resumed; Kaioken never
...
kaioken's own AI services — the server-side helper completions behind thread
...
plugin's `bb.host` entry; the user selects it with `KAIOKEN_INFERENCE` /
`KAIOKEN_TRANSCRIPTION` set to `<id>/<model>` (`kaioken settings ai-services` lists
...
retries the first three codes and then uses `KAIOKEN_INFERENCE_FALLBACK`. Voice
retries the configured `KAIOKEN_TRANSCRIPTION` model. Exhausted voice timeouts map
