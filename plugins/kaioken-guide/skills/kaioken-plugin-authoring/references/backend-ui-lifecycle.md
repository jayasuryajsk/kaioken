...
`needs-configuration` (shown in `kaioken plugin list` and the UI) instead of
...
finish. It then closes database handles and invalidates the old `kaioken` object.
Disable and shutdown use the same sequence without a replacement. A
captured `kaioken` from a previous load throws `PluginContextStaleError` on use
