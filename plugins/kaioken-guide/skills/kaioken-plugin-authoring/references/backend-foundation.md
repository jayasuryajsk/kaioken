...
`kaioken plugin logs <id> [-n N] [-f]`.

### bb.settings

`bb.settings.define(descriptors)` declares settings descriptors (rendered
in Settings → Installed plugins and editable via `kaioken plugin config <id> set <key>
...
autosaves, `kaioken plugin config`, `experimental_set`, and fake-host writes. The
...
- `bb.storage.kv` — namespaced JSON key-value rows in kaioken.db:
...
or `null` when `KAIOKEN_APP_URL` is empty. It is not bind-gated.
...
`@get-kaioken/plugin-sdk/host`, which SIGTERMs every process whose working
...
receives the normalized user `PATH` without daemon-owned `KAIOKEN_*` variables.
...
Host production code may import public `@get-kaioken/plugin-sdk` entrypoints, Node
APIs, and ordinary third-party dependencies. It must not import private
monorepo packages such as `@kaioken/domain`, `@kaioken/host-workspace`, or any other
`@kaioken/*` package; the host artifact build rejects those imports anywhere in its
...
Where `@get-kaioken/plugin-sdk` goes in `package.json` depends on what the host
...
`@get-kaioken/plugin-sdk/provider-bridge`, `@get-kaioken/plugin-sdk/ai-services`, or a
published `@get-kaioken/plugin-sdk/host` contract such as
...
kaioken does this). Either way the daemon never resolves the SDK or private Kaioken
packages from the plugin at runtime.

Pure JavaScript dependencies are bundled. For external tools, use
`child_process` to probe or invoke tools on `PATH`. kaioken V1 provides no
...
kaioken connect machine enrollment or its connected daemon reports that the local
