...
experimental_openFilePreview(options), experimental_openFileExternally(options) }`.
  ...
  `openUrl` owns HTTP(S) only and returns false for schemes Kaioken
...
- `experimental_useCodeTheme()` → `{ mode, name, theme }` — the code theme kaioken
  ...
  the same document kaioken's own highlighter paints from. Reach for it ONLY when
  ...
  approximate the palette by reading kaioken's CSS variables: `--canvas` / `--ink`
...
- Use a vendored Kaioken prompt icon-button recipe for native-matching action chrome
...
component kit is removed. The app module still exports focused Kaioken capability
components such as `ThreadChat`, `Markdown`, file links, pickers, source and
diff viewers, and the new-thread composer.

- Builtin plugins in this repo import shared UI from `@kaioken/shared-ui` (the
...
- `kaioken plugin new` pre-vendors button, card, input, checkbox, dialog (plus
  ...
  whose `@kaioken` registry is pinned to the release tag matching the running
  Kaioken. Import via the `@/*` alias: `import { Button } from
"@/components/ui/button"` (tsconfig maps it; `kaioken plugin build` reads it).
- Add more with stock shadcn tooling: `npx shadcn add @kaioken/select
@kaioken/table` — the Kaioken registry carries the full stock set (~44 items:
  accordion, alert-dialog, calendar, chart, command, form, sheet, table,
  …), generated from the Kaioken app's own component source, so vendored code is
  version-matched to your Kaioken by construction. Edit the copies freely; they
...
- Never bundled (runtime-shimmed, import freely): react, the portaling
  ...
  (the scaffold declares all of them; `kaioken plugin types` repins them; `kaioken
...
- Everything else bundles from YOUR `node_modules` (hugeicons, lucide,
  ...
  after adding components (`kaioken plugin new` runs the first one; `shadcn add`
  installs each item's declared deps). Users of your prebuilt artifact need no
  npm. Managed source installs do.
- The old kaioken extras (`EmptyState`, `PageBody`, `Spinner`) are
...
instead of `experimental_UrlLink`. Use `KaiokenNavigate.openUrl` instead of
...
renderers never had the `experimental_Original` alias. Kaioken removes these aliases
in kaioken 0.42.
