...
bundled JavaScript/TypeScript in the kaioken app shell without a React slot. It is
...
code, so install only plugins you trust. kaioken does not use `eval`, `Function`,
...
- `experimental_appOverlay` → `{}` (deliberately no props). An additive,
  ...
  `{ id, component }`. Kaioken mounts every registration once per app window
  ...
  `useComposerView`, remain limited to that surface. Kaioken supplies no chrome,
...
- `navPanel` → `{ subPath: string }` — owns the whole route at
  ...
  Kaioken automatically wraps every plugin page in the same host-owned App panel
  ...
  Terminal itself. Kaioken owns the desktop split, compact drawer, header/panel
  ...
  content tab closes an otherwise empty panel; if fixed tabs remain, Kaioken falls
  ...
  `experimental_target: { validate(value): value is Target }`; Kaioken checks JSON
...
- `threadPanelAction` → an entry in the thread right panel's new-tab
  ...
  `@get-kaioken/plugin-sdk` and `@get-kaioken/plugin-sdk/app`; they persist with the tab across reloads (null when
...
- `experimental_sidebarFooter.register` → managed host-rendered icon items in
  ...
  `{ kind: "disclosure", component }`; kaioken toggles that component above the
  ...
  returns nothing. Kaioken keeps only one disclosure open across all plugins.
...
- `experimental_sidebarNavigation` → replaces the bounded navigation controls
  ...
  the quick palette. No inline search field or query state exists. Kaioken keeps the
...
- `fileOpener` → `{ path: string, source, Original }` — register as a viewer/editor
  ...
  by default. Users can pin Kaioken's preview or a specific opener per extension
...
projectId, experimental_hostId? }` (nullable fields). The optional host ID
  ...
  `Original` is Kaioken's preview bound to this file; render it to
