...
bb.providers.register({
  ...
  // Sessionless maintenance support (each defaults to false) so kaioken can skip
  ...
  capabilities: {
    ...
    supportsThreadArchive: false, // kaioken mirrors archive/unarchive onto it
    supportsThreadRename: false, // kaioken forwards renames to it
  ...
  // path (kaioken then probes once per machine), "workspace" (the default) when
  ...
  // inherited KAIOKEN_* variables stripped; exactly these are forwarded.
  env: { passthrough: ["KAIOKEN_ECHO_AGENT_EXECUTABLE"] },
...
not in the manifest, so `kaioken plugin build` cannot check it — keep it free of
...
mask, so a monochrome mark follows the kaioken theme (and the declared
`strings.iconTint`) with no frontend bundle — this is how every provider kaioken
...
(The four first-party provider plugins ship no `app.tsx`: kaioken vendors their
...
(`kaioken settings general providerOrder '["my-agent","codex"]'` and
`kaioken settings general defaultProviderId my-agent`).
...
host-dependent: Kaioken asks that provider's bridge for `provider/health` and lists
...
the same name, the earlier registration wins and Kaioken logs the conflict. A
...
credentials and sensitive URLs with `secret: true`; Kaioken passes the real value
...
while the proxy is usable, or `null` otherwise. Kaioken uses it only when the
...
```ts
// host.ts (bb.host)
import { experimental_defineProviderBridge } from "@get-kaioken/plugin-sdk/provider-bridge";
...
`@get-kaioken/plugin-sdk/provider-bridge` — protocol schemas including the
`thread/delta` grammar, and the bridge kit — so add `@get-kaioken/plugin-sdk` to
...
artifact cannot import kaioken's private `@kaioken/*` workspace packages; an installed
...
mints every kaioken turn and item id and constructs the canonical timeline
events.
...
For an ACP agent, use `@get-kaioken/plugin-sdk/provider-bridge/acp`. Re-export
...
`@get-kaioken/plugin-sdk/provider-bridge/testing`, against your bridge
...
import a private `@kaioken/*` package from a plugin: an installed plugin cannot
resolve it.
...
`KAIOKEN_PROVIDER_BRIDGE_RECORD_DIR=<dir>` in its environment, run a thread on
your provider, and kaioken writes `<dir>/<providerId>/<threadId>/<direction>.ndjson`
