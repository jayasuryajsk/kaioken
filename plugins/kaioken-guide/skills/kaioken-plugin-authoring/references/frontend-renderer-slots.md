...
- `experimental_sourceCodeRenderer` / `experimental_diffRenderer` →
  replace kaioken's source or diff renderer everywhere it draws supplied content:
  ...
  wins, and a missing, disabled, or crashing replacement falls back to kaioken's
  renderer. Installing and enabling the plugin activates it, and the user can
  pin kaioken's renderer or a specific provider under
...
Original }`. `experimental_fullFileContents` is either
  ...
  Every value is already resolved. Render `Original` (kaioken's
  ...
  name for one release (it warns once; removed in kaioken 0.42).
...
- `commandPaletteAction` → a row in kaioken's quick palette (Mod+Shift+P), listed
  under "Plugins" beside kaioken's own commands. Host-rendered chrome, no plugin
...
- `experimental_timelineRenderer` → the expanded body of the timeline rows a
  ...
  delegations, plan steps) always use kaioken's renderers and are customized only
...
- `experimental_providerIcon` → the React component kaioken draws as one agent,
