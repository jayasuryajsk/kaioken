...
### Unit tests with `@get-kaioken/plugin-sdk/testing`

`@get-kaioken/plugin-sdk/testing` is the official framework-independent harness for
...
`@get-kaioken/plugin-sdk` as an exact devDependency, so the harness is on disk after
`npm install`; an older plugin that still vendors `types/` must add that
devDependency (or run `kaioken plugin migrate`) before tests can import the harness.
...
The fake plugin host's `kaioken` satisfies `KaiokenPluginApi` with host-faithful
...
await harness.lifecycle.dispose(); // abort services, hooks LIFO, close database; stale kaioken throws
```
...
the package, and private `@kaioken/*` dependencies. It permits public SDK paths,
Zod, Node modules, package-local files, and declared extra patterns. It skips
`node_modules` and `dist`.

Host entry (`host.ts`) — `@get-kaioken/plugin-sdk/testing/host`:
...
Frontend (`app.tsx`) — `@get-kaioken/plugin-sdk/testing/app` (vitest + jsdom):
...
### Live loop against a running kaioken

- `kaioken plugin dev` is the loop: save → rebuild declared `bb.app` and `bb.host`
  ...
  `dist/app.js` + `app.css`; `kaioken plugin build` and installs minify them.
- `kaioken plugin list` shows status, services, schedules (with last_error),
  handler stats, and the CLI command; `kaioken plugin logs <id> -f` follows
...
application/json" -d '{}' <server>/api/v1/plugins/<id>/rpc/<method>`,
  `kaioken <command> …` for the CLI, `kaioken plugin run <id> …` as the explicit form.
- Keep pure logic in plain functions/modules so it is unit-testable without
  a kaioken server; the factory file should mostly wire registrations.

BB Official plugins in `plugins/` (a kaioken checkout):
...
- `github` — a gh-CLI-backed issue/PR browser in a single navPanel (with
  ...
  plugins import `@kaioken/shared-ui`; out-of-repo authors vendor the same
  components from the registry), background sync service, rpc + realtime,
  project setting, a `kaioken github` CLI command, and agent-spawn buttons.
...
- Settings saves do not reload healthy or degraded plugins; live `onChange`
  ...
  plugin is `needs-configuration`; `kaioken plugin reload <id>` remains available
...
- Schedules only fire while the plugin is loaded (rows are durable, the
  runner is not).
- CLI `run(argv)` argv excludes the command name; core kaioken command names
  are reserved; workspace-sandboxed agent threads (Accept Edits / Approve
  for me) may fail to reach the kaioken CLI when the provider sandbox blocks
...
- Handler stats shown by `kaioken plugin list` persist across reloads (reset on
  remove).
...
- `onDispose` hooks run LIFO; stale `kaioken` handles from before a reload throw
  on use.
...
- The declarations you read are pinned to one SDK version, not a live view:
  new plugins get them from the exact `@get-kaioken/plugin-sdk` devDependency, older
  ones from a vendored `types/*.d.ts` copy. Run `kaioken plugin types` before
  ...
  "Looking up the exact API". `kaioken plugin migrate` moves an older plugin off the
