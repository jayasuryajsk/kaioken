...
```
kaioken plugin new hello            # scaffolds ./kaioken-plugin-hello: a todo list with a sidebar page, `kaioken hello` CLI, and a skill
cd kaioken-plugin-hello
kaioken plugin install .            # registers the directory in place (--yes to skip the prompt)
kaioken plugin dev                  # rebuild app/host bundles + reload on every save
```
...
- `bb.server` (required) — backend entry. Path installs load it as
  TypeScript directly (no build step); `kaioken plugin build` also emits a
  ...
  `kaioken plugin build` into minified `dist/app.js` + `app.css` + `app.meta.json`
  (`kaioken plugin dev` keeps them readable); path and git installs build it
  ...
  at runtime. Runtime imports that kaioken does not shim belong in `dependencies`.
  ...
  and tooling only — including every package kaioken shims at runtime (sonner,
  ...
  needs a `devDependencies` entry at the host's version (`kaioken plugin new`
  writes all of them; `kaioken plugin types` repins them). Never put one in
...
- Git installs use npm, then build declared app, server, and host source. They
  ...
  `PATH`.
- Building yourself (CI, or verifying a build without a running kaioken): add
  `kaioken-app` to `devDependencies` and set `"build": "kaioken plugin build"`.
  `kaioken plugin build` needs no running server, but the manifest still needs
  `bb.server`. Depending on `kaioken-app@X` builds
  against exactly that release's shim configuration. kaioken downloads its build
...
- `bb.themes` (optional) — contributes palettes to Settings → Appearance and
  `kaioken theme list`. Each entry is
  `{ id, name, description?, css: "./themes/name.css", codeTheme? }`;
  `codeTheme` is `{ dark?, light? }` where each side is a bundled Shiki /
  Pierre name or a plugin-relative VS Code theme `.json` file. kaioken namespaces
...
- `bb.branding` (required) — declare `bb.branding.icon` as either the plugin's
  canonical Kaioken icon name, such as `Zap`, or a plugin-relative compact SVG path
  ...
  Kaioken validates path-shaped SVGs (well-formed XML with an `<svg>` root, no
  ...
  colors are ignored.
  Kaioken reuses this icon on roomy surfaces when no logo override is declared.
  ...
  light logo fail the manifest. `kaioken plugin build` refuses an SVG logo that
  ...
  reject the invalid paths and SVGs described above. Every SVG Kaioken serves
  ...
  logo is required. Kaioken uses a declared logo where space permits, such as roomy
  ...
  `kaioken plugin reload`. Named inline icons use `currentColor`; compact SVG assets
...
- `bb.branding.experimental_icons` (optional) — the plugin's own icon
  ...
  `bb.providers.register` `icon`. Kaioken serves each file hashed from
  ...
  `provider/unhandled`); a `server: "kaioken"` tool row is checked against the
...
- `engines.bb` — optional compatibility string checked during runtime and
  update selection against the Kaioken app version.
- `engines.bbPluginSdk` — optional SDK compatibility string. The scaffold uses
  the repository SDK version. Kaioken validates this range before use. Absent means
  ...
  Compatible updates (`kaioken plugin outdated` / `kaioken plugin update`) only select
  candidates that satisfy these ranges; newer incompatible releases are
  reported as blocked rather than applied. Dev builds (kaioken `0.0.0`) skip
  enforcing `engines.bb` and annotate that on check results.
- **Manual updates:** `kaioken plugin outdated` checks tracking sources and
  `kaioken plugin update` applies compatible candidates (reinstall of an already
  ...
  users.
- `kaioken plugin build` stamps authoritative metadata into every declared
  artifact's `dist/*.meta.json`: `sdkMajor`, `sdkVersion`,
  `artifactFormatVersion` (currently `1`), `pluginId`, `pluginVersion`, and
  `builtWith: { kaiokenVersion, pluginSdkVersion }`. Managed installs reject
  artifacts whose `pluginId`/`pluginVersion` disagree with the package
  manifest, or whose SDK major does not match the host.
- Default to `kaioken-plugin-hello` for the package name. Scoped names such as
  `@acme/kaioken-plugin-hello` are also supported. The plugin id is the final
  package-name component minus the `kaioken-plugin-` prefix. Kaioken lowercases it,
...
`import { defineRpcContract, type KaiokenPluginApi } from
"@get-kaioken/plugin-sdk"`. Validator imports such as Zod are normal plugin runtime
dependencies (and are bundled by `kaioken plugin build`).
...
Use `kaioken plugin reload <id>` only when the change or plugin requires it.
