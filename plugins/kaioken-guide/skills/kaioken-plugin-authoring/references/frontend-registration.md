...
`app.tsx` default-exports `definePluginApp` from `@get-kaioken/plugin-sdk/app`.
React and the SDK are **never bundled** — `kaioken plugin build` shims them to
the host's shared runtime, so the bundle only works inside kaioken.
...
Skills, and plugin panels. Kaioken keeps the drawer, thread list, footer,
...
Kaioken without another replacement lookup. Kaioken restores the original controls if
the selected replacement is unavailable or crashes. Users can select
Automatic, Kaioken, or one plugin under Settings → Appearance → Navigation.
...
The user can pin Kaioken's list or a specific provider under
...
If the chosen plugin is disabled, uninstalled, or its component throws, kaioken
...
interface PluginThreadListProps {
  ...
  searchQuery: string;
  /** Kaioken's bound thread list. Render it to delegate conditionally without
...
// `indicator` (kaioken's resolved status kind) + `indicatorLabel` (its a11y string).
// Draw your own glyph for `indicator`; the SDK ships no status component.
// Treat an unknown indicator value as "none" — kaioken adds kinds over time.
...
actions.open(id, { split: true }); // kaioken's split placement rules
...
actions.requestDelete(id); // opens kaioken's delete confirmation
```

Destructive actions deliberately route through the host's own flow, so there
is no silent `delete`: deletion is recursive, and only kaioken can show the
confirmation that counts the child threads.

Unit-test a list with `renderSlot(...)` from `@get-kaioken/plugin-sdk/testing/app`:
...
menu from `experimental_useSidebarThreadActions` — it exposes everything kaioken's
own menu does, including `requestDelete`, which opens kaioken's confirmation.

**Keyboard support is a DOM contract.** kaioken's thread shortcuts find rows by
