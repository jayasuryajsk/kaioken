...
- `ThreadChat` — kaioken's complete chat surface for an existing thread, rendered
...
- `experimental_ProviderModelPicker` — kaioken's controlled provider, model, and
...
routing?, allowProviderChange?, align?, disabled?, className? }`, where `routing` is
  ...
  kaioken's composers. Provider switches wait for the target provider's verified
  ...
  ```tsx
  import { experimental_ProviderModelPicker as ProviderModelPicker } from "@get-kaioken/plugin-sdk/app";
...
- `experimental_PermissionModePicker` — kaioken's controlled permission-mode
  picker. Props:
  ...
  ```tsx
  import { experimental_PermissionModePicker as PermissionModePicker } from "@get-kaioken/plugin-sdk/app";
...
- `experimental_BranchPicker` — kaioken's controlled branch picker for a standard
  branch choice. Props:
...
- `experimental_SourceCode` — kaioken's source viewer. Props:
  ...
  null). kaioken owns syntax highlighting, gutters, and the live code theme.
- `experimental_Diff` — kaioken's diff viewer. Props:
...
className? }` —
  ...
  (default) or `"split"`. kaioken normalizes the patch, so a GitHub REST patch or
  ...
  consistent with the patch, kaioken enables expand-context controls between
  ...
  ```tsx
  import { experimental_Diff as Diff } from "@get-kaioken/plugin-sdk/app";
...
- `Markdown` — kaioken's chat-message markdown renderer (same typography,
...
- `UrlLink` — a real anchor whose ordinary HTTP(S) activation
  ...
  internal Kaioken routes in SPA history, preserves modifier clicks, copying,
...
- `experimental_FileLink` — a real anchor for an explicit live file target:
  ...
  current surface's shared Kaioken preview. Its lazy context menu offers the
...
- `experimental_NewThreadComposer` — kaioken's complete compose surface for
...
className?, draftKey? }` — the `default*` props are SEEDS, not controlled
  ...
  already running in. kaioken contributes no branch chip of its own: the worktree
  ...
  project". That choice submits Kaioken's personal-project id in `projectId` (not
...
serviceTier?, executionInputSources, environment, input }`. Forward it
  ...
  ```tsx
  // app.tsx
  import { experimental_NewThreadComposer as NewThreadComposer } from "@get-kaioken/plugin-sdk/app";
