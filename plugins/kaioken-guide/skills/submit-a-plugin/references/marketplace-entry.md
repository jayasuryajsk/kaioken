...
the word plugin. Do not add the word Kaioken unless the product name holds it.
...
An App Store listing has two parts. Kaioken holds both in one field.

**The hook is the first sentence.** Kaioken clamps the description to two lines in a
...
```json
{
  "id": "notes",
  "displayName": "Notes",
  "description": "Keeps project notes beside each Kaioken thread.",
  ...
  "source": {
    "git": {
      "url": "https://github.com/acme/kaioken-plugin-notes.git",
      "range": "^1.2.3"
    }
  }
}
...
entry can also use a supported Kaioken host icon name.
...
Kaioken masks SVG icons with the surrounding text color. Use a single-color SVG for
...
Install the plugin in Kaioken first. Capture the real plugin surface. Do not draw a
...
the session. Drive Kaioken with that tool. Open the plugin surface. Capture the
image.
