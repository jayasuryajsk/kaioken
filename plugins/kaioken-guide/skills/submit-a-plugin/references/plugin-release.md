...
1. Read package.json.
2. Inspect name, version, engines, and kaioken.
...
7. For a multi-plugin repository, inspect .kaioken/plugins.json and find the plugin
   subdirectory.
8. Run focused tests, type checks, and builds with the repository package
   manager.
9. Run kaioken plugin build from the plugin directory.

The ID algorithm removes the npm scope and a lowercase kaioken-plugin- prefix. It
...
For example, @acme/kaioken-plugin-notes supplies notes.
...
prebuilt Kaioken files. A Git install can build source during installation.

1. Run kaioken plugin build.
...
{
  "npm": {
    "package": "@acme/kaioken-plugin-notes",
    "range": "^1.2.3"
  }
}
```
