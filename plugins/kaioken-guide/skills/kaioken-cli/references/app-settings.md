# kaioken app settings reference
...
- `kaioken settings general <key> <value>` accepts any key listed under
  `generalSettings` in `kaioken settings show`. Boolean preferences take `true`,
...
- `kaioken settings ui list [--json]` prints every key with its value, revision,
  and description; `kaioken settings ui get <key> [--json]` prints one.
- `kaioken settings ui set <key> <value> [--json]` takes a plain string for enum
  and provider keys and JSON for lists or `null`; it reads the current
  revision, writes with it, and retries once on a conflict.
- `kaioken settings ui reset <key> [--json]` writes the default and advances the
  revision.

## Keyboard shortcuts

- `showKeyboardHints` defaults to true. Set it with
  `kaioken settings keyboard hints <true|false|on|off>` to control whether
...
- Reset removes the override and follows kaioken's current default. Clear stores an
...
- The complete default table is in `docs/configuration.md` in the kaioken source
  repository.

## Diagnostic events

- `showDiagnosticEvents` defaults to false in all builds. Set it with
  `kaioken settings general showDiagnosticEvents <true|false|on|off>`.
...
- `steerActiveThreadOnEnter` defaults to true for a new install. An earlier
  ...
  `kaioken settings general steerActiveThreadOnEnter <true|false|on|off>`.
...
- `streamerMode` defaults to false. Set it with
  `kaioken settings general streamerMode <true|false|on|off>`.
- When enabled, every `customModels` entry from `~/.kaioken/config.json` is hidden
  in all model lists: the pickers, `kaioken provider models`, and
...
- `managedBranchPrefix` defaults to `kaioken/`. Set it with
  `kaioken settings general managedBranchPrefix <prefix>`.
- kaioken puts the prefix in front of every branch name it creates for a managed
  worktree or a new checkout branch, so the default gives
  `kaioken/fix-login-flow-thr_ab12cd34ef`.
...
- kaioken rejects a prefix that cannot start a valid git branch name, such as one
  with a space or a leading `-`. The maximum length is 64 characters.
- The new prefix applies to branches kaioken creates after the change. It does not
...
- Eligible accepted root user messages can be edited without enabling an
  experiment. Use `kaioken thread edit-message` or the message editor in the app.

## Provider session release

- Kaioken releases restorable provider sessions after 30 idle minutes.
- Active turns, commands, agents, workflows, and monitors keep sessions loaded.

## Mobile app

- The `mobileApp` experiment defaults to false while the kaioken mobile app is in
  early access.
- Enable it with `kaioken settings experiment mobileApp true`. It shows the
  **Add mobile device** card under Settings → Remote access.

## Changelog preview

- The `changelogPreview` experiment defaults to false.
- Enable it with `kaioken settings experiment changelogPreview true` to show the
...
- Enable it with `kaioken settings experiment sidebarProgressiveDisclosure true`.
...
- Enable it with `kaioken settings experiment timelineWindowing true`.
