...
- For failed threads, inspect `kaioken thread show <id> --json` and
  `kaioken thread log <id>` before deciding whether to retry, clarify, or update the
  user.
- Use `kaioken thread retry <thread-id>` to re-send a failed turn's original message
  ...
  queue the retry on the clock (same `<when>` grammar as `kaioken thread tell
...
- Use `kaioken thread stop <id>` when a thread is stuck or no longer needed.
- `kaioken thread stop <id>` also releases an idle or stuck agent runtime. The
  command is idempotent and preserves thread history.
- Use `kaioken thread compact <id>` to send the built-in `/compact` command to an idle or errored thread. Completion or failure appears in the timeline. Provider support varies; consult its skill and reported capabilities.
- Use `kaioken thread clear <id>` on an idle or failed thread to reset its active
  timeline and model context in place while keeping the same Kaioken thread,
  workspace, durable event history, and sticky execution settings.
- Use `kaioken thread cancel-plan <id>` to exit an active Plan turn without
  optimistically clearing its banner. Use `kaioken thread clear-goal <id>` to clear
