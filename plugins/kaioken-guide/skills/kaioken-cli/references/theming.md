# kaioken theme authoring reference

How to write a good kaioken app theme: the design model, the full design-token
...
The folder name is the theme id. Run `kaioken theme dir` to print the exact directory
...
and re-run `kaioken theme set <name>` (or just re-select it) to re-apply.
...
1. `kaioken theme dir` — print the custom-theme directory (e.g. `~/.kaioken/theme`).
2. Write your stylesheet to `<that-dir>/<name>/theme.css`. Start the name with
   ...
   hyphens.
3. `kaioken theme set <name>` — activate it. To edit later, change the file in place
   and re-run `kaioken theme set <name>`.
...
```text
<kaioken-data-dir>/theme/my-theme/
  theme.css
  pierre-dark.json
  pierre-light.json
```
...
There is no separate code-theme setting. `kaioken theme show` prints the resolved
