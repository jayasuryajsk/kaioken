...
- A kaioken plugin is a TypeScript package running inside the kaioken server, extending
  it with services, schedules, HTTP/RPC endpoints, settings — and `kaioken` CLI
  subcommands that agents run through bash like any other command.
- Use `kaioken plugin list` to inspect installed plugins and their current state.
- **Kaioken plugin catalog** (store under `/api/v1/plugin-catalog`):
  - The reserved **BB Official marketplace** has the name `bb-official`. It
    ...
    `kaioken marketplace list`. Its plugins appear in the first Browse shelf, Kaioken
  ...
  - The store lists the **BB Community marketplace** catalog: a manifest
    ...
    (override with `KAIOKEN_MARKETPLACE_URL`, which the server reads only at
    ...
    plugin code, and a failed refresh keeps the last catalog kaioken validated.
  - `kaioken plugin search <query> [--json]` — search the catalog by id,
    name, description, category, or tag; status shows installed / compatible /
    requires newer kaioken. The table includes a **Category** column. An
...
- **Third-party marketplaces** (routes under `/api/v1/marketplaces`):
  - `kaioken marketplace add <source>` — add a marketplace from an https manifest
    URL, `git:<url>[@<ref>]` (kaioken reads `marketplace.json` from the checkout),
    or `path:<directory>`. The CLI resolves a relative path from its current
    directory before it sends the request. Kaioken validates the
    ...
    Kaioken ignores unknown v2 fields, except in npm and git source objects. Kaioken
    rejects unknown source keys because a source key changes the installed
    code.
  - `kaioken marketplace list [--json]` — name, source, entry count, last refresh.
  - `kaioken marketplace refresh [name] [--json]` — re-read one catalog or every
    one of them. Discovery metadata and icons only. A failed refresh keeps the
    last catalog kaioken validated and exits non-zero.
  - `kaioken marketplace remove <name> [--json]` — forget a marketplace. Its
    ...
    resolution, so `kaioken plugin outdated`/`update` keep working from the
    recorded source.
  - Install a specific marketplace's entry with
    `kaioken plugin install <entry-id>@<marketplace>`. A bare entry id resolves
  ...
  - Install a bundled plugin by its bare name or with
    `<entry-id>@bb-official`. kaioken copies it from the app bundle.
- Commands:
  - `kaioken plugin install <src>` — `<entry-id>@<marketplace>`, an HTTP(S) Git
    ...
    on its own. kaioken records the selected tag and its commit and refuses to
    ...
    refused — use `kaioken plugin update`. Installing a local path for an id that
    ...
    entry of the repository's `.kaioken/plugins.json` collection manifest (the two
  ...
  - `kaioken plugin outdated` — check installed plugins for compatible updates
    (table; `--json` for raw results). Shows latest compatible candidate and
    any blocked incompatible newer release. Dev builds (kaioken `0.0.0`) annotate
    that `engines.bb` is not enforced.
  - `kaioken plugin update <id>` / `kaioken plugin update --all` — apply compatible
    ...
    non-TTY refuses without it). Use `kaioken plugin outdated` to preview available
    updates. Changing a pinned git:/npm: source requires `kaioken plugin remove`
    ...
    place and `kaioken plugin reload <id>`, or `kaioken plugin install path:<new dir>`
    to move it; both keep its configuration.
  - `kaioken plugin list` — status, background services, schedules, handler timings,
    and each plugin's contributed `kaioken` command.
  - `kaioken plugin source <id> [--json]` — requested and resolved source, the
  ...
  - `kaioken plugin enable|disable <id>`, `kaioken plugin reload [id]` (exits 1 when a
    ...
    `kaioken plugin remove <id>` (deletes the plugin's settings, secrets, and
    schedules; managed git/npm files are deleted, and local path sources stay
    on disk).
  - `kaioken plugin config <id> [set <key> <value> | unset <key>]` — declared
    settings; boolean and number arguments are converted to their declared
    types. Reload the plugin after configuring (`kaioken plugin reload <id>`).
  - `kaioken plugin token <id>` — print a short-lived bearer token for that plugin.
  - List, reload, enable, disable, config, and remove support `--json`.
  - `kaioken plugin logs <id> [-n N] [-f]` — the plugin's `bb.log` output.
  - `kaioken plugin run <id> [args...]` — explicit form; collisions log an activation
    warning and are annotated by `kaioken plugin list`.
  - `kaioken plugin new <name>` — scaffold a todo-list plugin (`server.ts`,
    `app.tsx` with a sidebar page, a `kaioken <name>` CLI command, a skill, and
    vendored UI components) and install its npm dependencies (scaffold sets
    `engines.bbPluginSdk` to `>=0.4.3`). The
    scaffold depends on `@get-kaioken/plugin-sdk`, pinned to this kaioken's exact SDK
    version in `devDependencies`, so the API declarations arrive with
    `npm install` at `node_modules/@get-kaioken/plugin-sdk/bundled-types/*.d.ts`
    ...
    rather than reporting success; `kaioken plugin build [path]` —
  ...
  - `kaioken plugin types [path]` — sync the plugin's `@get-kaioken/plugin-sdk` surface
    to the running kaioken (default: cwd). For a plugin that depends on the npm
    package it rewrites the exact `devDependencies` pin to this kaioken's SDK
    version and brings the type-only devDependencies of the packages kaioken shims
    at runtime (sonner, vaul, the portal radix families, @pierre/diffs, clsx,
    tailwind-merge, class-variance-authority) to this kaioken's versions — adding
    ...
    mismatch (for CI). `kaioken plugin build` and `kaioken plugin dev` refresh vendored
    declarations automatically and leave npm-package plugins alone. Needs no
    server.
  - `kaioken plugin migrate [path] [--yes]` — convert a plugin that still vendors
    `types/` to the `@get-kaioken/plugin-sdk` npm package (default: cwd): add the
    exact `devDependencies` pin, raise `engines.bbPluginSdk` when this kaioken's SDK
    is newer than the declared floor, move an SDK entry declared in
    `dependencies` into `devDependencies`, drop the `@get-kaioken/plugin-sdk` (and
    pre-rename `@get-bb/plugin-sdk`) entries from `compilerOptions.paths` (other
    paths like `@/*` are untouched), and delete `types/kaioken-plugin-sdk*.d.ts`
    ...
    also rewrites quoted `@get-bb/plugin-sdk` import/export specifiers (and their
    subpaths) in the plugin's own `.ts`/`.tsx` sources to `@get-kaioken/plugin-sdk`,
  ...
  - `kaioken plugin dev [path]` — watch loop for an installed plugin (default:
    cwd): on every change it rebuilds the frontend bundle (when `bb.app` is
    declared; unminified, unlike `kaioken plugin build`) and reloads the plugin;
  ...
  - Frontend entries default-export `definePluginApp` from
    `@get-kaioken/plugin-sdk/app` and register UI slots (homepageSection,
...
- **Writing a plugin?** Use the `kaioken-plugin-authoring` skill — the complete
  authoring reference for the backend `KaiokenPluginApi` (settings, storage, sdk,
  ...
  `@get-kaioken/plugin-sdk/app` contract (slots, hooks, UI kit), with working patterns
  and gotchas. `kaioken guide plugins` has the short walkthrough.
