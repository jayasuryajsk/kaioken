...
- `kaioken theme` controls the **app-wide color palette** — a set of CSS-variable
...
- **Custom themes live on disk** under the app data dir, one folder per theme:
  `<kaioken-data-dir>/theme/<name>/theme.css` (the packaged app uses `~/.kaioken/theme/…`).
  The folder name _is_ the theme id. This mirrors how user skills live under
  `<kaioken-data-dir>/skills/<name>/`.
- All theme commands support `--json`.
- Commands:
  - `kaioken theme list` — built-in and custom themes and which palette is active.
  - `kaioken theme dir` — print the absolute custom-theme directory (where to create
    `<name>/theme.css`). Use this instead of guessing the path.
  - `kaioken theme set <id> [--favicon-color <color>]` — activate a built-in
  ...
  - `kaioken theme show [id] [--css]` — print the active palette, or resolve a
    ...
    that Kaioken bundles the CSS. Settings → Appearance previews a palette live on
    hover without saving; this command is the CLI counterpart.
  - `kaioken theme reset` — back to `default` while preserving the favicon color.
  - `kaioken theme favicon set <color>` — set the favicon color while preserving the
    active theme. Colors: `default`, `red`, `orange`, `yellow`, `green`, `teal`,
    `blue`, `purple`, `pink`.
  - `kaioken theme favicon reset` — reset the favicon color to `default` while
    preserving the active theme.

### Creating or editing a custom theme

This is the Kaioken habit: custom app-theme work belongs in
`<kaioken-data-dir>/theme/<name>/theme.css` — never a stray `.css` file elsewhere.

1. Find the directory: `kaioken theme dir` (e.g. `~/.kaioken/theme`).
...
3. Activate it: `kaioken theme set <name>`. Changes apply live to every open window.
