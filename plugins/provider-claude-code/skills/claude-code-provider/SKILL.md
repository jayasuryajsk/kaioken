...
Read settings with `kaioken plugin config provider-claude-code`; change a declared key
with `kaioken plugin config provider-claude-code set <key> <value>`.
...
- Structured plan, message editing, and compaction are supported through the
  corresponding `kaioken thread` commands. Unlisted model IDs are accepted by the
