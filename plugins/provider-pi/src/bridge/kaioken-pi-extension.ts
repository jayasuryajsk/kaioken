export const KAIOKEN_PI_EXTENSION_SOURCE = String.raw`
...
// ---- JSON Schema → TypeBox (the kaioken tool definitions carry JSON Schema) ----
...
export default function kaiokenExtension(pi) {
  const toolsFile = process.env.PI_KAIOKEN_TOOLS_FILE;
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    // predate the kaioken tools, so make sure every injected tool is active.
