---
name: workflows
description: "Author or run durable Kaioken workflows when the user requests workflow execution or multi-agent orchestration."
---
...
A workflow puts deterministic control flow around normal Kaioken worker threads. The
...
kaioken workflows run --file .kaioken/workflows/review-change.js --args '{"items":[]}'
```
...
Use kaioken workflows status for compact progress. Redirect bounded history pages
into KAIOKEN_THREAD_STORAGE before inspection because history can contain full
prompts and results.

## Quality check

- The user explicitly requested workflow orchestration.
- The source passes kaioken workflows validate.
