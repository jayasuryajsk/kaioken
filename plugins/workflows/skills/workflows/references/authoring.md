...
- `agent(prompt: string, opts?)`: spawn a Kaioken worker. Without `schema`, returns
...
Workers spawned by `agent()` are normal Kaioken threads and retain the tools and
workspace access allowed by their Kaioken permission mode. The QuickJS script itself
...
Kaioken validates the tuple against the live provider/model catalog immediately
...
Native `label` is an alias for Kaioken's existing `title`, and native `schema` is an
alias for Kaioken's existing `outputSchema`. Either spelling remains supported.
...
the structured output. Kaioken validates the value with Ajv. The initial invalid
