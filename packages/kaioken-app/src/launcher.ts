...
import {
  KAIOKEN_APP_MANAGED_CONFIG_KEYS,
  ...
  type KaiokenAppManagedConfigKey,
  type KaiokenAppManagedConfigValues,
...
import { validateLogLevel } from "@kaioken/config/log-level";
import { validateOptionalUrl } from "@kaioken/config/public-url";
import { parseServerBindHost, type ServerBindHost } from "@kaioken/config/server";
...
type ManagedConfigValueKey = KaiokenAppManagedConfigKey;
type ManagedConfigKey = "KAIOKEN_SERVER_URL" | "serverUrl" | ManagedConfigValueKey;

const MANAGED_CONFIG_KEYS = KAIOKEN_APP_MANAGED_CONFIG_KEYS;
const MANAGED_CONFIG_KEY_VALUES = new Set<string>(MANAGED_CONFIG_KEYS);
const STARTUP_ONLY_MANAGED_CONFIG_KEYS = new Set<string>(["KAIOKEN_LOG_LEVEL"]);
const STARTUP_ONLY_MANAGED_ENV_KEYS = new Set<string>([
  "KAIOKEN_APP_SURFACE",
  "KAIOKEN_APP_URL",
  "KAIOKEN_DATA_DIR",
  "KAIOKEN_DEV_APP_PORT",
  "KAIOKEN_EXTERNAL_URL",
  "KAIOKEN_HOST_DAEMON_PORT",
  "KAIOKEN_INFERENCE",
  "KAIOKEN_INFERENCE_FALLBACK",
  "KAIOKEN_INHERITED_SKILLS_ROOTS",
  "KAIOKEN_LOG_LEVEL",
  "KAIOKEN_MANAGED_DEV_BUILTIN_PLUGIN_HOT_RELOAD",
  "KAIOKEN_POSTHOG_API_KEY",
...
const kaiokenAppPackageJsonSchema = z
  .object({
...
type ManagedConfigValues = KaiokenAppManagedConfigValues;
type ManagedEnvConfig = KaiokenAppManagedEnvConfig;
type ManagedEnvFile = KaiokenAppManagedEnvFile;
type ManagedConfig = KaiokenAppManagedConfig;
...
export interface KaiokenAppStartContext {
  appDistDir: string;
  appVersion: string;
  configFile: string;
...
interface KaiokenAppRuntimeState {
  config: ManagedConfig;
  context: KaiokenAppStartContext;
  env: NodeJS.ProcessEnv;
...
function warnExistingDaemonLock(lockDir: string): void {
  ...
  log(" ", dim("If startup fails, stop the other kaioken process or remove it."));
  process.stdout.write("\n");
}

function warnExistingRuntimeRecord(dataDir: string): void {
  log(yellow("!"), "Another kaioken already runs on this data directory");
  log(" ", dim(`record: ${formatBbAppRuntimeFilePath(dataDir)}`));
  log(" ", dim("Run `kaioken-app stop` to stop it."));
  process.stdout.write("\n");
}
...
function supportedConfigKeysText(): string {
  return ["KAIOKEN_SERVER_URL", ...MANAGED_CONFIG_KEYS].join(", ");
}
...
export function resolveWorktreeRuntimePolicy(
  ...
  const rawDataDir = requireWorktreePolicyEnvValue(args.env, "KAIOKEN_DATA_DIR");
  ...
  return {
    ...
    serverBindHost: parseServerBindHost(
      args.env.KAIOKEN_SERVER_BIND_HOST ?? KAIOKEN_LOOPBACK_HOST,
    ),
...
function applyWorktreeRuntimePolicy(
  ...
  const nextEnv: NodeJS.ProcessEnv = {
    ...env,
    KAIOKEN_DATA_DIR: policy.dataDir,
    KAIOKEN_HOST_DAEMON_PORT: String(policy.hostDaemonPort),
    KAIOKEN_INHERITED_SKILLS_ROOTS: policy.inheritedSkillsRoots,
    KAIOKEN_SERVER_BIND_HOST: policy.serverBindHost,
    KAIOKEN_SERVER_PORT: String(policy.serverPort),
    KAIOKEN_TELEMETRY: String(policy.telemetry),
  };
  delete nextEnv.KAIOKEN_DEV_APP_PORT;
  return nextEnv;
}
...
function createEnvFromOptions(
  ...
  if (args.options.dataDir !== undefined) {
    env.KAIOKEN_DATA_DIR = args.options.dataDir;
  }
  if (args.options.autoUpdate === true) {
    env.KAIOKEN_HOST_DAEMON_AUTO_UPDATE = "1";
  }
  if (args.options.hostDaemonPort !== undefined) {
    env.KAIOKEN_HOST_DAEMON_PORT = args.options.hostDaemonPort;
  }
  if (args.options.serverBindHost !== undefined) {
    env.KAIOKEN_SERVER_BIND_HOST = args.options.serverBindHost;
  }
  if (args.options.serverPort !== undefined) {
    env.KAIOKEN_SERVER_PORT = args.options.serverPort;
  }
  if (args.options.serverUrl !== undefined) {
    env.KAIOKEN_SERVER_URL = args.options.serverUrl;
  }
  if (args.options.hostId !== undefined) {
    env.KAIOKEN_HOST_ID = args.options.hostId;
  }
  if (args.options.hostType !== undefined) {
    env.KAIOKEN_HOST_TYPE = args.options.hostType;
  }
  if (args.options.joinCode !== undefined) {
    env.KAIOKEN_HOST_ENROLL_KEY = args.options.joinCode;
  }
  if (args.options.enrollKey !== undefined) {
    env.KAIOKEN_HOST_ENROLL_KEY = args.options.enrollKey;
  }
  return env;
}
...
function resolveServerUrl(args: ResolveServerUrlArgs): string {
  return (
    ...
    toOptionalString(args.env.KAIOKEN_SERVER_URL) ??
...
export function resolveServerListenerUrl(
  ...
  const bindHost = parseServerBindHost(args.bindHost ?? KAIOKEN_LOOPBACK_HOST);
...
function applyManagedConfigEnv(
  ...
  return {
    ...
    ...(args.config.machineCredential !== undefined
      ? {
          KAIOKEN_CONNECT_MACHINE_CREDENTIAL: args.config.machineCredential,
        }
      : {}),
    ...(args.config.connectMachineId !== undefined
      ? { KAIOKEN_CONNECT_MACHINE_ID: args.config.connectMachineId }
      : {}),
    ...args.config.config,
    ...args.envFile.env,
  };
...
function createServerBaseEnv(args: CreateServerBaseEnvArgs): NodeJS.ProcessEnv {
  return {
    ...
    ...(args.serverBindHostOverride !== undefined
      ? { KAIOKEN_SERVER_BIND_HOST: args.serverBindHostOverride }
      : {}),
  };
}
...
async function readManagedConfig(
  ...
  try {
    ...
    if (error instanceof SyntaxError) {
      throw new Error(
        `Invalid kaioken-app config JSON at ${formatBbAppConfigPath(args.dataDir)}`,
      );
    }
    if (error instanceof z.ZodError) {
      throw new Error(
        `Invalid kaioken-app config at ${formatBbAppConfigPath(args.dataDir)}: ${error.message}`,
      );
    }
...
async function readManagedEnvFile(
  ...
  try {
    ...
    if (error instanceof SyntaxError) {
      throw new Error(
        `Invalid kaioken-app env JSON at ${formatBbAppEnvPath(args.dataDir)}`,
      );
    }
    if (error instanceof z.ZodError) {
      throw new Error(
        `Invalid kaioken-app env at ${formatBbAppEnvPath(args.dataDir)}: ${error.message}`,
      );
    }
    if (error instanceof Error && "code" in error && error.code === "ENOENT") {
      return {};
...
function validateManagedConfigForWrite(config: ManagedConfigForWrite): void {
  if (config.serverUrl !== undefined) {
    validateOptionalUrl("KAIOKEN_SERVER_URL", config.serverUrl);
  }
  const configValues = config.config;
  if (configValues === undefined) {
    return;
  }
  if (configValues.KAIOKEN_APP_URL !== undefined) {
    validateOptionalUrl("KAIOKEN_APP_URL", configValues.KAIOKEN_APP_URL);
  }
  if (configValues.KAIOKEN_INFERENCE !== undefined) {
    validateInferenceModel(configValues.KAIOKEN_INFERENCE);
  }
  if (configValues.KAIOKEN_INFERENCE_FALLBACK !== undefined) {
    validateInferenceFallbackModel(configValues.KAIOKEN_INFERENCE_FALLBACK);
  }
  if (configValues.KAIOKEN_TRANSCRIPTION !== undefined) {
    validateTranscriptionModel(configValues.KAIOKEN_TRANSCRIPTION);
  }
  if (configValues.KAIOKEN_LOG_LEVEL !== undefined) {
    validateLogLevel(configValues.KAIOKEN_LOG_LEVEL);
  }
}
...
const KAIOKEN_APP_VERSION_DEV_FALLBACK = "0.0.0-dev";
...
export function readBbAppPackageVersion(packageRoot: string): string {
  try {
    ...
    return kaiokenAppPackageJsonSchema.parse(JSON.parse(rawContents)).version;
  } catch {
    return KAIOKEN_APP_VERSION_DEV_FALLBACK;
  }
}
...
export function resolveBbAppStartContext(
  ...
  const serverPort = resolvePortFromEnv({
    defaultPort: KAIOKEN_PROD_SERVER_PORT,
    env: args.env,
    name: "KAIOKEN_SERVER_PORT",
  });
  const daemonPort = resolvePortFromEnv({
    defaultPort: KAIOKEN_PROD_HOST_DAEMON_PORT,
    env: args.env,
    name: "KAIOKEN_HOST_DAEMON_PORT",
  });
...
export async function resolveBbAppRuntimeState(
  ...
  if (args.serverUrlMode === "local") {
    ...
    delete localEnv.KAIOKEN_SERVER_URL;
    delete localServerEnv.KAIOKEN_SERVER_URL;
    return {
      config,
  ...
  const finalEnv = {
    ...managedEnv,
    KAIOKEN_SERVER_URL: resolveServerUrl({
      config,
...
export function resolveBbAppCommand(args: string[]): KaiokenAppCommand {
  if (args.length === 0) {
    return { kind: "start" };
  }
...
function printConfigHelp(dataDir: string): void {
  process.stdout.write(`kaioken-app config

Usage:
  kaioken-app config
  kaioken-app config list
  kaioken-app config refresh
  kaioken-app config set <key> <value>
  kaioken-app config unset <key>

Supported keys:
  ${supportedConfigKeysText()}

Startup-only:
  KAIOKEN_LOG_LEVEL changes require a full kaioken-app restart with
  kaioken-app stop && kaioken-app start, or a desktop app restart.
...
function printEnvHelp(dataDir: string): void {
  process.stdout.write(`kaioken-app env

Usage:
  kaioken-app env
  kaioken-app env list
  kaioken-app env set <key> <value>
  kaioken-app env unset <key>

Startup-only server and launcher keys:
  KAIOKEN_APP_SURFACE, KAIOKEN_APP_URL, KAIOKEN_DATA_DIR, KAIOKEN_DEV_APP_PORT,
  KAIOKEN_EXTERNAL_URL, KAIOKEN_HOST_DAEMON_PORT, KAIOKEN_INFERENCE, KAIOKEN_INFERENCE_FALLBACK,
  KAIOKEN_INHERITED_SKILLS_ROOTS, KAIOKEN_LOG_LEVEL,
  KAIOKEN_MANAGED_DEV_BUILTIN_PLUGIN_HOT_RELOAD, KAIOKEN_POSTHOG_API_KEY,
  KAIOKEN_SERVER_BIND_HOST, KAIOKEN_SERVER_PORT, KAIOKEN_TELEMETRY, KAIOKEN_TRANSCRIPTION,
  and KAIOKEN_FF_* feature flags.
  Changes require a full kaioken-app restart with kaioken-app stop && kaioken-app start,
  or a desktop app restart. KAIOKEN_APP_URL, KAIOKEN_INFERENCE,
  KAIOKEN_INFERENCE_FALLBACK, and KAIOKEN_TRANSCRIPTION can instead be changed live
  with kaioken-app config.
...
function printClientHelp(dataDir: string): void {
  process.stdout.write(`kaioken-app client

Usage:
  kaioken-app client ssh-target list [--json]
  kaioken-app client ssh-target set <server-origin> <ssh-target> [--host-id <id>]
  kaioken-app client ssh-target remove <server-origin> [--host-id <id>]
...
function resolveManagedConfigKey(rawKey: string): ManagedConfigKey {
  ...
  if (key === "KAIOKEN_SERVER_URL" || key === "serverUrl") {
    return key;
  }
  ...
  if (SECRET_SHAPED_ENV_NAME_PATTERN.test(key)) {
    throw new Error(
      `kaioken-app config does not store secrets. Use "kaioken-app env set ${key} <value>" instead.`,
    );
  }
  throw new Error(
    `Unsupported kaioken-app config key "${rawKey}". Supported keys: ${supportedConfigKeysText()}`,
  );
}
...
function formatManagedConfig(config: ManagedConfig): string {
  const lines: string[] = [];
  if (config.serverUrl !== undefined) {
    lines.push(`KAIOKEN_SERVER_URL=${config.serverUrl}`);
  }
...
async function refreshRunningServerConfig(
  ...
  try {
    ...
    if (args.required) {
      throw new Error(`Could not reach kaioken server at ${args.serverUrl}`);
    }
    return false;
  }
  ...
  let message = `kaioken server rejected config reload with HTTP ${response.status}`;
  try {
...
function isStartupOnlyManagedKey(
  ...
  return STARTUP_ONLY_MANAGED_ENV_KEYS.has(key) || key.startsWith("KAIOKEN_FF_");
}

function printStartupOnlyChangeNotice(key: string): void {
  process.stdout.write(
    `${key} is startup-only. The running process keeps its current value; a full kaioken-app restart is required to apply this change. Run \`kaioken-app stop && kaioken-app start\`, or restart the desktop app.\n`,
  );
  if (key === "KAIOKEN_SERVER_BIND_HOST") {
    process.stdout.write(
...
async function refreshRunningServerConfigAfterWrite(
  ...
  if (refreshed) {
    if (isStartupOnlyManagedKey(source, key)) {
      ...
      process.stdout.write("Reloaded running kaioken server config.\n");
    }
    return;
  }
  process.stdout.write(
    `No running kaioken server found at ${serverUrl}; config will apply on next start.\n`,
  );
}
...
async function runConfigCommand(args: RunConfigCommandArgs): Promise<void> {
  ...
  if (commandArgs.length === 1 && commandArgs[0] === CONFIG_REFRESH_COMMAND) {
    ...
    if (startupOnlyKeys.length > 0) {
      process.stdout.write(
        `Startup-only settings currently configured (${startupOnlyKeys.join(", ")}) apply on the next full kaioken-app restart.\n`,
      );
    }
    return;
  }
  if (commandArgs[0] === CONFIG_UNSET_COMMAND) {
    if (commandArgs.length !== 2) {
      throw new Error("Usage: kaioken-app config unset <key>");
    }
  ...
  if (commandArgs[0] !== SET_COMMAND || commandArgs.length !== 3) {
    throw new Error("Usage: kaioken-app config set <key> <value>");
  }
...
async function runEnvCommand(args: RunEnvCommandArgs): Promise<void> {
  ...
  if (commandArgs[0] === CONFIG_UNSET_COMMAND) {
    if (commandArgs.length !== 2) {
      throw new Error("Usage: kaioken-app env unset <key>");
    }
  ...
  if (commandArgs[0] !== SET_COMMAND || commandArgs.length !== 3) {
    throw new Error("Usage: kaioken-app env set <key> <value>");
  }
...
async function runClientCommand(args: RunClientCommandArgs): Promise<void> {
  ...
  if (commandArgs[0] !== CLIENT_SSH_TARGET_COMMAND) {
    throw new Error(
      `Unsupported kaioken-app client command "${commandArgs[0]}". Use "ssh-target".`,
    );
  }
  ...
  if (subcommand === CONFIG_LIST_COMMAND || subcommand === undefined) {
    if (commandArgs.length > 2) {
      throw new Error("Usage: kaioken-app client ssh-target list [--json]");
    }
    process.stdout.write(
  ...
  if (subcommand === SET_COMMAND) {
    if (commandArgs.length !== 4) {
      throw new Error(
        "Usage: kaioken-app client ssh-target set <server-origin> <ssh-target> [--host-id <id>]",
      );
    }
  ...
  if (subcommand === REMOVE_COMMAND) {
    if (commandArgs.length !== 3) {
      throw new Error(
        "Usage: kaioken-app client ssh-target remove <server-origin> [--host-id <id>]",
      );
    }
    await writeClientConfigFile({
  ...
  throw new Error(
    `Unsupported kaioken-app client ssh-target command "${subcommand}". Use list, set, or remove.`,
  );
}

function requiredHostArtifactPaths(context: KaiokenAppStartContext): ArtifactPath[] {
  return [
    { kind: "file", label: "host daemon entry", path: context.daemonEntry },
    {
      kind: "file",
      label: "bundled kaioken CLI",
      path: join(context.daemonBundleDir, "kaioken"),
    },
    {
      kind: "chunk-dir",
      label: "bundled kaioken CLI chunks",
      path: join(context.daemonBundleDir, "kaioken-chunks"),
    },
    {
      kind: "file",
      label: "provider bridge worker",
      path: join(context.daemonBundleDir, "kaioken-provider-bridge-worker.mjs"),
    },
    {
      kind: "file",
      label: "parcel watcher child",
      path: join(context.daemonBundleDir, "kaioken-parcel-watcher-child.mjs"),
    },
    {
      kind: "file",
      label: "plugin host worker",
      path: join(context.daemonBundleDir, "kaioken-plugin-host-worker.mjs"),
    },
  ];
}
...
export function assertBbAppArtifacts(context: KaiokenAppStartContext): void {
  const missingArtifact = requiredFullStackArtifactPaths(context).find(
    (artifact) => !artifactPresent(artifact),
  );
  if (missingArtifact) {
    throw new Error(
      `Missing ${missingArtifact.label} at ${missingArtifact.path}. Rebuild kaioken-app before running this package.`,
    );
  }
}

export function assertBbHostArtifacts(context: KaiokenAppStartContext): void {
  const missingArtifact = requiredHostArtifactPaths(context).find(
    (artifact) => !artifactPresent(artifact),
  );
  if (missingArtifact) {
    throw new Error(
      `Missing ${missingArtifact.label} at ${missingArtifact.path}. Rebuild the kaioken host artifact before running this package.`,
    );
  }
}
...
async function requireExpectedHostDaemonId(args: {
  ...
  const hostId =
    toOptionalString(args.env.KAIOKEN_HOST_ID) ??
...
async function maybeAddAutoJoinEnv(
  ...
  if (toOptionalString(args.env.KAIOKEN_HOST_ENROLL_KEY) !== undefined) {
    return args.env;
  }
...
function normalizeServerUrlForComparison(serverUrl: string): string {
  ...
  if (url.hostname === "localhost") url.hostname = KAIOKEN_LOOPBACK_HOST;
...
export async function waitForHostDaemonStatus(
  ...
  const statusUrl = `http://${KAIOKEN_LOOPBACK_HOST}:${args.port}/status`;
...
function createSharedEnv(args: CreateSharedEnvArgs): NodeJS.ProcessEnv {
  return {
    ...args.env,
    KAIOKEN_APP_VERSION: args.context.appVersion,
    KAIOKEN_DATA_DIR: args.context.dataDir,
    KAIOKEN_HOST_DAEMON_PORT: String(args.context.daemonPort),
    KAIOKEN_SERVER_PORT: String(args.context.serverPort),
    NODE_ENV: "production",
  };
}
...
export function createServerEnv(args: CreateServerEnvArgs): NodeJS.ProcessEnv {
  return {
    ...
    KAIOKEN_CLI: join(args.context.daemonBundleDir, "kaioken"),
    KAIOKEN_CLI_DIR: args.context.daemonBundleDir,
    KAIOKEN_DATA_DIR: args.context.dataDir,
    KAIOKEN_HOST_DAEMON_PORT: String(args.context.daemonPort),
    KAIOKEN_SERVER_PORT: String(args.context.serverPort),
    NODE_ENV: "production",
...
export function createDaemonEnv(
  ...
  return {
    ...
    KAIOKEN_APP_VERSION: context.appVersion,
    KAIOKEN_BRIDGE_DIR: context.daemonBundleDir,
    KAIOKEN_CLI_DIR: context.daemonBundleDir,
    KAIOKEN_DATA_DIR: context.dataDir,
    KAIOKEN_HOST_DAEMON_PORT: String(context.daemonPort),
...
function createCliEnv(args: CreateCliEnvArgs): NodeJS.ProcessEnv {
  ...
  if (toOptionalString(cliEnv.KAIOKEN_SERVER_URL) === undefined) {
    cliEnv.KAIOKEN_SERVER_URL = args.context.serverUrl;
  }
...
function resolveHostDaemonServerUrl(
  ...
  return toOptionalString(args.env.KAIOKEN_SERVER_URL) ?? args.context.serverUrl;
}
...
function createHostDaemonOnlyEnv(
  ...
  return {
    ...
    KAIOKEN_APP_VERSION: args.context.appVersion,
    KAIOKEN_BRIDGE_DIR: args.context.daemonBundleDir,
    KAIOKEN_CLI_DIR: args.context.daemonBundleDir,
    KAIOKEN_DATA_DIR: args.context.dataDir,
    KAIOKEN_HOST_DAEMON_PORT: String(args.context.daemonPort),
...
function resolveEnrollmentRequirements(
  ...
  const enrollKey = toOptionalString(args.env.KAIOKEN_HOST_ENROLL_KEY);
  return {
...
function resolveHostDaemonCommand(
  ...
  throw new Error(
    `kaioken-app host-daemon accepts no subcommand except ${HOST_DAEMON_JOIN_COMMAND}`,
  );
}
...
export async function createHostDaemonJoinEnv(
  ...
  const suppliedJoinCode = toOptionalString(args.env.KAIOKEN_HOST_ENROLL_KEY);
  const machineCredential = toOptionalString(
    args.env.KAIOKEN_CONNECT_MACHINE_CREDENTIAL,
  );
  const connectMachineId = toOptionalString(args.env.KAIOKEN_CONNECT_MACHINE_ID);
  if (suppliedJoinCode !== undefined) {
    ...
    return {
      ...args.env,
      KAIOKEN_HOST_ENROLL_KEY: suppliedJoinCode,
      KAIOKEN_HOST_ID: requestedHostId,
    };
  }
  const enrollKeyResponse = await requestHostEnrollKey({
    requestedHostId,
...
export async function runBundledCliCommand(
  ...
  const kaiokenCliOverride = toOptionalString(args.env.KAIOKEN_CLI);
  const cliPath = kaiokenCliOverride ?? join(args.context.daemonBundleDir, "kaioken");
...
export async function runBbServer(
  ...
  if (parsedArgs.options.help) {
    process.stdout.write(`kaioken-server

Usage:
  kaioken-server [--data-dir <path>] [--server-bind-host <host>] [--server-port <port>]
...
`);
  ...
  if (parsedArgs.positionals.length > 0) {
    throw new Error("kaioken-server does not accept arguments.");
  }
  ...
  const configuredServerBindHost = runtime.serverEnv.KAIOKEN_SERVER_BIND_HOST;
  if (configuredServerBindHost !== undefined) {
    try {
      ...
      if (
        ...
        envFile.env?.KAIOKEN_SERVER_BIND_HOST === configuredServerBindHost &&
        error instanceof Error
      ) {
        throw new Error(
          `Invalid kaioken-app env at ${runtime.context.envFile}: ${error.message}`,
        );
      }
      throw error;
    }
...
async function runHostDaemonOnly(args: RunHostDaemonOnlyArgs): Promise<void> {
  ...
  if (!enrollment.enrolled && enrollment.enrollKey === undefined) {
    endStep(
      red("✗"),
      `Not enrolled - set KAIOKEN_HOST_ENROLL_KEY to join ${serverUrl}`,
    );
    process.stdout.write("\n");
    log(" ", dim("Run this command to request enrollment and start daemon:"));
    log(" ", dim(`  kaioken-app host-daemon join --server-url ${serverUrl}`));
    process.stdout.write("\n");
    process.exitCode = 1;
    return;
  }
  ...
  try {
    ...
    process.stdout.write("\n");
    log(green("●"), bold("kaioken host-daemon is ready"));
    process.stdout.write("\n");
...
export async function runBbHostDaemon(
  ...
  const parsedArgs = parseLauncherArgs(cliArgs);
  if (parsedArgs.options.help) {
    process.stdout.write(`kaioken-host-daemon

Usage:
  kaioken-host-daemon [--server-url <url>] [--host-daemon-port <port>] [--host-id <id>] [--host-type <type>] [--enroll-key <key>] [--auto-update]
  kaioken-host-daemon join --server-url <url> [--host-daemon-port <port>] [--join-code <code> --host-id <id>] [--auto-update]
`);
    return;
  }
...
function printBbAppHelp(): void {
  process.stdout.write(`kaioken-app

Usage:
  kaioken-app [--data-dir <path>] [--server-bind-host <host>] [--server-port <port>] [--host-daemon-port <port>]
  kaioken-app start
  kaioken-app stop
  kaioken-app config set <key> <value>
  kaioken-app config refresh
  kaioken-app env set <key> <value>
  kaioken-app client ssh-target set <server-origin> <ssh-target> [--host-id <id>]
  kaioken-app host-daemon [--server-url <url>] [--host-daemon-port <port>] [--host-id <id>] [--host-type <type>] [--enroll-key <key>] [--auto-update]
  kaioken-app host-daemon join --server-url <url> [--host-daemon-port <port>] [--join-code <code> --host-id <id>] [--auto-update]

CLI:
  npx --package kaioken-app kaioken <command>
`);
}
...
export async function startFullStackServerProcess(
  ...
  const serverRun = spawnNamedManagedProcess({
    ...
    command: process.execPath,
    env: { ...args.env, KAIOKEN_SERVER_LAUNCH_ID: launchId },
    logDir: args.context.logDir,
    processName: "server",
  });
...
async function runStopCommand(args: { dataDir: string }): Promise<void> {
  ...
  if (runtimeFile === null) {
    log(dim("●"), `No running kaioken recorded in ${args.dataDir}`);
    return;
  }
  ...
  const result = await stopVerifiedProcess({
    ...
    verifyTokens: kaiokenAppRuntimeVerifyTokens(runtimeFile.entryPath),
  });
  ...
  if (result.kind === "not-running") {
    ...
    log(
      dim("●"),
      `kaioken was not running (removed a stale record of pid ${String(runtimeFile.pid)})`,
    );
    return;
  }
  ...
  if (result.kind === "still-running") {
    process.stderr.write(
      `kaioken (pid ${String(runtimeFile.pid)}) did not stop, even after SIGKILL.\n`,
    );
    process.exitCode = 1;
    return;
  }
  ...
  log(
    green("✓"),
    `Stopped kaioken (pid ${String(runtimeFile.pid)})${result.usedKill ? " with SIGKILL" : ""}`,
  );
}
...
export async function runBbApp(
  ...
  if (command.kind === "invalid") {
    process.stderr.write(`Unknown kaioken-app command: ${command.command}\n\n`);
    printBbAppHelp();
    process.exitCode = 1;
    return;
  }
  ...
  try {
    ...
    process.stdout.write("\n");
    log(green("●"), bold("kaioken is ready"));
    process.stdout.write("\n");
