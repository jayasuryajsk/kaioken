...
const server = createServer((request, response) => {
  response.writeHead(200, { "content-type": "application/json" });
  response.end(JSON.stringify({ ok: true, launchId: process.env.KAIOKEN_SERVER_LAUNCH_ID }));
});
server.on("error", () => process.exit(1));
setTimeout(() => {
  server.listen(Number(process.env.KAIOKEN_SERVER_PORT), "127.0.0.1");
}, 300);
`;
...
describe("startFullStackServerProcess", () => {
  ...
  function writeFakeServerEntry(): string {
    const dir = mkdtempSync(join(tmpdir(), "kaioken-app-health-"));
  ...
  it("hands the child a launch id and accepts its /health once it listens", async () => {
    ...
    const serverRun = await startFullStackServerProcess({
      context,
      env: {
        KAIOKEN_SERVER_PORT: String(context.serverPort),
