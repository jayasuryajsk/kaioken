...
import { z } from "zod";
import { resolvePortFromEnv } from "@kaioken/config/runtime";
import {
...
const invalidConfigCommandCases: InvalidConfigCommandCase[] = [
  {
    expectedError: /KAIOKEN_INFERENCE must use provider\/model format/u,
    key: "KAIOKEN_INFERENCE",
    value: "gpt-4o-mini",
  },
  {
    expectedError: /KAIOKEN_INFERENCE_FALLBACK must use provider\/model format/u,
    key: "KAIOKEN_INFERENCE_FALLBACK",
    value: "gpt-5.4-mini",
  },
  {
    expectedError: /KAIOKEN_TRANSCRIPTION must use provider\/model format/u,
    key: "KAIOKEN_TRANSCRIPTION",
    value: "gpt-4o-mini-transcribe",
  },
  {
    expectedError: /KAIOKEN_APP_URL must be a valid URL/u,
    key: "KAIOKEN_APP_URL",
    value: "not-a-url",
  },
  {
    expectedError: /KAIOKEN_SERVER_URL must be a valid URL/u,
    key: "KAIOKEN_SERVER_URL",
    value: "not-a-url",
  },
  {
    expectedError: /KAIOKEN_LOG_LEVEL must be one of/u,
    key: "KAIOKEN_LOG_LEVEL",
    value: "bogus",
  },
];

const startupOnlyManagedEnvCases: StartupOnlyManagedEnvCase[] = [
  { key: "KAIOKEN_APP_SURFACE", value: "desktop" },
  { key: "KAIOKEN_APP_URL", value: "https://app.example.test" },
  { key: "KAIOKEN_DATA_DIR", value: "/tmp/kaioken-managed-data" },
  { key: "KAIOKEN_DEV_APP_PORT", value: "4173" },
  { key: "KAIOKEN_EXTERNAL_URL", value: "https://external.example.test" },
  { key: "KAIOKEN_FF_PLACEHOLDER", value: "true" },
  { key: "KAIOKEN_FF_TIMELINE_WINDOW_EVENT_BUDGET", value: "2000" },
  { key: "KAIOKEN_HOST_DAEMON_PORT", value: "48887" },
  { key: "KAIOKEN_INFERENCE", value: "codex/test-inference" },
  {
    key: "KAIOKEN_INFERENCE_FALLBACK",
    value: "codex/test-inference-fallback",
  },
  { key: "KAIOKEN_INHERITED_SKILLS_ROOTS", value: "/tmp/kaioken-skills" },
  { key: "KAIOKEN_LOG_LEVEL", value: "debug" },
  { key: "KAIOKEN_MANAGED_DEV_BUILTIN_PLUGIN_HOT_RELOAD", value: "1" },
  { key: "KAIOKEN_POSTHOG_API_KEY", value: "test-posthog-key" },
  { key: "KAIOKEN_SERVER_BIND_HOST", value: "127.0.0.1" },
  { key: "KAIOKEN_SERVER_PORT", value: "48886" },
  { key: "KAIOKEN_TELEMETRY", value: "false" },
  { key: "KAIOKEN_TRANSCRIPTION", value: "codex/test-transcription" },
];
...
function createTestStartContext(): KaiokenAppStartContext {
  return {
    appDistDir: "/tmp/kaioken-app-test/app/dist",
    appVersion: "0.0.0-test",
    configFile: "/tmp/kaioken-app-test/config.json",
    daemonBundleDir: "/tmp/kaioken-app-test/host-daemon/dist",
    daemonEntry: "/tmp/kaioken-app-test/host-daemon/dist/daemon-bundle.mjs",
    daemonLockDir: "/tmp/kaioken-app-test/daemon.lock.lock",
    daemonLockFile: "/tmp/kaioken-app-test/daemon.lock",
    daemonPort: 38887,
    dataDir: "/tmp/kaioken-app-test",
    dbPath: "/tmp/kaioken-app-test/kaioken.db",
    envFile: "/tmp/kaioken-app-test/env.json",
    logDir: "/tmp/kaioken-app-test/logs",
    packageRoot: "/tmp/kaioken-app-test/package",
    serverEntry: "/tmp/kaioken-app-test/server/dist/index.js",
    serverPort: 38886,
...
describe("kaioken-app launcher", () => {
    ...
    try {
      await expect(
        waitForHostDaemonStatus({
          childProcess: null,
          expectedHostId: "host-expected",
          expectedServerUrl: "https://kaioken.example.test",
          port: address.port,
          timeoutMs: 25,
        }),
  ...
  it("resolves production defaults for npx startup", () => {
    const context = resolveBbAppStartContext({
      entrypointUrl: pathToFileURL("/repo/packages/kaioken-app/dist/kaioken-app.js").href,
      env: {},
      homeDir: "/home/tester",
    });

    expect(context.dataDir).toBe("/home/tester/.kaioken");
    expect(context.configFile).toBe("/home/tester/.kaioken/config.json");
    expect(context.envFile).toBe("/home/tester/.kaioken/env.json");
    ...
    expect(context.serverEntry).toBe(
      "/repo/packages/kaioken-app/server/dist/index.js",
    );
    expect(context.daemonEntry).toBe(
      "/repo/packages/kaioken-app/host-daemon/dist/daemon-bundle.mjs",
    );
  ...
  it("uses workspace build outputs when run from a source checkout", () => {
    const context = resolveBbAppStartContext({
      entrypointUrl: pathToFileURL("/repo/packages/kaioken-app/src/launcher.ts")
        .href,
      env: {},
      homeDir: "/home/tester",
    });

    expect(context.packageRoot).toBe("/repo/packages/kaioken-app");
  ...
  it("falls back to the dev sentinel when package.json is missing", () => {
    expect(readBbAppPackageVersion("/nonexistent/kaioken-app/path")).toBe(
      "0.0.0-dev",
    );
  });
  ...
  it("honors explicit production ports and data directory", () => {
    const env = {
      ...
      KAIOKEN_HOST_DAEMON_PORT: "48887",
      KAIOKEN_SERVER_PORT: "48886",
    };

    expect(resolveDataDir({ env, homeDir: "/home/tester" })).toBe(
      "/home/tester/custom-kaioken",
    );
    expect(
      resolvePortFromEnv({ defaultPort: 1, env, name: "KAIOKEN_SERVER_PORT" }),
    ).toBe(48886);
    expect(
      resolvePortFromEnv({ defaultPort: 1, env, name: "KAIOKEN_HOST_DAEMON_PORT" }),
    ).toBe(48887);
  });
  ...
  it("starts kaioken when no command or the explicit start command is provided", () => {
    expect(resolveBbAppCommand([])).toEqual({ kind: "start" });
    expect(resolveBbAppCommand(["start"])).toEqual({ kind: "start" });
  });

  it("stops kaioken on the explicit stop command only", () => {
  ...
  it("keeps CLI commands on the kaioken binary", () => {
  ...
  it("resolves config commands", () => {
    expect(
      resolveBbAppCommand(["config", "set", "KAIOKEN_APP_URL", "https://bb.test"]),
    ).toEqual({
      args: ["set", "KAIOKEN_APP_URL", "https://bb.test"],
      kind: "config",
    });
  });
  ...
  it("resolves client commands", () => {
    expect(
      ...
      args: ["ssh-target", "set", "https://kaioken.example.test", "devbox"],
      kind: "client",
    });
  });
  ...
  it("reports the server bind host separately from the loopback connection URL", async () => {
    ...
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-bind-host-"));
    const runtime = await resolveBbAppRuntimeState({
      entrypointUrl: pathToFileURL("/repo/packages/kaioken-app/dist/kaioken-app.js").href,
      env: { KAIOKEN_DATA_DIR: dataDir },
      homeDir: "/home/tester",
    ...
    expect(runtime.serverEnv.KAIOKEN_SERVER_BIND_HOST).toBe("0.0.0.0");
    expect(
      resolveServerListenerUrl({
        bindHost: runtime.serverEnv.KAIOKEN_SERVER_BIND_HOST,
  ...
  it("strips parent thread context from the production server without stripping the CLI", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-thread-context-"));
    const runtime = await resolveBbAppRuntimeState({
      entrypointUrl: pathToFileURL("/repo/packages/kaioken-app/dist/kaioken-app.js").href,
      env: {
        ...
        KAIOKEN_THREAD_STORAGE: "/home/tester/.kaioken/thread-storage/thr_parent",
      },
      homeDir: "/home/tester",
      options: { help: false },
      serverUrlMode: "local",
    ...
    expect(runtime.serverEnv.KAIOKEN_ENVIRONMENT_ID).toBeUndefined();
    expect(runtime.serverEnv.KAIOKEN_THREAD_ID).toBeUndefined();
    expect(runtime.serverEnv.KAIOKEN_THREAD_STORAGE).toBeUndefined();
    expect(runtime.serverEnv.KAIOKEN_PROJECT_ID).toBe("proj_parent");

    const daemonEnv = createDaemonEnv(runtime.context, runtime.env);
    expect(daemonEnv.KAIOKEN_ENVIRONMENT_ID).toBeUndefined();
    expect(daemonEnv.KAIOKEN_THREAD_ID).toBeUndefined();
    expect(daemonEnv.KAIOKEN_THREAD_STORAGE).toBeUndefined();
    expect(daemonEnv.KAIOKEN_PROJECT_ID).toBe("proj_parent");

    expect(runtime.env.KAIOKEN_ENVIRONMENT_ID).toBe("env_parent");
    expect(runtime.env.KAIOKEN_THREAD_ID).toBe("thr_parent");
    expect(runtime.env.KAIOKEN_THREAD_STORAGE).toBe(
      "/home/tester/.kaioken/thread-storage/thr_parent",
    );
    ...
    const exitCode = await runBundledCliCommand({
      args: [
        "-e",
        "require('node:fs').writeFileSync(process.argv[1], process.env.KAIOKEN_THREAD_ID ?? 'missing')",
        cliThreadIdPath,
      ],
      context: runtime.context,
      env: { ...runtime.env, KAIOKEN_CLI: process.execPath },
    });
  ...
  it("rejects an invalid server bind host before launcher startup", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-invalid-bind-host-"));

    await expect(
      runBbApp(["--data-dir", dataDir, "--server-bind-host", "localhost"]),
    ).rejects.toThrow('KAIOKEN_SERVER_BIND_HOST must be "127.0.0.1" or "0.0.0.0"');
  });

  it("uses a supplied join code without requesting a loopback enroll key", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-remote-join-"));
    const context = { ...createTestStartContext(), dataDir };

    const env = await createHostDaemonJoinEnv({
      context,
      env: {
        KAIOKEN_HOST_ENROLL_KEY: "bbde_supplied",
        KAIOKEN_HOST_ID: "host_remote",
        KAIOKEN_CONNECT_MACHINE_CREDENTIAL: "bbcm_machine",
        KAIOKEN_CONNECT_MACHINE_ID: "machine-1",
      },
      serverUrl: "https://kaioken.example.test",
    });
  ...
  it("requires the join-code host ID on a fresh machine", async () => {
    ...
    await expect(
      createHostDaemonJoinEnv({
        ...
        env: { KAIOKEN_HOST_ENROLL_KEY: "bbde_supplied" },
        serverUrl: "https://kaioken.example.test",
      }),
  ...
  it("uses managed config server URL when env and flags omit it", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-config-"));
    writeFileSync(
      join(dataDir, "config.json"),
      JSON.stringify({ serverUrl: "https://kaioken.example.test" }),
      "utf8",
    );

    const context = (
      await resolveBbAppRuntimeState({
        entrypointUrl: pathToFileURL("/repo/packages/kaioken-app/dist/kaioken-app.js")
          .href,
        env: { KAIOKEN_DATA_DIR: dataDir },
        homeDir: "/home/tester",
        options: { help: false },
    ...
    expect(context.serverUrl).toBe("https://kaioken.example.test");
  });

  it("uses managed config server URL over ambient env", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-server-config-"));
    writeFileSync(
      join(dataDir, "config.json"),
    ...
    const runtime = await resolveBbAppRuntimeState({
      ...
      env: {
        KAIOKEN_DATA_DIR: dataDir,
        KAIOKEN_SERVER_URL: "https://ambient.example.test",
      },
      homeDir: "/home/tester",
      options: { help: false },
      serverUrlMode: "managed",
    ...
    expect(runtime.env.KAIOKEN_SERVER_URL).toBe("https://stored.example.test");
  });

  it("keeps full-stack startup local even when managed config has a server URL", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-local-config-"));
    writeFileSync(
      join(dataDir, "config.json"),
      JSON.stringify({ serverUrl: "https://kaioken.example.test" }),
      "utf8",
  ...
  it("applies managed config environment values over ambient env", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-env-config-"));
    writeFileSync(
      join(dataDir, "config.json"),
      JSON.stringify({
        config: {
          KAIOKEN_APP_URL: "https://kaioken.example.test",
          KAIOKEN_LOG_LEVEL: "debug",
        },
      }),
      "utf8",
    );
    ...
    const runtime = await resolveBbAppRuntimeState({
      entrypointUrl: pathToFileURL("/repo/packages/kaioken-app/dist/kaioken-app.js").href,
      env: { KAIOKEN_DATA_DIR: dataDir, OPENAI_API_KEY: "ambient-openai-key" },
      homeDir: "/home/tester",
      options: { help: false },
      serverUrlMode: "local",
    });

    expect(runtime.env.KAIOKEN_APP_URL).toBe("https://kaioken.example.test");
    expect(runtime.env.KAIOKEN_LOG_LEVEL).toBe("debug");
    expect(runtime.env.OPENAI_API_KEY).toBe("stored-openai-key");
    expect(runtime.serverEnv.KAIOKEN_LOG_LEVEL).toBe("debug");
  ...
  it("applies the worktree policy after conflicting saved environment values", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-worktree-policy-"));
    const storedDataDir = join(dataDir, "stored-data");
    writeFileSync(
      join(dataDir, "env.json"),
      JSON.stringify({
        env: {
          KAIOKEN_DATA_DIR: storedDataDir,
          KAIOKEN_DEV_APP_PORT: "4173",
          KAIOKEN_HOST_DAEMON_PORT: "48887",
          KAIOKEN_INHERITED_SKILLS_ROOTS: "/stored/skills",
    ...
    const worktreeEnv: NodeJS.ProcessEnv = {
      KAIOKEN_DATA_DIR: dataDir,
      KAIOKEN_HOST_DAEMON_PORT: "47887",
      KAIOKEN_INHERITED_SKILLS_ROOTS: "/worktree/skills",
      KAIOKEN_SERVER_PORT: "47886",
    };
    ...
    for (const env of [runtime.env, runtime.serverEnv]) {
      expect(env).toMatchObject({
        ...
        KAIOKEN_HOST_DAEMON_PORT: "47887",
        KAIOKEN_INHERITED_SKILLS_ROOTS: "/worktree/skills",
        KAIOKEN_SERVER_BIND_HOST: "127.0.0.1",
        KAIOKEN_SERVER_PORT: "47886",
  ...
  it("uses launcher flags over managed config and ambient server URL", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-flag-config-"));
    writeFileSync(
      join(dataDir, "config.json"),
      JSON.stringify({ serverUrl: "https://stored.example.test" }),
      "utf8",
    ...
    expect(runtime.env.KAIOKEN_SERVER_URL).toBe("https://flag.example.test");
  });

  it("stores managed config values from the config command", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-config-command-"));
    ...
    expect(
      ...
      config: {
        KAIOKEN_APP_URL: "https://kaioken.example.test",
        KAIOKEN_INFERENCE: "anthropic/claude-sonnet-4-5",
        KAIOKEN_INFERENCE_FALLBACK: "codex/gpt-5.4-mini",
      },
    });
  ...
  it("stores client SSH targets from the client command", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-client-command-"));
  ...
  it("preserves customModels across managed config writes", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-config-custom-"));
    const customModels = [
      {
  ...
  it("preserves invalid customModels across managed config set writes", async () => {
    const dataDir = mkdtempSync(
      join(tmpdir(), "kaioken-app-config-invalid-custom-models-set-"),
    );
    const customModels = [
  ...
  it("preserves customAcpAgents across managed config writes", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-config-custom-acp-"));
  ...
  it("preserves invalid customAcpAgents across managed config set writes", async () => {
    const dataDir = mkdtempSync(
      join(tmpdir(), "kaioken-app-config-invalid-custom-acp-set-"),
    );
    const customAcpAgents = [
      {
        id: "bad agent",
  ...
  it("preserves invalid customAcpAgents across managed config unset writes", async () => {
    const dataDir = mkdtempSync(
      join(tmpdir(), "kaioken-app-config-invalid-custom-acp-unset-"),
    );
    const customAcpAgents = [
      {
        id: "bad agent",
    ...
    writeFileSync(
      join(dataDir, "config.json"),
      `${JSON.stringify({
        config: { KAIOKEN_APP_URL: "https://kaioken.example.test" },
        customAcpAgents,
      })}\n`,
      "utf8",
    );

    await runBbApp(["--data-dir", dataDir, "config", "unset", "KAIOKEN_APP_URL"]);
  ...
  it("keeps secrets out of the config command", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-secret-config-"));
    ...
    ).rejects.toThrow(/kaioken-app env set OPENAI_API_KEY/u);
  });

  it("stores managed env values from the env command", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-env-command-"));
  ...
  it("rejects invalid server bind hosts before writing managed env", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-invalid-bind-env-"));
  ...
  it("unsets an invalid server bind host already in managed env", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-recover-bind-env-"));
    const envPath = join(dataDir, "env.json");
    writeFileSync(
      envPath,
      JSON.stringify({ env: { KAIOKEN_SERVER_BIND_HOST: "localhost" } }),
      "utf8",
    );
  ...
  it("names the env file when a persisted server bind host is invalid", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-bad-bind-env-"));
    const envPath = join(dataDir, "env.json");
    writeFileSync(
      envPath,
      JSON.stringify({ env: { KAIOKEN_SERVER_BIND_HOST: "localhost" } }),
  ...
  it("blames the flag, not the env file, for an invalid flag value", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-bad-bind-flag-"));
    const envPath = join(dataDir, "env.json");
    writeFileSync(
      envPath,
      JSON.stringify({ env: { KAIOKEN_SERVER_BIND_HOST: "localhost" } }),
    ...
    expect((failure as Error).message).toContain("KAIOKEN_SERVER_BIND_HOST");
    expect((failure as Error).message).not.toContain(envPath);
  });

  it("rejects invalid env key names", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-invalid-env-"));
  ...
  it("uses the explicit server bind host flag over managed env", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-bind-host-env-"));
    writeFileSync(
      join(dataDir, "env.json"),
      JSON.stringify({ env: { KAIOKEN_SERVER_BIND_HOST: "0.0.0.0" } }),
      "utf8",
    );
    ...
    expect(runtime.serverEnv.KAIOKEN_SERVER_BIND_HOST).toBe("127.0.0.1");
  });

  it("unsets managed env values", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-env-unset-"));
    writeFileSync(
      join(dataDir, "env.json"),
      JSON.stringify({
        env: {
  ...
  it("rejects invalid managed config values before writing or reloading", async () => {
    ...
    try {
      for (const testCase of invalidConfigCommandCases) {
        const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-invalid-config-"));
        const configPath = join(dataDir, "config.json");
        const initialConfig = {
          config: {
            KAIOKEN_APP_URL: "https://existing.example.test",
          },
        };
        writeFileSync(
          configPath,
  ...
  it("prints a restart notice when setting a startup-only config key", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-startup-config-set-"));
    const server = await startConfigReloadTestServer();
    ...
    try {
      ...
      expect(output).toContain(
        "KAIOKEN_LOG_LEVEL is startup-only. The running process keeps its current value; a full kaioken-app restart is required to apply this change. Run `kaioken-app stop && kaioken-app start`, or restart the desktop app.",
      );
      expect(output).not.toContain("Reloaded running kaioken server config.");
  ...
  it("prints restart notices for every server startup-only managed env key", async () => {
    ...
    try {
      for (const testCase of startupOnlyManagedEnvCases) {
        const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-startup-env-set-"));
        const output = await captureStdout(() =>
          runBbApp([
            "--data-dir",
            dataDir,
        ...
        expect(output).toContain(
          `${testCase.key} is startup-only. The running process keeps its current value; a full kaioken-app restart is required to apply this change.`,
        );
        expect(output).not.toContain("Reloaded running kaioken server config.");
      }
  ...
  it("warns that wildcard exposure remains after unsetting the server bind host", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-startup-bind-unset-"));
    writeFileSync(
      join(dataDir, "env.json"),
      JSON.stringify({ env: { KAIOKEN_SERVER_BIND_HOST: "0.0.0.0" } }),
      "utf8",
    ...
    try {
      ...
      expect(output).toContain(
        "KAIOKEN_SERVER_BIND_HOST is startup-only. The running process keeps its current value; a full kaioken-app restart is required to apply this change. Run `kaioken-app stop && kaioken-app start`, or restart the desktop app.",
      );
      expect(output).toContain(
  ...
  it("keeps the reload confirmation for reloadable keys", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-reloadable-env-set-"));
    const server = await startConfigReloadTestServer();
    ...
    try {
      ...
      expect(output).toContain("Reloaded running kaioken server config.");
  ...
  it("reports that startup-only config will apply on next start without a running server", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-startup-next-start-"));
  ...
  it("notes configured startup-only keys after explicit refresh", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-startup-refresh-"));
    writeFileSync(
      join(dataDir, "config.json"),
      JSON.stringify({ config: { KAIOKEN_LOG_LEVEL: "debug" } }),
      "utf8",
    );
    writeFileSync(
      join(dataDir, "env.json"),
    ...
    try {
      ...
      expect(output).toContain("Reloaded running kaioken server config.");
      expect(output).toContain(
        "Startup-only settings currently configured (KAIOKEN_FF_PLACEHOLDER, KAIOKEN_LOG_LEVEL, KAIOKEN_SERVER_BIND_HOST, KAIOKEN_SERVER_PORT, KAIOKEN_TELEMETRY) apply on the next full kaioken-app restart.",
      );
    } finally {
      await server.close();
    }
  ...
  it("asks a running local server to reload after config writes", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-config-reload-"));
    const server = await startConfigReloadTestServer();
  ...
  it("supports explicitly refreshing running server config", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-config-refresh-"));
    const server = await startConfigReloadTestServer();
  ...
  it("uses KAIOKEN_SERVER_URL for config refresh when set", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-config-env-refresh-"));
    const server = await startConfigReloadTestServer();
    const previousServerUrl = process.env.KAIOKEN_SERVER_URL;

    try {
      process.env.KAIOKEN_SERVER_URL = server.url;
      ...
      if (previousServerUrl === undefined) {
        delete process.env.KAIOKEN_SERVER_URL;
      } else {
        process.env.KAIOKEN_SERVER_URL = previousServerUrl;
      }
      await server.close();
    }
  });

  it("uses persisted KAIOKEN_SERVER_URL for config refresh without env or flags", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-config-persisted-url-"));
    const server = await startConfigReloadTestServer();
  ...
  it("uses --server-url over env and persisted config for config refresh", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-app-config-flag-url-"));
    ...
    try {
      ...
      process.env.KAIOKEN_SERVER_URL = envServer.url;
      await runBbApp([
        "--data-dir",
        dataDir,
        "--server-url",
  ...
  it("limits npm package metadata to documented runtimes", () => {
    ...
    expect(metadata.files).toContain(
      "host-daemon/dist/kaioken-plugin-host-worker.mjs",
    );
    expect(metadata.files).toContain("host-daemon/dist/kaioken");
    expect(metadata.files).toContain("host-daemon/dist/kaioken-chunks");
    expect(metadata.os).toEqual(["darwin", "linux"]);
  });

  it("requires the bundled CLI's chunk directory next to host-daemon/dist/kaioken", () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "kaioken-app-artifacts-"));
    try {
      const context = resolveBbAppStartContext({
        entrypointUrl: pathToFileURL(join(packageRoot, "dist", "kaioken.js")).href,
        env: {},
      ...
      for (const artifact of [
        context.serverEntry,
        context.daemonEntry,
        join(context.daemonBundleDir, "kaioken"),
        join(context.daemonBundleDir, "kaioken-provider-bridge-worker.mjs"),
        join(context.daemonBundleDir, "kaioken-parcel-watcher-child.mjs"),
        join(context.daemonBundleDir, "kaioken-plugin-host-worker.mjs"),
      ...
      const missingChunks =
        /^Missing bundled kaioken CLI chunks at .*\/host-daemon\/dist\/kaioken-chunks\. Rebuild kaioken-app/;
      expect(() => assertBbAppArtifacts(context)).toThrow(missingChunks);

      const chunkDir = join(context.daemonBundleDir, "kaioken-chunks");
      mkdirSync(chunkDir);
      expect(() => assertBbAppArtifacts(context)).toThrow(missingChunks);
  ...
  it("prunes stale kaioken CLI chunks from package build output", () => {
    const pruneScript = resolve(
      dirname(fileURLToPath(import.meta.url)),
      "..",
      "scripts",
      "prune-kaioken-chunks.mjs",
    );
    const packageRoot = mkdtempSync(join(tmpdir(), "kaioken-app-prune-"));
    try {
      const chunkDir = join(packageRoot, "host-daemon", "dist", "kaioken-chunks");
      mkdirSync(chunkDir, { recursive: true });
      writeFileSync(
        join(packageRoot, "host-daemon", "dist", "kaioken"),
        'import"./kaioken-chunks/chunk-LIVE.js";\n',
      );
      ...
      writeFileSync(
        ...
        JSON.stringify({
          name: "kaioken-app-prune-fixture",
          version: "0.0.1",
          files: ["host-daemon/dist/kaioken", "host-daemon/dist/kaioken-chunks"],
        }),
      );
  ...
  it("keeps the desktop app surface the desktop shell passes to the server", () => {
    ...
    const desktopServerEnv = createServerEnv({
      context,
      env: { KAIOKEN_APP_SURFACE: "desktop" },
    });
    const webServerEnv = createServerEnv({ context, env: {} });
    const invalidSurfaceServerEnv = createServerEnv({
      context,
      env: { KAIOKEN_APP_SURFACE: "bogus" },
    });

    expect(desktopServerEnv.KAIOKEN_APP_SURFACE).toBe("desktop");
    expect(webServerEnv.KAIOKEN_APP_SURFACE).toBe("web");
    expect(invalidSurfaceServerEnv.KAIOKEN_APP_SURFACE).toBe("web");
  });
});
