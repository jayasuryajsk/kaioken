...
/**
 * Delete the kaioken CLI chunks that host-daemon/dist/kaioken does not reach.
 *
 * Bundled into dist/prune-kaioken-chunks.mjs: the build runs that copy after
...
const entry = resolve(distDir, "kaioken");
const chunkDir = resolve(distDir, "kaioken-chunks");

for (const [label, pathToCheck] of [
  ["bundled kaioken CLI", entry],
  ["bundled kaioken CLI chunks", chunkDir],
]) {
  try {
    await access(pathToCheck);
  } catch {
    throw new Error(
      `Missing ${label} at ${pathToCheck}: run from packages/kaioken-app after building it.`,
    );
  }
}
...
if (removed.length > 0) {
  ...
  process.stderr.write(
    `kaioken-app: pruned ${removed.length} stale kaioken CLI chunk file(s)\n`,
  );
}
