...
// every builtin unable to resolve @get-kaioken/plugin-sdk at import time).
...
const tempRoot = await mkdtemp(join(tmpdir(), "kaioken-app-tarball-"));
...
async function timed(label, run) {
  ...
  try {
    ...
    process.stdout.write(
      `kaioken-app tarball smoke: ${label} ${formatElapsed(startedAt)}\n`,
    );
  }
}
...
async function packTarball() {
  ...
  if (liveChunk === undefined) {
    throw new Error("Built kaioken-app has no CLI chunk to exercise");
  }
  ...
  try {
    ...
    const staleChunkPath = `host-daemon/dist/kaioken-chunks/${staleChunkName}`;
...
function spawnPackedBridge({ bridgePath, packageDir, pluginId }) {
  ...
  return spawn(
    process.execPath,
    [
      join(packageDir, "host-daemon", "dist", "kaioken-provider-bridge-worker.mjs"),
      bridgePath,
      pluginId,
      dataDir,
    ],
  ...
  childProcess.stdin.write(
    `${JSON.stringify({
      ...
      params: {
        ...
        client: { name: "kaioken-app-smoke", version: "0.0.0" },
      },
    })}\n`,
  );
  childProcess.stdin.write(
...
async function smokeProviderBridgeBundles(packageDir) {
  await smokeBridgeModelList({
    ...
    // so the packed bundle to smoke is the one `kaioken plugin build` produced for
  ...
  await smokeBridgeModelList({
    ...
    // packed bundle to smoke is the one `kaioken plugin build` produced for the
...
// The daemon forks kaioken-plugin-host-worker.mjs (a sibling of daemon-bundle.mjs)
...
async function smokePluginHostWorkerBundle(packageDir) {
  ...
  try {
    ...
    process.stdout.write("kaioken-app tarball smoke: plugin host worker ready\n");
  } catch (error) {
...
async function smokeHelpCommands(binDir) {
  await runCommand({
    ...createInstalledBinInvocation(binDir, "kaioken-app", ["--help"]),
    label: "kaioken-app help",
  });
  await runCommand({
    ...createInstalledBinInvocation(binDir, "kaioken", ["--help"]),
    label: "kaioken cli help",
  });
  await runCommand({
    ...createInstalledBinInvocation(binDir, "kaioken-server", ["--help"]),
    label: "kaioken-server help",
  });
  await runCommand({
    ...createInstalledBinInvocation(binDir, "kaioken-host-daemon", ["--help"]),
    label: "kaioken-host-daemon help",
  });
}
...
async function smokeConfigCommand(binDir) {
  ...
  await runCommand({
    ...createInstalledBinInvocation(binDir, "kaioken-app", [
      "--data-dir",
      dataDir,
      "env",
      "set",
    ...
    label: "kaioken-app env OPENAI_API_KEY",
  });
  await runCommand({
    ...createInstalledBinInvocation(binDir, "kaioken-app", [
      "--data-dir",
    ...
    label: "kaioken-app config KAIOKEN_APP_URL",
  });
  ...
  if (envJson.env?.OPENAI_API_KEY !== "test-openai-key") {
    throw new Error("Expected kaioken-app env to persist OPENAI_API_KEY");
  }
  if (configJson.config?.KAIOKEN_APP_URL !== "https://kaioken.example.test") {
    throw new Error("Expected kaioken-app config to persist KAIOKEN_APP_URL");
  }
}
...
async function smokeSdkPackage(tarballPath) {
  ...
  await timed("npm install tarball", () =>
    runCommand({
      ...
      command: "npm",
      cwd: sdkDir,
      label: "install kaioken-app SDK smoke package",
    }),
  );
  await runCommand({
    args: [
      "--input-type=module",
      "-e",
      'import { BBSdk } from "kaioken-app"; if (typeof BBSdk !== "function" || typeof new BBSdk().experimental_desktopBrowsers?.listInstances !== "function") process.exit(1);',
    ],
    command: "node",
    cwd: sdkDir,
    label: "kaioken-app SDK JavaScript import",
  });
  await writeFile(
    join(sdkDir, "sdk-smoke.ts"),
    [
      'import { BBSdk, KaiokenHttpError } from "kaioken-app";',
      "",
      'const bb = new BBSdk({ baseUrl: "http://127.0.0.1:38886" });',
      "const error: typeof KaiokenHttpError = KaiokenHttpError;",
      "void bb.status.get();",
  ...
  await timed("npx typescript check", () =>
    runCommand({
      ...
      command: "npx",
      cwd: sdkDir,
      label: "kaioken-app SDK TypeScript import",
    }),
  );
  return sdkDir;
}
...
async function smokeInstalledRepack(installedPackageDir) {
  const stdout = await runCommand({
    ...
    label: "repack installed kaioken-app",
  });
  const [packed] = JSON.parse(stdout);
  if (!Array.isArray(packed?.files)) throw new Error("Invalid npm pack output");
  const chunkPrefix = "host-daemon/dist/kaioken-chunks/";
  ...
  if (
    ...
    throw new Error("Installed kaioken-app repack did not preserve its live chunks");
  }
}
...
async function smokeBuiltinPluginsRunning({ binDir, cliEnv }) {
  ...
  while (Date.now() <= deadline) {
    const stdout = await runCommand({
      ...createInstalledBinInvocation(binDir, "kaioken", [
        "plugin",
        "list",
        "--json",
      ]),
      env: cliEnv,
      label: "kaioken plugin list",
    });
...
async function smokeFullStack(binDir, sdkDir) {
  ...
  const stack = spawnManagedProcess({
    ...
    env: {
      KAIOKEN_LOG_LEVEL: "info",
    },
    label: "kaioken-app full stack",
  });
  ...
  try {
    ...
    await runCommand({
      ...createInstalledBinInvocation(binDir, "kaioken", ["status"]),
      env: cliEnv,
      label: "kaioken cli status",
    });
    ...
    await runCommand({
      args: [
        ...
        [
          ...
          "const bb = new BBSdk({ baseUrl: process.env.KAIOKEN_SERVER_URL });",
          "await bb.status.get();",
        ].join("\n"),
      ],
      command: "node",
  ...
  const daemonSpecs = [
    {
      ...
      label: "kaioken-app host-daemon join 1",
      port: firstDaemonPort,
    },
    {
      dataDir: join(tempRoot, "join-daemon-data-2"),
      label: "kaioken-app host-daemon join 2",
      port: secondDaemonPort,
    },
  ];
  const server = spawnManagedProcess({
    ...createInstalledBinInvocation(binDir, "kaioken-server", [
      "--data-dir",
      serverDataDir,
      "--server-port",
      String(serverPort),
    ...
    env: {
      KAIOKEN_LOG_LEVEL: "warn",
    },
    label: "kaioken-server",
  });
  ...
  try {
    ...
    for (const spec of daemonSpecs) {
      const daemon = spawnManagedProcess({
        ...
        env: {
          KAIOKEN_LOG_LEVEL: "info",
          KAIOKEN_SERVER_URL: staleEnvServerUrl,
        },
        label: spec.label,
      });
      daemons.push(daemon);
    ...
    const cliEnv = {
      KAIOKEN_DATA_DIR: serverDataDir,
      KAIOKEN_HOST_DAEMON_PORT: String(firstDaemonPort),
      KAIOKEN_SERVER_URL: serverUrl,
    };
    await smokeBuiltinPluginsRunning({ binDir, cliEnv });
...
try {
  ...
  const installedPackageDir = join(sdkDir, "node_modules", "kaioken-app");
  ...
  process.stdout.write(
    `kaioken-app tarball smoke passed in ${formatElapsed(smokeStartedAt)}\n`,
  );
} finally {
