...
A project maps to a code repository. All threads belong to a project.
  ...
  kaioken project list                         List ordinary projects
    --include-personal                    Also include the personal project
  kaioken project history <id>                 List prompt history
  kaioken project reorder <id>                 Reorder in the sidebar
    --after <id>                          Previous project, or omit for start
    --before <id>                         Next project, or omit for end
  kaioken project create --name "..." [options]
  ...
  kaioken project show <id>                    Show project details
  kaioken project update <id>                  Update a project
    --name <name>                         New name

  kaioken project delete <id>                  Delete project and all threads
    --yes                                 Skip confirmation

Discovery:

  kaioken project branches <id> --host <id>   List branches for a machine source
  kaioken project paths <id>                   Search workspace paths
  kaioken project files <id>                   List workspace files
  kaioken project content <id> <path>          Read file content (binary is base64)
  kaioken project commands <id> --provider <id>
...
Attachments:
  ...
  kaioken project attachment upload <id>       Upload bytes from the CLI machine
  ...
  kaioken project attachment download <id> <attachment-path>
...
Sources:
  ...
  kaioken project source add <projectId>       Add a source
  ...
  kaioken project source update <projectId> <sourceId>
    --path <path>
    --default

  kaioken project source delete <projectId> <sourceId>
...
Use `kaioken file list|paths` for filesystem listings that include ignored files,
