...
Providers are agent backends (e.g., codex, claude-code). Each supports different models.
  ...
  kaioken provider list [--machine <id-or-name> | --environment <id>]
                                          List available providers
  kaioken provider models [providerId] [--machine <id-or-name> | --environment <id>]
...
When provider and model are omitted from kaioken thread spawn, the project's
remembered defaults apply. If the project has no remembered choice, kaioken uses
...
settings, or run:
  ...
  kaioken provider-retry status [thread-id] [--json]    Inspect pending retries
  kaioken provider-retry cancel <thread-id> [--json]    Cancel an automatic retry
  kaioken provider-retry retry <thread-id> [--json]     Send a pending retry now
...
Claude Code runs without its Claude in Chrome browser tools under kaioken by
default. Enable them with
`kaioken plugin config provider-claude-code set chromeEnabled true`. The host needs
...
kaioken indexes the native user and project skill roots for Codex, Claude Code, Pi,
...
command menu. `kaioken skill list` shows native skills for Claude Code, Codex, and
Cursor.
...
appears automatically. Discover and select one with:
  ...
  kaioken provider models acp-opencode --environment "$BB_ENVIRONMENT_ID"
  kaioken thread spawn --provider acp-opencode --model <provider/model>

kaioken applies the selected model to the ACP session before the first prompt.
...
session mode, not a model. kaioken does not select OpenCode agents; configure the
...
provider id), a model id, and an optional displayName. kaioken skips an invalid
entry with a warning. The entry then appears in kaioken provider models output and
...
model to the OpenCode config instead. Edit the JSON and run kaioken-app config
...
JSON array. Set it with kaioken plugin config provider-acp set customAgents '[...]'.
Each entry needs id (lowercase letters, digits and dashes), displayName, and
command. kaioken derives provider id acp-<id> from the slug id. The id is permanent.
The id cursor is reserved because kaioken always lists that agent. The ids opencode,
...
the vendor side channels kaioken reads. Use nativeSkillRoots to add native skills to
the composer.
...
kaioken hides the /compact command for agents that do not declare it. The plugin
re-registers its providers as soon as the setting changes, so no restart or
config refresh is needed.

The old customAcpAgents array in the app data-dir config.json is deprecated. kaioken
...
setting does not accept: kaioken drops that field when it reads the old array, and a
...
user and project paths use the same relative-path rules. kaioken indexes these roots
as read-only sources. It then injects the selected skills into all providers.
The kaioken user and project roots keep higher precedence than matching shared roots.
