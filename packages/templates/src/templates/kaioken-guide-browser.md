...
`kaioken browser` is the experimental core API for automation integrations controlling Kaioken desktop tabs. The Browser Automation plugin adds its own script/session commands; another plugin can use the same core connection independently.

Start with `kaioken browser instances --host <host-id> --json`. For every tab/control operation provide `--host <host-id> --instance <instance-id> --generation <generation> --thread <thread-id>`. The browser host can differ from the agent host. Never infer an active desktop window.
...
Cookie import copies signed-in sessions from a browser installed on the desktop host into a Kaioken browser profile. These two commands take `--host`, `--instance`, and `--generation` but no `--thread`:
