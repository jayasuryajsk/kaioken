...
Spawning:
  ...
  kaioken thread spawn --project <id> --prompt "..." [options]
    ...
    --parent-self                  Parent to the current thread (KAIOKEN_THREAD_ID)
    ...
    --environment-provider <id>    Run on an environment provider by id (list them with
                                   `kaioken environment providers`). The provider
    ...
    --environment-inputs <json>    JSON value for an --environment-provider that declares
                                   inputs; `kaioken environment providers --json` prints each
  ...
  remembered project defaults. With no remembered model, kaioken uses the explicitly
  ...
  Before selecting a provider, run `kaioken environment providers --project <id>
...
Editing a sent message:
  ...
  kaioken thread edit-message <id> --message "Replacement text"
    --self                              Target the current thread (KAIOKEN_THREAD_ID)
  ...
  changes. From an agent thread, the command carries `KAIOKEN_THREAD_ID` so the
  replacement runs under agent permission policy.

Listing:

  kaioken thread list                           List threads
  ...
  kaioken thread search <query> [--limit <1-50>]
                                             Search threads and messages
  kaioken thread history <id>                   List prompt history

  kaioken thread count                          Count threads without listing them
  ...
  counting rows: `kaioken thread list` pages a bounded window and would miscount.
...
Sections:
  ...
  kaioken thread section list
  kaioken thread section create <name>
...
Inspecting:
  ...
  kaioken thread log [id]                       Show thread event log
    --self                                 Target current thread
  ...
  kaioken thread output [id]                    Get the final output of a thread
    --self                                 Target current thread

  kaioken thread wait <id>                      Wait for a thread status or event (defaults to --status idle)
...
Opening threads and files in the app:
  ...
  kaioken thread open <path>                    Open a file in the current Kaioken thread panel
  kaioken thread open <thread-id> [path]        Open a thread, optionally with a panel file
    --line <number>                        Line number to focus
    --split <placement>                    right, down, left, top, or replace
  kaioken thread pane <action> [thread-id]      Maximize, restore, toggle, spotlight, or clear spotlight

  Inside a Kaioken thread, KAIOKEN_THREAD_ID selects the current thread automatically and
  the thread ID argument is omitted for file-only opens. Pass an explicit thread
  ID with --split to open another thread. Outside a Kaioken thread, pass the thread ID
  ...
  Pane actions broadcast to connected Kaioken app windows and affect the matching
  ...
  target thread workspace. Absolute paths under KAIOKEN_THREAD_STORAGE open as
  thread-storage files for the current thread. Use this for Markdown or HTML
  artifacts you create for the user so they open in the Kaioken IDE.

Messaging:

  kaioken thread tell <id> <message>            Send a follow-up message
  ...
  `kaioken thread interactions`; `kaioken thread cancel-plan` leaves Plan mode early.
  SDK callers build the same input with
  `createBuiltinPlanCommandTextInput(text)` from `@kaioken/sdk` and pass it as
  `input` to `threads.spawn` or `threads.send`.

  kaioken thread stop [id]                      Stop work and release the agent runtime
  kaioken thread compact [id]                   Request compaction of an idle or errored thread's context
  kaioken thread clear [id]                     Clear model context for an idle or failed thread
  kaioken thread cancel-plan [id]               Exit the provider's active Plan mode
  kaioken thread clear-goal [id]                Clear the provider's active Goal
    --self                                 Target current thread
  ...
  `thread clear` keeps the Kaioken thread, workspace, durable event history, and
...
Ownership:
  ...
  kaioken thread update [id]                    Update thread metadata
    --self                                 Target current thread
  ...
  Model and reasoning updates stay within the thread's current provider. Kaioken
  ...
  kaioken thread read [id]                      Mark read
  kaioken thread unread [id]                    Mark unread
  kaioken thread reorder-pinned <id> [--after <id>] [--before <id>]

Interactions:

  kaioken thread interactions list [id]         List a thread's pending and past interactions
  kaioken thread interactions show <interaction-id> [id]
                                           Show one interaction (approval details, questions, or a plugin form's data)
  kaioken thread interactions approve <interaction-id> [id]
                                           Allow a command, file-change, plan, or tool-use approval
  kaioken thread interactions deny <interaction-id> [id]
                                           Deny an approval
  kaioken thread interactions grant <interaction-id> [id] --scope turn|session
                                           Grant a permission interaction
  kaioken thread interactions answer <interaction-id> [id] --choice <questionId=value> --text <questionId=text>
                                           Answer a provider's user question
  kaioken thread interactions respond <interaction-id> [id] --value '<json>'
...
Queued messages:
  ...
  kaioken thread queue list [<thread-id>] [--wait-holder plugin:<plugin-id>]
  kaioken thread queue create <thread-id> <message>
  kaioken thread queue update <thread-id> <message-id> <message> [--file <path>] [--image <path>]
  kaioken thread queue send <thread-id> <message-id> [--mode auto|steer]
  kaioken thread queue reorder <thread-id> <message-id> [--after <id>] [--before <id>]
  kaioken thread queue group <thread-id> <boundary-id> --prefix <comma-separated-ids>
  kaioken thread queue delete <thread-id> <message-id>
...
Persisted panel tabs:
  ...
  kaioken thread tabs show <thread-id>
  kaioken thread tabs set <thread-id> --expected-revision <n> --tabs-json '<json>'

Lifecycle:

  kaioken thread retry [id]                     Retry the thread's failed turn
    --self                                 Target current thread
  ...
  kaioken thread archive [id]                   Archive a thread (and children/hidden forks)
  ...
  kaioken thread unarchive [id]                 Unarchive a thread
    --self                                 Unarchive current thread

  kaioken thread delete <id>                    Delete permanently
    --yes                                  Skip confirmation

  Deleting a thread removes its record immediately, but provider-owned
  environment cleanup is asynchronous. Use `kaioken environment show <id>` to
