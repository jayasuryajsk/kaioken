---
kind: instruction
title: kaioken Guide — Plugins
summary: Command reference for installing, configuring, running, and authoring kaioken plugins and their contributed CLI commands.
intent: Provide complete plugin command documentation plus an authoring walkthrough for agents and humans building kaioken plugins.
editingNotes: Keep flags accurate against the CLI implementation (apps/cli/src/commands/plugin.ts, apps/cli/src/commands/marketplace.ts) and the server plugin service; a CLI test asserts every `kaioken plugin` and `kaioken marketplace` subcommand appears in this chapter. The full authoring reference is the kaioken-plugin-authoring builtin skill.
---
Plugin commands

A kaioken plugin is a TypeScript package that extends the kaioken server in-process and
...
settings, storage, host-local operations — and `kaioken` CLI subcommands that agents
and humans run like any other command. Plugins are full-trust code in both
runtimes.

Plugins are on by default. Builtin plugins (`builtin:<name>`) ship with kaioken;
user-installed plugins come from `kaioken plugin install` or the official store.
Plugin state lives under `<kaioken-data-dir>/plugins/<id>/` (per-plugin SQLite file,
secrets, logs).

The builtin Custom instructions plugin adds a multiline editor under Settings
→ Custom instructions. Saved text is persisted on this kaioken host and included in
...
provider API requests through the kaioken server. Enable it and add an account:
...
kaioken pool account add --provider claude --api-key <key> [--label <text>] [--priority <n>]
...
to run on the kaioken server machine, and neither the code nor account tokens enter
...
path continues to read the kaioken server host's `~/.codex/auth.json`.
...
five minutes while the account is idle. Use `kaioken pool account refresh <id>` to
...
account. `kaioken pool config` prints the quota switch threshold and both upstream
URLs. Use `kaioken pool config set <key> <value>` to change one; the two URL values
...
The builtin Keep Awake plugin prevents macOS idle sleep while kaioken is running.
...
narrated. Inspect it with `kaioken provider-retry status`. See
`kaioken guide providers` for the eligibility rules. The plugin only reacts to a
...
`kaioken plugin config provider-retry set maximumWait <value>`.
...
Settings → Installed plugins or run `kaioken plugin enable workflows` before using:

  kaioken workflows validate (--script '<javascript>'|--source '<javascript>'|
                        --file <path>|--name <name>)
  kaioken workflows run (--script '<javascript>'|--source '<javascript>'|
                   --file <path>|--name <name>)
                   [--args '<json>'] [--resume <run-id>]
  kaioken workflows status <run-id>
  kaioken workflows history <run-id> [--cursor <call-index>] [--limit <1-100>]
  kaioken workflows list [--limit <1-50>]
  kaioken workflows stop <run-id>

Commands must run from a Kaioken project thread. Workflows has six plugin
settings, configurable with `kaioken plugin config workflows set <key> <value>`:
...
remote hosts without granting the plugin arbitrary filesystem access. Use `kaioken
provider list --environment "$BB_ENVIRONMENT_ID" --json` and then `kaioken provider
...
`kaioken plugin install memory`. Once installed, it injects a compact global and
...
deletion.
  ...
  kaioken memory catalog [--scope project|global|all] [--json]
  kaioken memory search <query> [--scope project|global|all] [--json]
  kaioken memory get <id> [--scope project|global|all] [--json]
  kaioken memory add --scope project|global --name <name> --summary <text>
                --details <text> --reason <text> [--kind <kind>]
                [--tag <tag>]... [--importance <0-100>] [--pinned] [--json]
  kaioken memory update <id> --expected-version <n> [fields...] [--json]
  kaioken memory forget <id> --expected-version <n> --reason <text> [--json]
  kaioken memory history <id> [--scope project|global|all] [--limit 1-100] [--json]
...
`kaioken plugin install docs`. Read-only discovery remains direct, while edits use
a manifest-backed local workspace:

  kaioken docs vaults [--json]
  kaioken docs list [--vault <id>] [--json]
  kaioken docs read <path> [--vault <id>]
  kaioken docs pull <path> [--folder] [--vault <id>] [--into <dir>]
  kaioken docs pull --all [--vault <id>] [--into <dir>]
  kaioken docs status [workspace-dir] [--delete] [--diff] [--json]
  kaioken docs push [workspace-dir] [--delete] [--dry-run] [--diff] [--json]

Pull preserves vault-relative paths and writes `.kaioken-docs-state.json`; edit the
...
`kaioken plugin install tasks`. It adds a task tracker, agent delegation,
and the `kaioken tasks` command. Common agent operations are:

  kaioken tasks show <key-or-id> [--json]
  kaioken tasks list [--project <prefix-or-id>] [filters...] [--sort manual|priority|due] [--limit 1-500] [--cursor <opaque>] [--json]
  kaioken tasks comment <key-or-id> (--body <markdown> | --body-file <path>) [--json]
  kaioken tasks attachment add <key-or-comment-id> --file <path> [--json]
  kaioken tasks attachment get <attachment-id> --out <path> [--json]
  kaioken tasks attach <key-or-id> [--thread <thread-id>] [--json]
  kaioken tasks detach <key-or-id> [--thread <thread-id>] [--json]
  kaioken tasks update <key-or-id> --status in_review [--json]
  kaioken tasks update <key-or-id> (--parent <parent-key-or-id> | --no-parent) [--json]

Run `kaioken tasks --help` for project, folder, task, label, attachment, and demo-data
commands, plus preset management, delegation, and attached-thread inspection.
Delegated threads are attached automatically; use `kaioken tasks attach` only when
work started outside Tasks, and `kaioken tasks detach` when a thread is done with a
task or a respawned worker replaced it. `kaioken tasks threads <key>` lists live
...
dotenv reconciliation:
  ...
  kaioken secret request <NAME...> --write-env <path>
...
added/updated/unchanged counts.
  ...
  kaioken plugin search <query>       Search the store: the plugins bundled with
                                 the app plus every registered marketplace
                                 catalog. Results include a Category column
  kaioken plugin install <entry>      Install a bundled official plugin by name
                                 ...
                                 .kaioken/plugins.json entry with that name
  ...
  kaioken plugin outdated             Check installed plugins for compatible
                                 updates (table; --json for raw results).
                                 Columns: installed, latest compatible,
                                 blocked newer (incompatible releases not
                                 selected), status. Dev builds (kaioken 0.0.0)
                                 annotate that engines.bb is not enforced
  kaioken plugin update <id> | --all  Apply compatible updates for one plugin or
                                 ...
                                 installs stay put
  kaioken plugin list                 Status, services, schedules, handler timings.
                                 `kaioken status` also names enabled plugins that
                                 are incompatible, failed, or missing
  kaioken plugin source <id> [--json] Show requested/resolved source, subdirectory,
                                 semver range with its tag prefix and resolved
                                 tag, engine ranges, install time, and recent
                                 activation history
  kaioken plugin enable|disable <id>  Load or unload an installed plugin
  kaioken plugin reload [id]          Re-run factories against current sources.
                                 Exits 1 when a plugin does not come up on
                                 them (previous instance kept, or degraded
                                 because a service ignored its abort)
  kaioken plugin config <id> [set <key> <value> | unset <key>]
                                 Show or change a plugin's declared settings
  kaioken plugin logs <id> [-n N] [-f]  Print (or follow) a plugin's bb.log output
  kaioken plugin run <id> [args...]   Run a plugin command explicitly (also works when core owns its name)
  kaioken plugin token <id> [--rotate]  Print the token for auth:"token" HTTP
                                 routes; --rotate generates a new token,
                                 invalidating the old one
  kaioken plugin remove <id>          Uninstall and delete the plugin's settings,
                                 secrets, and schedules (managed git:/npm:
                                 files deleted; local path sources stay on
                                 disk; builtin removals are remembered)
  kaioken plugin new <name>           Scaffold a todo-list plugin (server.ts,
                                 app.tsx with a sidebar page, a `kaioken <id>` CLI
                                 command, and a skill) and install its npm
                                 dependencies, including @get-kaioken/plugin-sdk
                                 pinned to this kaioken's exact SDK version (no
                                 server required)
  kaioken plugin types [path]         Sync a plugin's @get-kaioken/plugin-sdk surface to
                                 this kaioken (default: cwd): repin the npm
                                 devDependency to this kaioken's SDK version and
                                 the type-only devDependencies of the packages
                                 kaioken shims at runtime (sonner, vaul, the portal
                                 radix families, ...) to this kaioken's versions, or
                                 rewrite the vendored types/ of a plugin that
                                 still carries them; --check writes nothing
                                 and exits non-zero on a mismatch
  kaioken plugin migrate [path]       Switch a plugin that still vendors types/ to
                                 the @get-kaioken/plugin-sdk npm package (default:
  ...
  kaioken plugin build [path]         Compile the plugin into dist/ — the backend
                                 ...
                                 *.meta.json is stamped with SDK
                                 ...
                                 pluginId, pluginVersion, and builtWith (kaioken +
                                 plugin SDK versions); no server required
  kaioken plugin dev [path]           Watch a plugin's sources (default: cwd) and
  ...
  kaioken marketplace add <source>    Add a marketplace from an https manifest URL,
                                 git:<url>[@<ref>], or path:<directory>. kaioken
                                 validates the manifest, caches the catalog,
                                 and fetches the entry icons. Adding a
                                 marketplace installs nothing
  kaioken marketplace list            Name, source, entry count, and last refresh of
                                 every marketplace (--json for raw rows)
  kaioken marketplace refresh [name]  Re-read one catalog, or every one of them.
                                 Discovery metadata and icons only — a refresh
                                 never installs, updates, or runs plugin code.
                                 A failed refresh keeps the last catalog kaioken
                                 validated and exits non-zero
  kaioken marketplace remove <name>   Forget a marketplace. Its catalog rows and
...
ordinary plugin package with its own package.json and kaioken manifest. An optional
collection manifest at .kaioken/plugins.json indexes them:
...
Install one plugin of the repository:
  ...
  kaioken plugin install git:github.com/acme/repo@main --plugin sidebar
  kaioken plugin install git:github.com/acme/repo@main --subdirectory plugins/sidebar
  kaioken plugin install path:/work/repo --plugin sidebar

--subdirectory is the primitive and works without a collection manifest.
--plugin resolves a name from .kaioken/plugins.json. Installs from one repository
...
and lists the entry names. kaioken records the subdirectory, so outdated, update,
rollback, and remove keep working per plugin.

BB Official plugins

Kaioken's official plugins ship inside the app. The reserved `bb-official`
marketplace describes these plugins with the standard v2 format. Its catalog
uses a local path. It never uses the network. `kaioken marketplace list` shows it
...
For example, use
`kaioken plugin install docs` or `kaioken plugin install docs@bb-official`. kaioken copies the
...
reviewed plugins that live outside the app bundle. kaioken requests the v2 manifest
from https://getbb.app/marketplace/v2/marketplace.json. A 404 response makes
kaioken request the v1 manifest. Other errors do not cause this fallback. Set
KAIOKEN_MARKETPLACE_URL to override the URL. kaioken reads the manifest at startup and
every two hours.

kaioken stores the last catalog that it validated. An invalid manifest keeps that
...
the short description, and `kaioken plugin search --json` returns it as `overview`.
An install uses the normal git or npm source pipeline. kaioken records the source
marketplace.

The BB Community marketplace also publishes install counts beside its
manifest, at https://getbb.app/marketplace/v1/stats.json. kaioken re-reads that
file on every refresh — the counts move while the manifest sits unchanged —
and shows them in the store and in the Installs column of `kaioken plugin search`.
The number is how many Kaioken installations reported installing the plugin
through anonymous telemetry, so it undercounts: telemetry is opt-out and only
production builds report. No third-party marketplace has counts; kaioken measures
them itself rather than repeating a publisher's claim.

BB Official entries use the same counts. kaioken finds each count in the Kaioken
Community `stats.json` file by the plugin id.

Third-party marketplaces

Anyone can host a marketplace manifest. Add one with its https manifest URL,
with git:<url>[@<ref>] (kaioken reads marketplace.json from the checkout), or with
path:<directory> on the kaioken server's machine:

  kaioken marketplace add https://plugins.acme.dev/marketplace.json
  kaioken marketplace add git:github.com/acme/kaioken-marketplace@main
  kaioken marketplace add path:/work/acme-marketplace

The manifest `name` is the marketplace identity. kaioken refuses a duplicate name.
...
loads screenshots from the URLs that the marketplace declares. kaioken clones a
git marketplace into a temporary checkout. kaioken keeps only the validated
manifest and icon bytes.

kaioken ignores unknown v2 fields, except in npm and git source objects. kaioken rejects
...
Install an entry of a specific marketplace with <entry-id>@<marketplace>:
  ...
  kaioken plugin install thread-hover-cards@acme-plugins
...
Before an install from a third-party marketplace, kaioken resolves and
...
The install must still match these confirmed source facts. kaioken refuses the
...
resolution, so `kaioken plugin outdated` and `kaioken plugin update` keep working from
...
For direct git:/npm: installs, updates are manual: `kaioken plugin outdated`
checks tracking sources and `kaioken plugin update` applies compatible candidates.
Reinstalling an already-installed managed plugin is refused — use
`kaioken plugin update`. A failed activation restores the pre-update snapshot and
...
pinned git:/npm: source changes only through `kaioken plugin remove` (which
...
`kaioken plugin reload <id>`, or `kaioken plugin install path:<new dir>` to move it to
...
repository's tags:
  ...
  kaioken plugin install git:github.com/acme/repo@^1.2.0
  kaioken plugin install git:github.com/acme/repo@semver:^1.2.0
  kaioken plugin install git:github.com/acme/repo@^1.2.0 --tag-prefix notes/

kaioken lists refs/tags, keeps the tags named [<tag-prefix>]vX.Y.Z that parse as
...
kaioken records the tag it selected and the commit that tag pointed at. If that
tag later points at another commit, kaioken refuses to resolve it and names both
...
`kaioken plugin search <query>` matches an id, name, description, category, or tag.
It searches bb-official and each other registered marketplace. The output has a
Category column. Status shows installed, compatible, or requires newer kaioken.
...
replaced by the bundles kaioken builds. Path installs compile dist/ at install time
...
install is refused. The server rebuilds source-built apps after a kaioken upgrade.
...
kaioken ships no build toolchain. The first time a git or path plugin is built on
a machine, kaioken downloads a pinned esbuild + Tailwind set into
...
kaioken — depend on the published `kaioken-app` package and call the CLI:

```jsonc
// your plugin's package.json
"devDependencies": { "kaioken-app": "^0.35.1" },
"scripts": { "build": "kaioken plugin build" }
```

`kaioken plugin build` talks to no server. Depending on `kaioken-app@X` builds with
...
the download on later runs. Only `kaioken plugin dev` needs a running kaioken, because
...
`@get-kaioken/plugin-sdk` entrypoints, Node APIs, and ordinary dependencies, but no
private `@kaioken/*` workspace packages; the host build rejects direct, transitive,
...
SDK package at runtime. That covers the bare `@get-kaioken/plugin-sdk` import. An
SDK subpath (`@get-kaioken/plugin-sdk/host`, `/provider-bridge`,
...
rather than shipping an import kaioken cannot serve.
Path installs always load server.ts from source, so `kaioken plugin dev`/reload see
edits immediately.

`kaioken plugin dev` is the edit loop: it requires the directory to already be
installed as a plugin (`kaioken plugin install .` first), ignores dist/,
...
`@get-kaioken/plugin-sdk/app` and register UI slots: homepageSection (root compose),
...
shadcn model): `kaioken plugin new` pre-vendors a starter set into
components/ui/ and `npx shadcn add @kaioken/<name>` pulls more from the Kaioken
component registry (the full stock shadcn set, version-matched to the
running Kaioken via the pinned ref in components.json). Product capabilities are
...
same catalog and picker as kaioken's composers; provider switches emit only after
...
host's version — the scaffold declares all of them and `kaioken plugin types`
...
@pierre/diffs directly, so kaioken owns patch normalization, syntax
...
expand-context controls without exposing Pierre types. Kaioken's original renderer
...
Everything else (zod included) bundles from the plugin's node_modules (`npm install` for authors; Kaioken installs
...
(same data as `kaioken plugin config`) appear under both Settings → Installed plugins
...
example `kaioken github …`). Unknown `kaioken` commands are looked up against installed
...
and `kaioken plugin list` shows the required `kaioken plugin run <id>` form. Inside agent
threads the generated `plugin-commands` skill lists the available plugin commands.

Settings changes do not auto-reload a plugin — run `kaioken plugin reload <id>`
...
shared `@get-kaioken/plugin-sdk` constant. Results above the ceiling are rejected in
...
The loop: `kaioken plugin new <name>` scaffolds `./kaioken-plugin-<name>` — a working
todo list with a backend, a sidebar page, a `kaioken <name>` command, and a skill;
delete what you do not need; `kaioken plugin install .` registers it; `kaioken plugin
...
`skills/`), `engines.bb` (supported kaioken range),
...
`kaioken-plugin-hello` for the package name by
default. Scoped names such as `@acme/kaioken-plugin-hello` are also supported. The
plugin id is the final package-name component minus `kaioken-plugin-`, so both forms
use `hello`.
...
Appearance and `kaioken theme list`; their selectable id is
`plugin:<plugin-id>:<theme-id>`. Disabling or removing the owning plugin
makes kaioken fall back to the default palette.

Branding is explicit. Declare `bb.branding.icon` as either the plugin's
canonical Kaioken icon name or a plugin-relative compact SVG such as
`./assets/icon.svg`. Kaioken validates and hash-serves path-shaped SVGs, then
...
`kaioken plugin build` refuses an SVG logo that carries a script vector (a
...
never refuse a logo, and every SVG kaioken serves carries `nosniff` and a
...
is required. Do not duplicate the same artwork across fields. Kaioken rejects nulls,
...
package @get-kaioken/plugin-sdk, pinned to this kaioken's exact SDK version, so
`npm install && npx tsc --noEmit` typechecks anywhere — no kaioken checkout
needed. The full API lands at
node_modules/@get-kaioken/plugin-sdk/bundled-types/kaioken-plugin-sdk.d.ts (plus
...
works for existing entries. Run `kaioken plugin migrate` before adding `bb.host` so
the `/host` and `/testing/host` declaration subpaths are available; migration
shows every change and asks first.
The SDK surface grows every release, so `kaioken plugin types` syncs a plugin to
the running kaioken — repinning the SDK devDependency and the shimmed packages'
type-only devDependencies, or rewriting types/ for a plugin that still
vendors them. Run it in a cloned or older plugin, and `kaioken
plugin types --check` in CI. `kaioken plugin build` and `kaioken plugin dev` keep a
...
one line each — bb.log (plugin-scoped logger behind `kaioken plugin logs`);
bb.settings.define (declarative settings incl. secrets, editable via
`kaioken plugin config`); bb.storage.kv (JSON rows ≤256KB) and
bb.storage.database()+migrate (the plugin's own database); bb.sdk (the full
kaioken SDK — handlers/services only, not the factory; spawned threads are
...
`@get-kaioken/plugin-sdk/host` and can be unit-tested with
experimental_createHostEntryHarness from
`@get-kaioken/plugin-sdk/testing/host`);
...
`kaioken <name>` command agents run through bash, with a shared 1 MiB combined
...
the built-in `kaioken-plugin-authoring` skill (agents: it loads on demand;
humans: plugins/kaioken-guide/skills/kaioken-plugin-authoring/
...
Cards installs from the BB Community marketplace (source: the kaioken-plugins
repo).
