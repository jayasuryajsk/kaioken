...
async function initRepo(): Promise<string> {
  ...
  await runGit({ args: ["config", "user.name", "Kaioken Tests"], cwd: repoPath });
  await runGit({
    args: ["config", "user.email", "kaioken@example.com"],
    cwd: repoPath,
  });
...
async function addDetachedWorktree(repoPath: string): Promise<string> {
  const worktreeParent = await makeTempDir("kaioken-workspace-worktree-");
...
async function replaceDotGitWithSymlink(repoPath: string): Promise<void> {
  ...
  const gitDirParent = await makeTempDir("kaioken-workspace-gitdir-");
...
describe.sequential("watchWorkspaceStatus", () => {
  it("starts watching before git init and promotes the repository watch", async () => {
    const workspacePath = await makeTempDir("kaioken-workspace-plain-");
