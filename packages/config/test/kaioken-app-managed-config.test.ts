...
import {
  kaiokenAppManagedConfigSchema,
  formatCustomAcpAgentProviderId,
  parseBbAppManagedConfig,
} from "../src/kaioken-app-managed-config.js";

describe("kaiokenAppManagedConfigSchema", () => {
  ...
  it("parses custom models with a known provider", () => {
    const parsed = kaiokenAppManagedConfigSchema.parse({
      customModels: [
        {
  ...
  it("rejects malformed acp-* custom model provider ids", () => {
    for (const providerId of ["acp-", "acp-Bad-Agent", "acp--x"]) {
      expect(
        kaiokenAppManagedConfigSchema.safeParse({
  ...
  it("rejects custom models with an unknown provider", () => {
    const result = kaiokenAppManagedConfigSchema.safeParse({
      customModels: [
  ...
  it("rejects an unsupported custom ACP logo format", () => {
    const parsed = kaiokenAppManagedConfigSchema.safeParse({
      customAcpAgents: [
        {
          id: "my-agent",
          displayName: "My Agent",
  ...
  it("refuses the removed `absolute` side by name on sharedSkillRoots and on a custom ACP agent", () => {
    const shared = kaiokenAppManagedConfigSchema.safeParse({
