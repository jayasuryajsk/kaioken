...
import { defaultFeatureFlags, hostTypeSchema, type HostType } from "@kaioken/domain";
...
import { KAIOKEN_LOOPBACK_HOST, parsePortValue } from "./runtime.js";
...
export function parseServerBindHost(value: string): ServerBindHost {
  ...
  throw new Error('KAIOKEN_SERVER_BIND_HOST must be "127.0.0.1" or "0.0.0.0"');
}
...
export const KAIOKEN_LOG_LEVEL_ENV = defineEnvVar<string>({
  description: "Log level: trace, debug, info, warn, error, fatal",
  name: "KAIOKEN_LOG_LEVEL",
  parse: parseLogLevelValue,
});

export const KAIOKEN_SERVER_PORT_ENV = defineEnvVar<number>({
  description: "HTTP port for the server",
  name: "KAIOKEN_SERVER_PORT",
  parse: parsePortEnvValue,
});

export const KAIOKEN_SERVER_BIND_HOST_ENV = defineEnvVar<ServerBindHost>({
  description: "HTTP bind host for the server",
  name: "KAIOKEN_SERVER_BIND_HOST",
  parse: parseServerBindHostEnvValue,
});

export const KAIOKEN_HOST_DAEMON_PORT_ENV = defineEnvVar<number>({
  description: "Port the host daemon listens on for local API requests",
  name: "KAIOKEN_HOST_DAEMON_PORT",
  parse: parsePortEnvValue,
});

export const KAIOKEN_SERVER_URL_ENV = defineEnvVar<string>({
  description: "URL of the kaioken server",
  name: "KAIOKEN_SERVER_URL",
  parse: parseRequiredUrlEnvValue,
});

export const KAIOKEN_APP_VERSION_ENV = defineEnvVar<string>({
  description:
    "Version of the running kaioken-app package. The kaioken-app launcher sets this from packages/kaioken-app/package.json; defaults to a sentinel for dev/source runs.",
  name: "KAIOKEN_APP_VERSION",
  parse: parseNonEmptyStringEnvValue,
});

export const KAIOKEN_SERVER_LAUNCH_ID_ENV = defineEnvVar<string>({
  description:
    "Internal per-spawn identity the kaioken-app launcher hands its server child. The server echoes it on /health so the launcher can tell its own child apart from another kaioken server that already owns the port.",
  name: "KAIOKEN_SERVER_LAUNCH_ID",
  parse: parseNonEmptyStringEnvValue,
});

export const KAIOKEN_APP_SURFACE_ENV = defineEnvVar<AppSurface>({
  description:
    "Internal launcher marker for telemetry attribution. Set by kaioken-app and desktop launchers.",
  name: APP_SURFACE_ENV_NAME,
  parse: parseAppSurfaceEnvValue,
});

export const KAIOKEN_APP_URL_ENV = defineEnvVar<string>({
  description:
  ...
  name: "KAIOKEN_APP_URL",
  parse: parseOptionalUrlEnvValue,
});

export const KAIOKEN_EXTERNAL_URL_ENV = defineEnvVar<string>({
  description:
    "Internet-facing HTTPS base URL used for generated public links. Does not control which host or port the server binds to.",
  name: "KAIOKEN_EXTERNAL_URL",
  parse: parseOptionalUrlEnvValue,
});

export const KAIOKEN_MARKETPLACE_URL_ENV = defineEnvVar<string>({
  description:
  ...
  name: "KAIOKEN_MARKETPLACE_URL",
  parse: parseOptionalUrlEnvValue,
});

export const KAIOKEN_INFERENCE_ENV = defineEnvVar<string>({
  description: "Inference model used for server-side completions",
  name: "KAIOKEN_INFERENCE",
  parse: parseInferenceModelValue,
});

export const KAIOKEN_INFERENCE_FALLBACK_ENV = defineEnvVar<string>({
  description:
...
export const KAIOKEN_TRANSCRIPTION_ENV = defineEnvVar<string>({
...
export const KAIOKEN_POSTHOG_API_KEY_ENV = defineEnvVar<string>({
  description:
    "PostHog project API key for anonymous usage telemetry. Telemetry is disabled when empty.",
  name: "KAIOKEN_POSTHOG_API_KEY",
  parse: parseStringEnvValue,
});

export const KAIOKEN_TELEMETRY_ENV = defineEnvVar<boolean>({
  description:
  ...
  name: "KAIOKEN_TELEMETRY",
  parse: parseBooleanEnvValue,
});

export const KAIOKEN_FF_PLACEHOLDER_ENV = defineEnvVar<boolean>({
  description:
  ...
  name: "KAIOKEN_FF_PLACEHOLDER",
  parse: parseBooleanEnvValue,
});

export const KAIOKEN_FF_TIMELINE_WINDOW_EVENT_BUDGET_ENV = defineEnvVar<number>({
  description:
    "Max events one thread-timeline window may span. Raise far above the default to restore unbounded windows.",
  name: "KAIOKEN_FF_TIMELINE_WINDOW_EVENT_BUDGET",
  parse: parsePositiveIntegerEnvValue,
});

export const KAIOKEN_DEV_APP_HOST_ENV = defineEnvVar<string>({
  description:
    "Development-only Vite bind host override for apps/app. Defaults to 127.0.0.1 when unset.",
  name: "KAIOKEN_DEV_APP_HOST",
  parse: parseStringEnvValue,
});

export const KAIOKEN_DEV_APP_PORT_ENV = defineEnvVar<number | undefined>({
  description: "Development-only Vite port for apps/app.",
  name: "KAIOKEN_DEV_APP_PORT",
  parse: parseOptionalPortEnvValue,
});

export const KAIOKEN_CLI_DIR_ENV = defineEnvVar<string | undefined>({
  description:
    "Directory containing the kaioken CLI executable to inject into runtime shells",
  name: "KAIOKEN_CLI_DIR",
  parse: parseOptionalTrimmedStringEnvValue,
});

export const KAIOKEN_INHERITED_SKILLS_ROOTS_ENV = defineEnvVar<string[]>({
  description:
    "Development-only path list of lower-priority inherited kaioken skill roots",
  name: "KAIOKEN_INHERITED_SKILLS_ROOTS",
  parse: parsePathListEnvValue,
});

export const KAIOKEN_BRIDGE_DIR_ENV = defineEnvVar<string | undefined>({
  description:
    "Directory containing provider bridge bundles for the host daemon runtime",
  name: "KAIOKEN_BRIDGE_DIR",
  parse: parseOptionalTrimmedStringEnvValue,
});

export const KAIOKEN_CONNECT_MACHINE_CREDENTIAL_ENV = defineEnvVar<
  string | undefined
>({
  description:
    "Daemon-managed kaioken connect credential for traversing the public machine gate",
  name: "KAIOKEN_CONNECT_MACHINE_CREDENTIAL",
  parse: parseOptionalTrimmedStringEnvValue,
});

export const KAIOKEN_CONNECT_MACHINE_ID_ENV = defineEnvVar<string>({
  description: "Cloud machine identifier paired with the kaioken connect credential",
  name: "KAIOKEN_CONNECT_MACHINE_ID",
  parse: parseNonEmptyStringEnvValue,
});

export const KAIOKEN_HOST_ENROLL_KEY_ENV = defineEnvVar<string | undefined>({
  description:
    "One-time enrollment token used to bootstrap a host daemon with the kaioken server",
  name: "KAIOKEN_HOST_ENROLL_KEY",
  parse: parseOptionalTrimmedStringEnvValue,
});

export const KAIOKEN_HOST_DAEMON_AUTO_UPDATE_ENV = defineEnvVar<boolean>({
  description:
    "Allow a remote host daemon to install the exact kaioken-app package served by its server on a newer protocol mismatch",
  name: "KAIOKEN_HOST_DAEMON_AUTO_UPDATE",
  parse: parseBooleanEnvValue,
});

export const KAIOKEN_HOST_ID_ENV = defineEnvVar<string | undefined>({
  description:
    "Preferred host ID to persist for the daemon instead of generating one locally",
  name: "KAIOKEN_HOST_ID",
  parse: parseOptionalTrimmedStringEnvValue,
});

export const KAIOKEN_HOST_NAME_ENV = defineEnvVar<string | undefined>({
  description:
    "Preferred host name to report instead of detecting the local hostname",
  name: "KAIOKEN_HOST_NAME",
  parse: parseOptionalTrimmedStringEnvValue,
});

export const KAIOKEN_HOST_TYPE_ENV = defineEnvVar<HostType | undefined>({
  description: "Host type override for daemon bootstrap",
  name: "KAIOKEN_HOST_TYPE",
  parse: parseHostTypeValue,
});

export const DEFAULT_KAIOKEN_APP_VERSION = DEFAULTS.appVersion;
export const DEFAULT_KAIOKEN_APP_SURFACE = DEFAULT_APP_SURFACE;
export const DEFAULT_KAIOKEN_APP_URL = "";
export const DEFAULT_KAIOKEN_SERVER_BIND_HOST: ServerBindHost = KAIOKEN_LOOPBACK_HOST;
export const DEFAULT_KAIOKEN_EXTERNAL_URL = "";
export const DEFAULT_OPENAI_API_KEY = "";
export const DEFAULT_KAIOKEN_POSTHOG_API_KEY =
  "phc_tejoYoNLV6vG8QAd5eYXXvcsENFYnP4brpZDGqG7zvpy";
export const DEFAULT_KAIOKEN_TELEMETRY = false;
export const DEFAULT_KAIOKEN_DEV_APP_HOST = "";
export const DEFAULT_KAIOKEN_MARKETPLACE_URL =
  "https://getbb.app/marketplace/v2/marketplace.json";
export const DEFAULT_KAIOKEN_INFERENCE = DEFAULTS.inferenceModel;
export const DEFAULT_KAIOKEN_INFERENCE_FALLBACK = DEFAULTS.inferenceFallbackModel;
export const DEFAULT_KAIOKEN_TRANSCRIPTION = DEFAULTS.transcriptionModel;
export const DEFAULT_KAIOKEN_FF_PLACEHOLDER = defaultFeatureFlags.placeholder;
export const DEFAULT_KAIOKEN_FF_TIMELINE_WINDOW_EVENT_BUDGET =
