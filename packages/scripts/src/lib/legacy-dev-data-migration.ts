...
import type { DevInstanceConfig } from "@kaioken/config/runtime";
...
const LEGACY_DEV_DATA_DIR_NAME = ".kaioken-dev";
...
function isMigratableLegacyEntryName(entryName: string): boolean {
  return (
    MIGRATABLE_LEGACY_ENTRY_NAMES.has(entryName) || /^kaioken\.db\./u.test(entryName)
  );
}
