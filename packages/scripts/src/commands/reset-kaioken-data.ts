...
function resolveResetDataDir(mode: KaiokenRuntimeMode): string {
  if (mode === "dev") {
...
export function renderHelpText(): string {
  ...
  ${dim("Notes")}
    Removes kaioken-managed state directories (${dim("~/.kaioken")}, ${dim("~/.kaioken-dev/<checkout-instance>")}).
    Does not touch external provider config managed by other tools.
    Production resets respect KAIOKEN_DATA_DIR. Development resets always target this checkout's dev data directory.
\n`;
}
...
async function confirmReset(targets: string[]): Promise<boolean> {
  ...
  try {
    ...
    log(yellow("!"), "This will permanently delete kaioken-managed local data at:");
    for (const target of targets) {
      log(" ", dim(target));
    }
    process.stdout.write("\n");
    log(
      " ",
      dim("Provider auth/config managed outside kaioken will be left untouched."),
    );
    process.stdout.write("\n");
...
async function main(argv: string[] = process.argv.slice(2)): Promise<void> {
  ...
  if (removedCount === 0) {
    log(dim("●"), "No kaioken-managed data directories were present");
  } else {
