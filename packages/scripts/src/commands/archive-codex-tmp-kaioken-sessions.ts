...
import { resolveCodexHome } from "@kaioken/config/codex-home";
import { bold, cyan, dim, green, yellow, log } from "../lib/script-helpers.js";

const DEFAULT_TMP_KAIOKEN_PATTERNS: readonly string[] = [
  "*/kaioken-standalone-*",
  "*/kaioken-integration-*",
  "*/kaioken-integ-*",
  "*/kaioken-qa-smoke-*",
...
export function parseArchiveTmpBbSessionsArgs(
  ...
  const options: ArchiveTmpBbSessionsOptions = {
    ...
    dryRun: false,
    patterns: [...DEFAULT_TMP_KAIOKEN_PATTERNS],
    yes: false,
  };
...
export function renderHelpText(): string {
  const defaultPatterns = DEFAULT_TMP_KAIOKEN_PATTERNS.join(", ");
  return `
  ${bold("codex archive tmp kaioken sessions")}

  ${dim("Usage")}
    pnpm codex:archive-tmp-kaioken-sessions -- [--dry-run] [--yes]
  ...
  ${dim("Notes")}
    ...
    Defaults cover the kaioken test temp dirs (QA standalone, integration, agent-runtime providers).
...
function buildBackupPath(dbPath: string): string {
  const timestamp = new Date()
    ...
    .replaceAll(":", "")
    .replace(/\.\d{3}Z$/, "Z");
  return `${dbPath}.backup-${timestamp}-archive-tmp-kaioken`;
}
  ...
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    send({
      ...
      params: {
        ...
        clientInfo: {
          name: "archive-tmp-kaioken-sessions",
          title: "archive tmp kaioken sessions",
          version: "0",
        },
      },
    });
