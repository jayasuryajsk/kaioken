...
const launcherPath = join(repoRoot, "scripts", "kaioken-dev-app");
...
describe("kaioken-dev-app", () => {
  it("keeps the caller's Node when another Node 22 version is installed", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "kaioken-dev-node-"));
    try {
      ...
      const result = spawnSync("bash", [launcherPath, "status"], {
        ...
        env: {
          ...process.env,
          KAIOKEN_DEV_REPO_ROOT: repoRoot,
          HOME: tempRoot,
          NVM_DIR: nvmRoot,
