...
function createTestRepo({ kaiokenAppVersion, desktopVersion }) {
  const repoRoot = mkdtempSync(join(tmpdir(), "kaioken-bump-version-"));
  testRoots.push(repoRoot);

  mkdirSync(join(repoRoot, "packages", "kaioken-app"), { recursive: true });
  mkdirSync(join(repoRoot, "apps", "desktop"), { recursive: true });
  writeFileSync(
    join(repoRoot, "packages", "kaioken-app", "package.json"),
    createPackageJson({ name: "kaioken-app", version: kaiokenAppVersion }),
  );
  writeFileSync(
    join(repoRoot, "apps", "desktop", "package.json"),
    createPackageJson({ name: "@kaioken/desktop", version: desktopVersion }),
  );
...
function runScript(repoRoot, args) {
  return spawnSync(process.execPath, [scriptPath, ...args], {
    ...
    env: {
      ...process.env,
      KAIOKEN_BUMP_VERSION_REPO_ROOT: repoRoot,
    },
  });
}
...
describe("bump-version", () => {
  it("exits non-zero for an invalid version argument", () => {
    ...
    expect(readVersion(repoRoot, "packages/kaioken-app/package.json")).toBe("0.0.6");
  ...
  it("rejects a bump lower than the highest current target version", () => {
    ...
    expect(result.stderr).toContain(
      "New version 0.0.7 must be greater than current max 0.0.9 across kaioken-app=0.0.6 @kaioken/desktop=0.0.9.",
    );
    expect(readPackageContent(repoRoot, "packages/kaioken-app/package.json")).toBe(
  ...
  it("updates both package versions for a valid version argument", () => {
    ...
    expect(result.stdout).toContain("Bumped: kaioken-app + @kaioken/desktop → 0.0.7");
    expect(readVersion(repoRoot, "packages/kaioken-app/package.json")).toBe("0.0.7");
  ...
  it("restores the first package file when the second rename fails", async () => {
    ...
    expect(
      readdirSync(join(repoRoot, "packages", "kaioken-app")),
...
describe("prepare-nightly-version", () => {
  ...
  it("updates kaioken-app and desktop to the same nightly version", async () => {
    const repoRoot = createTestRepo({
      kaiokenAppVersion: "1.2.3",
    ...
    expect(readVersion(repoRoot, "packages/kaioken-app/package.json")).toBe(
