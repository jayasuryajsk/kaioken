...
// repo's installed shared-runtime packages. `kaioken plugin build` shims the
...
// output is not committed: turbo runs this as `@kaioken/plugin-build#generate`
...
// The shimmed npm modules, from the same list `kaioken plugin build` shims
...
const RUNTIME_SOURCE_MODULES = {
  "@kaioken/shared-ui/question-form-host": path.join(
    scriptDir,
    "..",
    "..",
    "shared-ui",
  ...
  "@kaioken/shared-ui/icon": path.join(
    scriptDir,
    "..",
    "..",
    "shared-ui",
...
// installed copies.
// Consumed by
// \`kaioken plugin build\` to emit static ESM re-export shims over
