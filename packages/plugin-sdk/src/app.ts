...
/**
 * `@get-kaioken/plugin-sdk/app` — typed facade over the Kaioken app's plugin runtime.
 *
 * This module's runtime is never bundled into plugins: `kaioken plugin build`
 * swaps the specifier for a shim reading
 * `globalThis.__bbPluginRuntime.pluginSdkApp` (which the Kaioken app fills with
 ...
 * Kaioken registry (`npx shadcn add @kaioken/<name>`); `toast` comes from
