# Releasing kaioken-app

This runbook is for agents preparing and publishing the `kaioken-app` npm package.
...
`kaioken-app` npm package (`publish-kaioken-app.yml`) and the desktop app
...
scheduled path in `publish-kaioken-app.yml` derives a unique next-patch prerelease,
publishes it under npm's `nightly` dist-tag, then builds the separately
installable `kaioken Nightly` app for macOS and Linux and publishes both at
...
```bash
gh workflow run publish-kaioken-app.yml \
  --ref main \
  -f npm_tag=nightly \
...
- Publish only a version that exists in `packages/kaioken-app/package.json`.
- Keep `packages/kaioken-app/package.json` and `apps/desktop/package.json` versions
...
2. Check the current npm state.
   ...
   ```bash
   npm view kaioken-app version dist-tags versions --json
   ```
...
3. Bump the lockstep release versions.
   ...
   This updates `packages/kaioken-app/package.json` and
   `apps/desktop/package.json`. Do not run `npm version` directly in
   `packages/kaioken-app`; CI enforces these versions in lockstep.
...
4. Update the release notes. Add the new version's section to the repo-root
   ...
   redeploy `@kaioken/web` after the release commit lands so the site shows the new
   version.
...
6. Run validation.
   ...
   pnpm exec turbo run typecheck test --filter=@kaioken/app --filter=@kaioken/config --filter=@kaioken/server --filter=kaioken-app
   pnpm exec turbo run smoke:tarball --filter=kaioken-app --force
   git diff --check
   ```
...
7. Commit the release change.
   ...
   ```bash
   git add README.md docs packages/kaioken-app/package.json packages/kaioken-app/README.md apps/desktop/package.json
   git commit -m "Prepare kaioken-app <version>"
   ```
...
same commit, so the run also builds the `kaioken Nightly` desktop app. Expect the run
...
```bash
npm_tag=latest
npm view "kaioken-app@$npm_tag" version
npm view kaioken-app version dist-tags versions --json
npx --yes "kaioken-app@$npm_tag" --help
```
...
curl -fsSL https://github.com/jayasuryajsk/kaioken/releases/download/desktop-latest/desktop-version.json
```
...
`@get-kaioken/plugin-sdk` rides the same release train as `kaioken-app`. The
`publish-plugin-sdk` job in `publish-kaioken-app.yml` runs on the same nightly cron
...
1. Publish `@get-kaioken/plugin-sdk` once by hand from `packages/plugin-sdk` with
...
3. On npmjs.com, add a trusted publisher for the package pointing at this
   repository, the workflow file `.github/workflows/publish-kaioken-app.yml`, and the
   `npm-release` environment — the same values `kaioken-app` uses.

Until all three steps are done, the `publish-plugin-sdk` job fails at
`npm publish`. That failure is expected and isolated; the kaioken-app and desktop
jobs are unaffected.
...
- If local emergency publish is unavoidable, use `npm publish --tag <tag>` from
  `packages/kaioken-app` only after explicit user approval and record the OTP-based
  path as a deviation.
- If a legacy prerelease tag should stop resolving to an old build, remove it
  with `npm dist-tag rm kaioken-app <tag>` using explicit user approval and normal
  npm authentication.
