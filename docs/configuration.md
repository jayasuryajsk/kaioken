...
The packaged `npx kaioken-app` flow stores persistent package settings under
`~/.kaioken/config.json`, provider environment values under `~/.kaioken/env.json`, and
client SSH target mappings under `~/.kaioken/client.json`.

Use `kaioken-app config` for non-secret kaioken settings:
...
npx kaioken-app config unset KAIOKEN_APP_URL
npx kaioken-app config refresh
```

Use `kaioken-app env` for provider credentials and provider-specific environment:
...
Commit `.kaioken-env-setup.sh` when a managed worktree needs repository setup.
Commit `.kaioken-env-teardown.sh` when kaioken must release external resources before it
...
`kaioken-app config list` shows non-secret values. `kaioken-app env list` redacts every
...
secret managed by kaioken connect: do not copy, edit, or commit it. Both fields are
intentionally omitted from `kaioken-app config list`. At runtime they are passed to
the standalone host daemon and its bundled `kaioken` CLI as
`KAIOKEN_CONNECT_MACHINE_CREDENTIAL` and `KAIOKEN_CONNECT_MACHINE_ID`. These are
installer-managed transport details, not user configuration knobs; re-add the
machine instead of setting them by hand.

Use `kaioken-app client ssh-target` to let a local helper open files from a remote
kaioken server in local editors. The SSH target is the value that works after
...
npx kaioken-app client ssh-target remove https://kaioken.example.test --host-id host_abc
```

Use `--host-id` when the server has more than one machine; copy the ID from
`kaioken machine list`. Omit it to preserve the single-machine auto-selection for
...
1. Explicit launcher flags, such as `--data-dir`, `--server-port`, or
   `--server-bind-host`.
2. Persistent `kaioken-app config`, `kaioken-app env`, and client values.
3. Ambient shell environment.
4. Built-in defaults.

For the packaged app, prefer `kaioken-app config`, `kaioken-app env`, and launcher flags
...
`KAIOKEN_DEV_CONNECT_BASE_URL=http://kaioken.localhost:<worktree-cloud-port>`. The
Connect plugin accepts this loopback origin only when `NODE_ENV=development`
and uses it only as the unpaired default. Explicit `kaioken connect --server ...`
or `--base-url ...` targets take precedence, and packaged/production kaioken keeps
the `https://getbb.app` default. This value is launcher-managed, not a
`kaioken-app config` setting.

After `kaioken-app config` writes `~/.kaioken/config.json` or `kaioken-app env` writes
`~/.kaioken/env.json`, it asks the running local server to reload. If kaioken is not
running, the new values apply on the next start. If you edit either file by
hand, run `npx kaioken-app config refresh` to apply the files to a running server.

The live reload applies config keys such as `KAIOKEN_APP_URL`, `KAIOKEN_INFERENCE`,
`KAIOKEN_INFERENCE_FALLBACK`, and `KAIOKEN_TRANSCRIPTION`, plus env values explicitly
consumed at runtime such as `OPENAI_API_KEY`. If one of those config keys is
stored with `kaioken-app env` instead, it is startup-only; use `kaioken-app config` when
you need a live change.

`KAIOKEN_LOG_LEVEL` is the startup-only `kaioken-app config` key. The complete current
set of startup-only server or launcher env entries is:

- `KAIOKEN_APP_SURFACE`, `KAIOKEN_APP_URL`, `KAIOKEN_DATA_DIR`, `KAIOKEN_DEV_APP_PORT`, and
  `KAIOKEN_EXTERNAL_URL`
- `KAIOKEN_HOST_DAEMON_PORT`, `KAIOKEN_INFERENCE`,
  `KAIOKEN_INFERENCE_FALLBACK`, and `KAIOKEN_INHERITED_SKILLS_ROOTS`
- `KAIOKEN_LOG_LEVEL`, `KAIOKEN_MANAGED_DEV_BUILTIN_PLUGIN_HOT_RELOAD`,
  `KAIOKEN_MARKETPLACE_URL`, `KAIOKEN_POSTHOG_API_KEY`, and `KAIOKEN_TELEMETRY`
- `KAIOKEN_SERVER_BIND_HOST`, `KAIOKEN_SERVER_PORT`, `KAIOKEN_TRANSCRIPTION`, and all
  `KAIOKEN_FF_*` feature flags
...
launcher restart (`kaioken-app stop && kaioken-app start`) or by restarting the desktop
app. In particular, changing or unsetting `KAIOKEN_SERVER_BIND_HOST` does not close
an existing `0.0.0.0` listener until that restart.

`kaioken-app config refresh` also notes any startup-only keys currently present in
...
`--server-port` to `kaioken-app config` or `kaioken-app env` commands so they write the
right file and refresh the right server.

## Stopping A Running kaioken

A running `kaioken-app start` writes `<dataDir>/kaioken-app-runtime.json` and removes the
file when it exits. The file records the launcher process id, the server URL,
the version, the start time, and how kaioken was started. Do not edit it.

Two things read that file:

- `npx kaioken-app stop` stops the kaioken that owns the data directory. Pass the same
  `--data-dir` you started with when it is not the default `~/.kaioken/`.
- The macOS desktop app asks before it uses a kaioken it did not start, and offers to
  stop that copy for you.

Both confirm that the recorded process really is a kaioken launcher before they
...
| ---------------------------- | ------------------------------------------------------- | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `KAIOKEN_APP_URL`            | `kaioken-app config`                                    | Optional for remote use | Human-facing app URL used for generated links and allowed browser origins. Leave empty for local-only use.                                                                                                                                                                                                                                                                                                          |
| `KAIOKEN_INFERENCE`          | `kaioken-app config`                                    | Optional                | Primary server-side helper model in `<service>/<model>` format, where `<service>` is an AI service a loaded plugin registers (`kaioken settings ai-services` lists them; `codex` comes with the codex plugin and uses the codex CLI's credentials with no reasoning) or a pi-ai provider the server calls directly with its API key. Defaults to `codex/gpt-5.6-luna`.                                              |
| `KAIOKEN_INFERENCE_FALLBACK` | `kaioken-app config`                                    | Optional                | Helper model used after a transient primary timeout, rate limit, or service-unavailable failure. Defaults to `codex/gpt-5.4-mini`.                                                                                                                                                                                                                                                                                  |
| `KAIOKEN_TRANSCRIPTION`      | `kaioken-app config`                                    | Optional                | Voice transcription model in `<service>/<model>` format: a plugin-registered AI service (`codex` with the codex plugin; audio up to 5MB) or `openai/<model>` with `OPENAI_API_KEY`. Defaults to `codex/gpt-transcribe`.                                                                                                                                                                                             |
| `KAIOKEN_MARKETPLACE_URL`    | `kaioken-app env`, or environment                       | Startup-only testing    | Manifest URL of the reserved `bb-community` plugin marketplace. It defaults to `https://getbb.app/marketplace/v2/marketplace.json`. If the default v2 request returns 404, the server requests v1. Set another URL to test catalog refreshes. The server requests that URL without fallback. It changes only `bb-community`. Add other marketplaces with `kaioken marketplace add`. Restart the app after a change. |
| `KAIOKEN_SERVER_URL`         | `kaioken-app config`                                    | Remote CLI/host use     | Server URL for standalone `kaioken` CLI and `host-daemon` commands on the current machine. The CLI defaults to `http://127.0.0.1:38886` when unset.                                                                                                                                                                                                                                                                 |
| `KAIOKEN_SERVER_BIND_HOST`   | `kaioken-app env`, environment, or `--server-bind-host` | Startup-only            | Server listener host. Defaults to `127.0.0.1`; accepts only `127.0.0.1` or `0.0.0.0`. A full launcher or desktop app restart is required; until then, a previous `0.0.0.0` listener remains exposed. This is not a `kaioken-app config` key.                                                                                                                                                                        |
| `KAIOKEN_SERVER_PORT`        | `kaioken-app env`, environment, or `--server-port`      | Startup-only            | HTTP listener port. Defaults to `38886`. A full launcher or desktop app restart is required after a persistent set or unset.                                                                                                                                                                                                                                                                                        |
| `KAIOKEN_HOST_DAEMON_PORT`   | `kaioken-app env`, environment, or `--host-daemon-port` | Startup-only            | Local host-daemon API port. Defaults to `38887`. A full launcher or desktop app restart is required after a persistent set or unset.                                                                                                                                                                                                                                                                                |
| `KAIOKEN_LOG_LEVEL`          | `kaioken-app config`                                    | Startup-only debugging  | Log level: `trace`, `debug`, `info`, `warn`, `error`, or `fatal`. A full launcher or desktop app restart is required.                                                                                                                                                                                                                                                                                               |
| `OPENAI_API_KEY`             | `kaioken-app env`                                       | OpenAI opt-in routes    | Required only when selecting explicit OpenAI provider routes such as `openai/gpt-4o-mini` or `openai/gpt-transcribe`.                                                                                                                                                                                                                                                                                               |
...
networks Cloudflare challenges that request; kaioken retries, then reports
...
API key instead: `codex login --with-api-key`, or set `KAIOKEN_TRANSCRIPTION` to
...
`bb.voiceInput.audioInputDeviceId`; it does not change `kaioken-app config` or the
...
`kaioken plugin config push-notifications set expoPushUrl <url>`. The plugin reads
...
`kaioken plugin config push-notifications set webEnabled false` (or the other
channel key). Web and desktop clients receive system notifications while a kaioken
...
test controls. `kaioken push-notifications test <web|desktop>` broadcasts a test to
...
idle sleep while kaioken is running. It only blocks idle sleep: closing a laptop lid
or choosing Sleep manually still sleeps the Mac. Configure it from an agent or
terminal with:

```sh
kaioken keep-awake status [--json]
kaioken keep-awake enable [--json]
kaioken keep-awake disable [--json]
kaioken keep-awake hosts all
kaioken keep-awake hosts <host-id>...
```
...
```sh
kaioken concurrency-limit status [--json]
...
environment resolution and raw provider events that kaioken does not yet understand.
...
Set it with `kaioken settings general showDiagnosticEvents <true|false>` or
...
it with
`kaioken settings general steerActiveThreadOnEnter <true|false>`, where `true` is
"Steer".

The "Streamer mode" toggle in Settings → General hides every `customModels`
entry from `~/.kaioken/config.json` in all model lists: the web and mobile pickers,
`kaioken provider models`, and `sdk.providers.models`. Turn it on before a screen
...
`kaioken settings general streamerMode <true|false>`.

The "New branch prefix" field in Settings → General sets the text kaioken
puts in front of every branch name it creates for a managed worktree or a new
checkout branch. It defaults to `kaioken/`, which produces
`kaioken/fix-login-flow-thr_ab12cd34ef`. Change it to `sawyer/` to group your branches
under your own namespace, or clear the field to create
`fix-login-flow-thr_ab12cd34ef` with no prefix. kaioken rejects a prefix that cannot
start a valid git branch name, such as one with a space or a leading `-`, and
the prefix is at most 64 characters. The prefix applies to branches kaioken creates
after you change it; it does not rename an existing branch or worktree. Set it
with `kaioken settings general managedBranchPrefix <prefix>`.
...
`kaioken plugin config provider-claude-code set chromeEnabled true`. kaioken then starts
...
restarts. Resetting a shortcut removes its override so future kaioken releases can
...
to on. Set it with
`kaioken settings keyboard hints <true|false>`. Turning it off hides the
...
`KAIOKEN_SERVER_URL` does not change where full `npx kaioken-app` startup binds locally.
It is for commands that need to target an already-running server, such as the
bundled `kaioken` CLI or a standalone host daemon. The CLI can omit it when targeting
the default local packaged server at `http://127.0.0.1:38886`; set it for remote
or non-default servers.

## Client SSH Targets

`~/.kaioken/client.json` is local to the machine showing the UI. The CLI resolves the
...
When a remote kaioken page asks the local helper to open a work-host path, the helper
uses this mapping to launch remote-capable editors and terminals over SSH.
Browsers or devices without a helper can still use kaioken; local editor actions are
simply unavailable.

## Custom ACP Agents

Known ACP agents appear when their CLI is installed on the host. kaioken exposes
...
```bash
kaioken plugin config provider-acp set customAgents '[
  {"id": "amp", "displayName": "Amp", "command": "amp", "args": ["acp"]}
]'
```

Each entry needs `id` (lowercase letters, digits and dashes), `displayName`,
and `command`. kaioken derives the provider id `acp-<id>`; it never changes once a
thread has used it. An id kaioken always lists (`cursor`) is reserved; an id kaioken
...
its own, and kaioken still lists the roots that agent's host config names (its
...
(only if the agent accepts an explicit compaction request — kaioken hides
`/compact` otherwise), and `dialect` (the vendor side channels kaioken reads for
...
in `~/.kaioken/config.json`. kaioken still **reads** that array so an existing agent keeps
...
so a configured agent shows the generic tool glyph, and kaioken drops the field when
...
`~/.kaioken/config.json`. Use this for a model the provider accepts but does not
list, such as a non-public preview id. This list has no set/unset CLI surface:
edit the JSON, then run `npx kaioken-app config refresh` or restart kaioken. `kaioken-app config list` prints the entries.
...
optional; kaioken derives the label from the model id when it is omitted. kaioken skips
an invalid entry with a warning and keeps the rest of the config.

Each entry appears in `kaioken provider models <providerId>` and in the model
...
(`kaioken settings general streamerMode true`) hides every entry from these lists
...
kaioken discovers it automatically.

An OpenCode "agent" (build, plan, or a custom primary agent) is a session
mode, not a model, so it does not belong in `customModels`. kaioken does not select
OpenCode agents; set the default agent in the OpenCode config instead.

## Agent Instructions

kaioken can inject user-level and workspace-level agent instructions into every
provider-backed thread's system prompt, alongside the skills convention.

For user-level defaults across projects, create `AGENTS.md` in the kaioken data dir:

```
<dataDir>/AGENTS.md
```

For repo-specific guidance, create `.kaioken/AGENTS.md` at the workspace root:

```
<workspace>/.kaioken/AGENTS.md
```
...
`<workspace>/.kaioken/AGENTS.md` second. An empty or whitespace-only file is treated
as absent.

No agent loads `.kaioken/AGENTS.md` natively, and provider-native instruction files
(`CLAUDE.md` for Claude Code, a repo-root `AGENTS.md` for Codex) remain
provider-specific. kaioken reads the files above itself and injects them, so use them
for guidance you want every kaioken thread to receive regardless of provider.

## Skills

User-level kaioken skills live under `<dataDir>/skills/<name>/SKILL.md`; for the
packaged app this is usually `~/.kaioken/skills`. Project skills live under
`<workspace>/.kaioken/skills/<name>/SKILL.md` and override same-named user or plugin
...
Project and user skills override plugin skills by name. Kaioken guide owns the
four bundled core skills and can disable them together or individually.

kaioken indexes each provider's native skill roots for that provider's `/` command
...
(a moved config directory, installed vendor plugins, config-file entries); kaioken
itself knows no agent's layout. The Skills page and `kaioken skill list` show
...
Hermes can organize skills in category directories. kaioken scans those roots
...
source. The CLI equivalents are `kaioken machine list`, `kaioken project create
--machine <id-or-name> ...`, `kaioken project source add --machine <id-or-name>
...`, and `kaioken thread spawn --machine <id-or-name> ...`.

Multi-machine execution is independent of browser access. Tailscale and kaioken
connect let another browser reach the kaioken server; multi-machine support lets
that server dispatch work to non-primary host daemons. The Settings → Machines
installer can use a paired kaioken connect account to route the daemon and its CLI
...
absent from the SDK and the `kaioken` CLI,
and machine credentials are rejected at both the kaioken connect gate and the
...
`/install/kaioken-app.tgz` serves its exact host-only package with a SHA-256 digest
and strong ETag. That package contains the daemon, its workers and native
dependencies, and the bundled `kaioken` CLI; it omits the server and web app. The
...
the machine's kaioken data directory rather than npm's system-wide prefix, so
...
and `kaioken machine retry-update <id-or-name>` can bypass the current backoff after
a transient failure.
...
| `compose.backdrop`                | `off`, `drift`, `rain`, `life`, `waves`, `matrix`, `stars`, `flow`, or `static` |
...
`null`, for example `kaioken settings ui set sidebar.organizationMode machine` or
`kaioken settings ui set sidebar.sectionOrder '["threads","pinned","projects"]'`.
...
`kaioken thread open <thread-id> --split right|down|left|top|replace` and the matching
SDK request, plus pane presentation controls through
`kaioken thread pane maximize|restore|toggle|spotlight|clear-spotlight [thread-id]` and
...
```sh
kaioken plugin enable account-pool
...
kaioken pool account add --provider claude --import
kaioken pool account add --provider codex --import
printf '%s\n' "$ANTHROPIC_API_KEY" | kaioken pool account add --provider claude --api-key-stdin [--label <text>] [--priority <n>]
...
machine from the kaioken server, and the code stays out of process arguments. The
...
The CLI import paths read the Claude Code or Codex login on the kaioken server host.
...
`CODEX_POOL_AUTH_TOKEN`; kaioken applies both when launching `codex app-server`
...
by the CLI. Plugin startup and `kaioken pool status` remove token files for machines
...
`kaioken pool token rotate --machine <id-or-name>`; the prior token remains valid
for ten minutes so in-flight requests can drain. Bypass or restore routing for
one thread with `kaioken pool bypass <thread-id>` or
`kaioken pool bypass <thread-id> --off`. Account listing, enable, disable, removal,
priority changes, and usage refreshes are available through
`kaioken pool account list|enable|disable|remove|priority|refresh`.
Provider routing is independently persisted and defaults on. Use
`kaioken pool routing <claude|codex> --off` to stop contributing pool environment
...
enabled and every five minutes while idle. Use `kaioken pool account refresh <id>`
...
`kaioken pool account reorder <claude|codex> <id>...`, to set the complete order for
one provider. Include disabled accounts too. Reordering changes the next failover
sequence without moving the current account. `kaioken pool account priority <id> <n>`
...
```sh
kaioken pool config
kaioken pool config set switchThreshold 0.98
kaioken pool config set anthropicUpstreamBaseUrl http://127.0.0.1:9000
kaioken pool config set codexUpstreamBaseUrl http://127.0.0.1:9001
```
...
**Manually** is unchanged. Toggle it with `kaioken settings experiment
...
`kaioken settings experiment timelineWindowing <true|false>`.

## Thread Timeline Window

Timeline pages select conversation groups using user-message anchors. The
`KAIOKEN_FF_TIMELINE_WINDOW_EVENT_BUDGET` setting (default 1500) guides the number
...
Older activity loads as the user scrolls. `kaioken thread log --all` walks one
...
loop stalled`. Both log at `info`, so they are visible in `~/.kaioken/logs/` without
raising `KAIOKEN_LOG_LEVEL`.

## Plugins

Plugins are on by default. Builtin plugins, including connect, ship with kaioken;
user-installed plugins come from `kaioken plugin install` or the bundled official
store.
...
<dataDir>/plugins/<id>/logs/       bb.log output (plugin.log, JSONL, rotated
                                   at 5MB; read with `kaioken plugin logs <id>`)
...
Kaioken's official plugins (GitHub, Docs, Memory, and Tasks) ship bundled
inside the app and install from the local bundled copy — no network, no remote catalog.
Discover them with `kaioken plugin search` or Plugins → Browse plugins; users
cannot add, remove, or configure the bundled official plugin set. Installed official
plugins are pinned to the bundled copy and update with Kaioken app releases. Local
path installs remain available directly through `kaioken plugin install ./path` or
`path:...`, and direct `npm:`/`git:` installs stay supported.

Marketplace catalogs and their validated icon bytes live in the kaioken database,
not on disk. `kaioken marketplace add|list|refresh|remove` and Settings → Plugin
marketplaces manage them; the reserved `bb-community` marketplace comes from
`KAIOKEN_MARKETPLACE_URL` and cannot be added or removed. Adding a marketplace
...
`package.json` and `kaioken` manifest; an optional `.kaioken/plugins.json` collection
...
`kaioken plugin install git:<url>[@<ref|semver-range>] --plugin <name>` (resolves a collection
...
`kaioken plugin outdated`, `update`, rollback, and `remove` act per plugin. A
...
Bundled builtin and official plugins update with Kaioken app releases. For direct
`git:`/`npm:` installs, update application is manual: `kaioken plugin outdated` or
the "Check for updates" key on the Plugins page checks tracking sources, and
`kaioken plugin update <id>` / `kaioken plugin update --all` or the "Update x.y.z" pill
...
refused — use `kaioken plugin update`. Before activation kaioken snapshots the plugin
...
`kaioken plugin disable provider-retry`.
...
```bash
kaioken plugin config provider-retry set maximumWait "24 hours"
```
...
`kaioken provider-retry status`, cancel one on that card or with
`kaioken provider-retry cancel <thread-id>`, or run
`kaioken provider-retry retry <thread-id>` to send it now instead of waiting. Limits
...
under Settings → Installed plugins or with `kaioken plugin enable workflows`. Its six
settings are bounded integers, edited with numeric inputs under Plugins →
Installed plugins or with `kaioken plugin config workflows set <key> <value>`:
...
`kaioken plugin install npm:<package>[@<version|tag|range>]` requires `npm` on PATH
(packages are installed with `--ignore-scripts`). Git plugins also use npm with
lifecycle scripts disabled, so they may depend on third-party packages; kaioken
...
inline. A committed `dist/` is always replaced by the bundles kaioken builds.
...
tracks the repository's `[<tag-prefix>]vX.Y.Z` release tags: kaioken installs the
...
plugins use `builtin:<name>` and ship with kaioken unless removed. Managed
(`git:`/`npm:`) installs
refuse plugins whose optional `engines.bb` or `engines.bbPluginSdk` ranges
do not match the running kaioken/SDK, or whose `dist/*.meta.json` plugin identity
...
`engines.bbPluginSdk` is a floor, not a ceiling. kaioken reads the lowest version
...
`0.4.3` or a later `0.x`. Only a plugin that asks for a newer SDK than this kaioken
...
The same tracking intent drives updates: `kaioken plugin outdated` checks for
compatible candidates (and reports blocked incompatible newer releases);
`kaioken plugin update <id>` / `kaioken plugin update --all` applies them. Pinned source
intent is never widened by update; remove and reinstall to choose a different
source intent. Dev builds (kaioken `0.0.0`) do not enforce `engines.bb` and annotate
that on check results.
Update confirmation matches install (full-trust code; `--yes` skips; non-TTY
refuses without it). Plugins are full-trust code running inside the kaioken server
process: they can read all local kaioken data, including other plugins' secrets.
...
```bash
npx kaioken-app --data-dir ~/.kaioken-test --server-port 48886 --host-daemon-port 48887
```

The server listens on `127.0.0.1` by default. Set
`--server-bind-host 0.0.0.0` (or `KAIOKEN_SERVER_BIND_HOST=0.0.0.0`) only when a
...
With wildcard binding they show `http://0.0.0.0:<port>`, while kaioken's health check
...
kaioken does not currently offer an IPv6 wildcard bind option.

The data directory is the root directory for all kaioken-managed state: the SQLite
...
and
plugins. It defaults to `~/.kaioken/` for the packaged app. The `pnpm dev` source launcher derives an isolated data
directory under `~/.kaioken-dev/<checkout-instance>/` from the checkout path. The
...
```bash
npx kaioken-app --server-port 48886 --host-daemon-port 48887
```
...
`~/.kaioken-machines/host-daemon-ports/` cover both default and custom
`KAIOKEN_DATA_DIR` locations. Its generated command accepts `--host-daemon-port
...
selectors (`KAIOKEN_DATA_DIR`, server URL/port, host-daemon local API port, and Vite
port) with deterministic values derived from the checkout path. The SQLite
database path is always derived from `KAIOKEN_DATA_DIR`. Both the main server and
Vite app bind to loopback by default; an explicit `KAIOKEN_DEV_APP_HOST` still
overrides the Vite listener. Remote HTTP dev via `KAIOKEN_DEV_APP_HOST` also requires
`KAIOKEN_SERVER_BIND_HOST=0.0.0.0` for realtime updates; the Tailscale Serve HTTPS
...
`packages/kaioken-app/dist/kaioken-app.js host-daemon` entrypoint. Source-only scripts do
not own production ports or data-dir defaults.

Source checkout commands such as `pnpm kaioken`, `pnpm kaioken:dev`, and `pnpm reset`
are thin wrappers around `@kaioken/scripts`. Those wrappers force `NODE_ENV` to the
intended mode so ambient shell state does not silently retarget kaioken.

Use `pnpm reset` or `pnpm reset:dev` to clear a data directory. These only
remove kaioken-managed state, not provider credentials.

`KAIOKEN_PROVIDER_BRIDGE_RECORD_DIR=<dir>` in the host daemon's environment turns
...
Agents use `kaioken browser-automation` through its bundled skill. Screenshot results
contain temporary JPEG paths and the browser host ID; remote captures can be
fetched with `kaioken file read <path> --host <host-id> --json`. Read or copy images
...
Kaioken guide is installed and enabled by default. In Settings → Installed plugins
→ Kaioken guide, `introduction` controls the Kaioken introduction, `skills` controls all
four bundled skills, and `kaiokenCli`, `pluginAuthoring`, `skillCreator`, and `submitPlugin` control
individual skills. All default to true. Disabling Kaioken guide removes its
...
Use `kaioken plugin config <id> set <key> true|false` or the SDK's
