...
| Package or app                                                      | Role                                                                                                |
| ------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| [`packages/kaioken-app`](../packages/kaioken-app)                             | Published npm package, `npx kaioken-app@latest` launcher, bundled `kaioken` CLI entry, and public SDK export. |
| [`apps/desktop`](../apps/desktop)                                   | macOS/Linux Electron shell that supervises the packaged runtime and loads the kaioken web UI.            |
| [`apps/app`](../apps/app)                                           | Web UI for inspecting projects, threads, environments, and running work.                            |
| [`apps/mobile`](../apps/mobile)                                     | Native phone client (Expo; iOS first, Android next) for a kaioken server over Direct URLs or kaioken connect. |
...
| [`apps/cli`](../apps/cli)                                           | Scriptable `kaioken` CLI for users and agents.                                                           |
| [`apps/web`](../apps/web)                                           | getbb.app site: marketing page + kaioken connect auth/dashboard (TanStack Start on Cloudflare Workers).  |
...
`kaioken-app` also exposes a Node scripting SDK:
`import { BBSdk } from "kaioken-app"`. See
[`packages/kaioken-app`](../packages/kaioken-app/README.md#scripting-with-the-sdk).
...
| Pi packages (`0.84.0`)         | Pi bridge and `kaioken-app` | Pi extensions import the host's Pi modules. The packaged bridge keeps this exact package tree on disk so extensions share one compatible runtime. Bump the Pi packages together. |
