...
- kaioken version (`kaioken --version` or Settings → About) and how it was installed (desktop app, `npx kaioken-app`, `install.sh`, from source at commit).
...
- Anything unusual about the setup: remote machine, kaioken Connect, worktree environment, custom ports or data dir, cgroup/memory limits.

### 3. Steps to reproduce

Numbered, copy-pasteable, minimal. Exact commands, exact UI clicks, exact prompt text. Include a tiny scratch repo or fixture if the bug needs one. Prefer `kaioken …` CLI steps over UI descriptions when both work; they are unambiguous.
...
Paste the actual output verbatim (terminal output, log lines, HTTP responses, `kaioken thread log` excerpts). For visual bugs attach a screenshot or short recording and say what to look at. Then state what you expected instead.

### 5. Evidence

- Logs from `~/.kaioken/logs/` (or the dev data dir printed by `pnpm dev`), trimmed to the relevant window.
...
- Link the kaioken thread or the investigation report that produced the finding.
