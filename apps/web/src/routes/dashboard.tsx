...
import { MAX_PER_ACCOUNT } from "@kaioken/connect-db";
import type { HandleValidationError, LabelAvailability } from "@kaioken/connect-db";
  ...
  return (
    <div className="mb-[18px] flex items-center gap-2.5">
      ...
      <span
        role="img"
        aria-label="kaioken"
        className="kaioken-mark h-[30px] w-[30px] rounded-lg"
      />
      <div className="leading-tight">
        <b className="block text-sm font-semibold">kaioken connect</b>
        <span className="text-xs text-muted-foreground">
          Your kaioken, reachable anywhere
        </span>
      </div>
    </div>
  );
    ...
    <Shell>
      <WebCard>
        ...
        <p className="mt-1 mb-4 text-sm text-muted-foreground">
          Give your kaioken a private URL and open it from any browser. Your code and
...
function ClaimField({
  ...
  initial = "",
  autoFocus,
  previewLead = "Your kaioken will live at",
  buildSubmitLabel,
  onClaim,
  onCancel,
  cancelLabel = "Cancel",
  ...
  return (
    <Shell>
      <WebCard>
        ...
        <p className="mt-1 mb-4 text-sm text-muted-foreground">
          This becomes your kaioken&rsquo;s permanent URL. Lowercase letters,
          numbers, and dashes.
        </p>
        <ClaimField
          layout="card"
...
function SetupCodePanel({
  ...
  const cli = code
    ? `npx -p kaioken-app@latest kaioken connect --code ${code.code} --server ${code.serverUrl}`
    : "";
...
function RepairCodeBlock({ serverId }: { serverId: string }) {
  ...
  return (
    <div>
      <BigCode code={code?.code ?? "····–····"} disabled={!code} />
      <p className="mt-2.5 text-xs text-subtle-foreground">
        Re-pairing replaces this kaioken&rsquo;s credential. Paste in{" "}
        <span className="font-medium text-foreground">Plugins → connect</span>
  ...
  return (
    <Overlay onClose={onCancel}>
      <h4 className="mb-1.5 text-[15px] font-semibold">
        {removing ? "Remove this address?" : "Disconnect your kaioken?"}
      </h4>
      <p className="mb-4 text-sm text-muted-foreground">
        ...
        {removing
          ...
          : "stops working on all devices immediately. Your kaioken keeps running locally; re-pairing needs a new connect code."}
      </p>
  ...
  if (atCap && !server) {
    return (
      <Overlay onClose={onClose}>
        <h4 className="mb-1.5 text-[15px] font-semibold">Connect another kaioken</h4>
        <p className="mb-4 text-sm text-muted-foreground">
  ...
  return (
    <Overlay onClose={onClose}>
      {!server ? (
        <>
          <h4 className="mb-1.5 text-[15px] font-semibold">
            Connect another kaioken
          </h4>
          <p className="mb-3 text-sm text-muted-foreground">
            Pick its address — every kaioken gets its own URL.
          </p>
          <ClaimField
            layout="dialog"
            autoFocus
            ...
            previewLead="This kaioken will live at"
        ...
        <>
          <h4 className="mb-1.5 text-[15px] font-semibold">Pair the new kaioken</h4>
      ...
      <div className="mt-3 rounded-xl border border-border bg-card p-2 shadow-sm">
        ...
        {state.machines.length === 0 ? (
          <p className="px-3 pb-2 text-xs text-subtle-foreground">
            Add machines from kaioken Settings → Machines.
          </p>
        ) : (
