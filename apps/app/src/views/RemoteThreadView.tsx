...
function RemoteComposer({
  ...
  const submitWith = useCallback(
    ...
    [draft, attachments.isAttaching, message, send, server.live, threadId],
  );
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 flex-1 truncate">
        ...
        Replies will work when it reconnects.
      </span>
    </div>
  );
}
  ...
  return (
    <RemoteServerProvider server={server}>
      <PageShell
        ...
        scrollAnchorThreadId={`${server.handle}:${threadId}`}
        footer={
          server.live &&
          thread.data !== undefined &&
          !thread.isError &&
          !pendingInteractions.isError ? (
            <RemoteComposer
              key={`${server.handle}:${threadId}`}
              server={server}
              threadId={threadId}
            ...
            >
              {thread.isError || pendingInteractions.isError
                ? `Could not load this task from ${server.name}. Reconnect before replying.`
                : "Loading task…"}
            </p>
          ) : (
      ...
      >
        ...
        {server.live ? (
          <ThreadTimelineSurface
            projectId={thread.data?.projectId}
            activeThinking={timeline.data?.activeThinking ?? null}
            hasOlderTimelineRows={timeline.hasNextPage}
            isLoadingOlderTimelineRows={timeline.isFetchingNextPage}
            onLoadOlderRows={async () => {
              await timeline.fetchNextPage();
            }}
