...
import { permissionModeRank } from "@kaioken/domain";
import {
...
import { formatOfflineMeta, useNow } from "@/components/sidebar/TimelineThreadList";
...
function RemoteComposer({ server, project }: RemoteComposerProps) {
  ...
  const permissionMode = resolvePermissionModeSelection({
    ...
    permissionModes:
      allowedPermissionModes.length > 0 ? allowedPermissionModes : permissionModes,
  });
  ...
  const execution = useMemo<ExecutionControlsProps>(
    () => ({
      ...
      model: {
        active: catalog.activeModel ? { model: catalog.activeModel.model } : null,
  ...
  const canSubmit =
    hostId !== null && providerId.length > 0 && model.length > 0 && !create.isPending;
  const submit = useCallback(() => {
    const text = message.trim();
    if (text.length === 0 || !canSubmit || hostId === null) return;
    create.mutate(
      {
  ...
  const composer = useMemo<FollowUpComposerProps>(
    ...
    [create.isPending, mentionRanges, message, project.name, server.name, submit, submitMode],
  );
...
export function RemoteComposeView() {
  ...
  return (
    <PageShell footer={<RemoteComposer server={server} project={project} />}>
      {header}
      <p className="px-2 text-sm text-muted-foreground">
        The thread is created and runs on {server.name}. You can follow and reply
        to it from here.
      </p>
    </PageShell>
  );
