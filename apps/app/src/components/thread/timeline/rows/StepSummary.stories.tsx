...
const commandTurboBuild: TimelineRow = commandRow({
  ...
  command:
    "pnpm exec turbo run build --filter=@kaioken/domain --filter=@kaioken/core-ui --filter=@kaioken/server-contract --concurrency=1",
  cwd: null,
  source: null,
  output: "",
  exitCode: 0,
...
const commandTurboTestServer: TimelineRow = commandRow({
  ...
  command:
    "pnpm exec turbo run test --filter=@kaioken/server --only --concurrency=1 -- --run test/threads/timeline-service.test.ts",
  cwd: null,
  source: null,
  output: "",
  exitCode: 0,
...
const commandTurboTestCoreUi: TimelineRow = commandRow({
  ...
  command:
    "pnpm exec turbo run test --filter=@kaioken/core-ui --concurrency=1 -- --run test/to-view-messages.assistant-streams.test.ts",
  cwd: null,
  source: null,
  output: "",
  exitCode: 0,
...
const commandTurboBuildForce: TimelineRow = commandRow({
  ...
  command:
    "pnpm exec turbo run build --filter=@kaioken/domain --filter=@kaioken/core-ui --force --concurrency=1",
  cwd: null,
  source: null,
  output: "",
  exitCode: 0,
