...
const UPDATES_RESULTS = [
  ...
  },
  {
    ...
    blocked: { version: "1.9.0", reasons: ["requires kaioken >= 0.15"] },
  },
];
...
describe("checkPluginUpdates", () => {
  it("returns the checked entries across outcome kinds", async () => {
    ...
    expect(entries[2]?.blocked?.reasons).toEqual(["requires kaioken >= 0.15"]);
  });
        ...
        results: [
          {
            ...
            installed: false,
            compatible: false,
            incompatibleReason: "requires kaioken >= 0.15",
          },
        ],
        collections: [
          {
