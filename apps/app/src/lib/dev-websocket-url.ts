...
function resolveDevWebSocketBaseUrl(): string | undefined {
  if (
    typeof __KAIOKEN_DEV_WS_BROWSER_HOST_PORT__ === "number" &&
    typeof __KAIOKEN_DEV_APP_BROWSER_HOST_PORT__ === "number"
  ) {
    return resolveBrowserHostDevWebSocketBaseUrl(
      __KAIOKEN_DEV_WS_BROWSER_HOST_PORT__,
      __KAIOKEN_DEV_APP_BROWSER_HOST_PORT__,
    );
  }
