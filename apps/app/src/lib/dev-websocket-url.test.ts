...
describe("buildDevWebSocketUrl", () => {
  ...
  it("connects directly to the backend for HTTP source dev", () => {
    vi.stubGlobal("__KAIOKEN_DEV_WS_BROWSER_HOST_PORT__", 23_802);
    vi.stubGlobal("__KAIOKEN_DEV_APP_BROWSER_HOST_PORT__", 15_802);
  ...
  it("uses the proxied app origin for HTTPS kaioken connect shares", () => {
    vi.stubGlobal("__KAIOKEN_DEV_WS_BROWSER_HOST_PORT__", 23_802);
    vi.stubGlobal("__KAIOKEN_DEV_APP_BROWSER_HOST_PORT__", 15_802);
