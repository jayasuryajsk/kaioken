...
describe("kaioken guide command output", () => {
  setupCommandOutputTestEnvironment();

  it("kaioken guide unknown chapter lists available chapters", async () => {
    await expect(
  ...
  it("kaioken guide terminals documents explicit scopes and ID-only mutations", async () => {
    ...
    expect(output).toContain("kaioken terminal list --thread <thread-id>");
    expect(output).toContain("kaioken terminal rename <terminal-id> <title>");
  });
});
