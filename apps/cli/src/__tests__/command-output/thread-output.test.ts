...
describe("kaioken thread output command output", () => {
  setupCommandOutputTestEnvironment();

  const register: CommandRegistrar = (program) =>
    registerThreadCommands(program, () => "http://server");

  it("kaioken thread output requires a thread id or --self", async () => {
    vi.stubEnv("KAIOKEN_THREAD_ID", "thread-output-context");
  ...
  it("kaioken thread output --self resolves from KAIOKEN_THREAD_ID", async () => {
    vi.stubEnv("KAIOKEN_THREAD_ID", "thread-output-context");
    const getOutput = vi.fn(async () => ({ output: "FINAL" }));
    stubServerApi({ "v1.threads.:id.output.$get": getOutput });
    ...
    expect(collectLogLines(vi.mocked(console.error))).not.toContain(
      "Thread thread-output-context (from KAIOKEN_THREAD_ID)",
    );
    expect(collectLogLines(vi.mocked(console.log))).toContain("FINAL");
  });

  it("kaioken thread output --json prints the raw output payload", async () => {
    const getOutput = vi.fn(async () => ({ output: "FINAL" }));
    stubServerApi({ "v1.threads.:id.output.$get": getOutput });
