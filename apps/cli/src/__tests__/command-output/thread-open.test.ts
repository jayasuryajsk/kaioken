...
describe("kaioken thread open command output", () => {
  setupCommandOutputTestEnvironment();

  const register: CommandRegistrar = (program) =>
    registerThreadCommands(program, () => "http://server");

  it("uses KAIOKEN_THREAD_ID and opens a thread-relative workspace path", async () => {
  ...
  it("uses an explicit thread id when KAIOKEN_THREAD_ID is not set", async () => {
    const { openThread } = stubThreadOpenApi({
  ...
  it("resolves an absolute workspace path through the target thread environment", async () => {
    vi.stubEnv("KAIOKEN_THREAD_ID", "thread-workspace");
  ...
  it("opens an absolute thread-storage path for the current thread", async () => {
    vi.stubEnv("KAIOKEN_THREAD_ID", "thread-storage");
    vi.stubEnv("KAIOKEN_THREAD_STORAGE", "/tmp/kaioken-thread-storage/thread-storage");
    const { getEnvironment, getThread, openThread } = stubThreadOpenApi({
      open: async () => ({ delivered: 1 }),
    });
    ...
    await runCommand(
      [
        "thread",
        "open",
        "/tmp/kaioken-thread-storage/thread-storage/reports/preview.html",
      ],
      register,
    );
  ...
  it("requires an explicit thread id outside a Kaioken thread", async () => {
    stubThreadOpenApi({});
  ...
  it("rejects a different explicit thread id when KAIOKEN_THREAD_ID is set", async () => {
    vi.stubEnv("KAIOKEN_THREAD_ID", "thread-current");
    stubThreadOpenApi({});
  ...
  it("documents the current-thread or explicit-thread command shape", async () => {
    ...
    expect(help).toContain(
      "Open a Kaioken thread, optionally with a file in its panel",
    );
  ...
  it("treats an explicit --split target as a thread id inside a Kaioken thread", async () => {
    vi.stubEnv("KAIOKEN_THREAD_ID", "thread-current");
    const { openThread } = stubThreadOpenApi({});
  ...
  it("opens a file for an explicit split target inside a Kaioken thread", async () => {
    vi.stubEnv("KAIOKEN_THREAD_ID", "thread-current");
    const { openThread } = stubThreadOpenApi({});
