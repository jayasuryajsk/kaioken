...
if (args[0] === "view") {
  const mode = process.env.KAIOKEN_TEST_NPM_VIEW ?? "published";
  if (mode === "e404") {
    process.stderr.write("npm error code E404\\nnpm error 404 Not Found - GET https://registry.npmjs.org/@get-kaioken%2fplugin-sdk\\n");
    process.exit(1);
  }
  if (mode === "error") {
...
// KAIOKEN_TEST_NPM_INSTALL=fail stands in for the installs that die before they
...
if (process.env.KAIOKEN_TEST_NPM_INSTALL === "fail") {
  process.stderr.write("npm error code EPERM\\nnpm error syscall open\\nnpm error Your cache folder contains root-owned files\\n");
  process.exit(1);
}

if (process.env.KAIOKEN_TEST_NPM_INSTALL === "fail-noisy-stdout") {
  process.stderr.write("npm error code EPERM\\nnpm error syscall open\\n");
...
// outranks it. KAIOKEN_TEST_NPM_ALWAYS_OMIT_DEV forces the omission to stand in for
// an install that silently drops packages.
const omitDev =
  process.env.KAIOKEN_TEST_NPM_ALWAYS_OMIT_DEV === "1" ||
...
// a workspace member; KAIOKEN_TEST_NPM_HOIST_TO stands in for that root.
const installRoot = process.env.KAIOKEN_TEST_NPM_HOIST_TO ?? process.cwd();
