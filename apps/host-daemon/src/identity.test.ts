...
describe("identity", () => {
  it("resolves a host ID once persisted and reuses it on subsequent runs", async () => {
    const dataDir = await makeTempDir("kaioken-host-daemon-identity-");
  ...
  it("does not persist KAIOKEN_HOST_ID until persistHostId is called", async () => {
    const dataDir = await makeTempDir("kaioken-host-daemon-identity-env-");
  ...
  it("lets a fresh KAIOKEN_HOST_ID be used after an earlier load failed to persist", async () => {
    const dataDir = await makeTempDir("kaioken-host-daemon-identity-retry-");
  ...
  it("rejects a KAIOKEN_HOST_ID that conflicts with a persisted host ID", async () => {
    const dataDir = await makeTempDir("kaioken-host-daemon-identity-conflict-");
  ...
  it("uses KAIOKEN_HOST_NAME when provided instead of detecting a hostname", async () => {
    const dataDir = await makeTempDir("kaioken-host-daemon-identity-host-name-");
