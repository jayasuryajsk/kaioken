...
describe("daemon lifecycle", () => {
  it("prevents a second instance from acquiring the lock", async () => {
    const dataDir = await makeTempDir("kaioken-host-daemon-lock-");
  ...
  it("reclaims a stale lock left behind by a crashed instance", async () => {
    const dataDir = await makeTempDir("kaioken-host-daemon-stale-lock-");
  ...
  it("releases the lock during clean shutdown", async () => {
    const dataDir = await makeTempDir("kaioken-host-daemon-shutdown-");
