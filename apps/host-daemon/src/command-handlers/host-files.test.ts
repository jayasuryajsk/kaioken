...
async function initRepo(): Promise<string> {
  const repoPath = await makeTempDir("kaioken-host-files-test-");
...
describe("readHostFile (no ref — disk read)", () => {
  ...
  it("marks missing roots as expected", async () => {
    const parentPath = await makeTempDir("kaioken-host-files-missing-root-");
  ...
  it("marks missing relative read roots as expected", async () => {
    const parentPath = await makeTempDir("kaioken-host-files-relative-root-");
...
describe("browseHostDirectory", () => {
  it("lists immediate children sorted directories-first, hiding noise", async () => {
    const root = await makeTempDir("kaioken-browse-");
  ...
  it("rejects a path that is not a directory", async () => {
    const root = await makeTempDir("kaioken-browse-file-");
