...
import { hostDaemonSessions } from "@kaioken/db";
import {
...
import { type HostType, type ThreadEvent } from "@kaioken/domain";
import type {
  HostDaemonCommand,
...
function isRuntimeWorkspaceFileCommand(command: HostDaemonRpcCommand): boolean {
  if (command.type === "host.list_files") {
    return command.path.endsWith(path.join(".kaioken", "skills"));
  }
  if (command.type !== "host.read_file") return false;
  return (
    command.path.endsWith(path.join(".kaioken", "AGENTS.md")) ||
    command.path.includes(`${path.sep}.kaioken${path.sep}skills${path.sep}`)
  );
}
