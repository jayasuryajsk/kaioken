...
describe("core machine provider orchestration", () => {
  ...
  it("automatically removes an ephemeral machine after its last thread is archived even when its environment never retires", async () =>
    withTestHarness(async (harness) => {
      const { host } = seedHostSession(harness.deps, {
      ...
      await vi.waitFor(() => expect(machineRemove).toHaveBeenCalledOnce());

      expect(environmentRemove).not.toHaveBeenCalled();
      expect(getEnvironment(harness.db, environment.id)).toMatchObject({
        status: "destroyed",
        teardownStatus: "removed",
      });
  ...
  it("does not automatically remove a persistent machine with a never-retiring environment", async () =>
    withTestHarness(async (harness) => {
      const { host } = seedHostSession(harness.deps, {
        id: "host_persistent_environment",
      });
      const { project } = seedProjectWithSource(harness.deps, {
        hostId: host.id,
        path: "/tmp/persistent-environment",
      });
      const environment = createEnvironment(harness.db, harness.hub, {
        projectId: project.id,
        hostId: host.id,
        ...
        environmentProvider: {
          pluginId: "test-environment-plugin",
          environmentProviderId: "test-environment",
          instanceKey: "persistent-environment",
          selection: {
            machine: { type: "existing", hostId: host.id },
            inputs: null,
          },
  ...
  it("removes a suspended ephemeral machine without resuming it or invoking its environment provider", async () =>
    withTestHarness(async (harness) => {
      const { host } = seedHostSession(harness.deps, {
        id: "host_suspended_ephemeral_remove",
      });
      const { project } = seedProjectWithSource(harness.deps, {
        hostId: host.id,
        path: "/tmp/suspended-ephemeral-remove",
      });
      const environment = createEnvironment(harness.db, harness.hub, {
        projectId: project.id,
        hostId: host.id,
        ...
        environmentProvider: {
          pluginId: "test-environment-plugin",
          environmentProviderId: "test-environment",
          instanceKey: "suspended-ephemeral-remove",
          selection: {
            machine: { type: "existing", hostId: host.id },
            inputs: null,
          },
...
it("retains a persisted removal claim after restart even when live work is restored", async () =>
  withTestHarness(async (harness) => {
    ...
    await sweepProviderMachine(harness.deps, host.id);
    expect(remove).not.toHaveBeenCalled();
    expect(getHost(harness.db, host.id)?.phase).toBe("removing");
    harness.db
      .update(threads)
      .set({ archivedAt: 20_002 })
      .where(eq(threads.id, thread.id))
      .run();
...
describe("coordinated machine suspension", () => {
  ...
  it("retains compute after failed save and retries after retryAt", async () =>
    withTestHarness(async (h) => {
      ...
      expect(getHost(h.db, host.id)).toMatchObject({
        phase: "active",
        statusMessage: expect.stringContaining("snapshot unavailable"),
  ...
  it("retains the machine when its provider refuses suspension", async () =>
    withTestHarness(async (h) => {
      ...
      expect(getHost(h.db, host.id)).toMatchObject({
        phase: "active",
        statusMessage: expect.stringContaining("Restore the pinned account"),
        suspendRetryAt: expect.any(Number),
      });
    }));
});
...
it("concurrent dispatch shares one restore and records an expired image failure", async () =>
  withTestHarness(async (h) => {
    ...
    expect(getHost(h.db, host.id)).toMatchObject({
      statusMessage: expect.stringContaining("Snapshot image no longer exists"),
      suspendRetryAt: expect.any(Number),
    });
    expect(getHost(h.db, host.id)?.phase).toBe("suspended");
  }));
