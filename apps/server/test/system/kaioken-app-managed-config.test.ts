...
} from "../../src/services/system/kaioken-app-managed-config.js";
...
function createRuntimeConfig(): ServerRuntimeConfig {
  return {
    ...
    builtinSkillsRootPath: "/tmp/kaioken-test/builtin-skills",
    customModels: [],
...
describe("kaioken-app managed config", () => {
  it("applies managed config over the ambient runtime config", () => {
    ...
    applyBbAppManagedConfig({
      ...
      managedConfig: {
        config: {
          KAIOKEN_APP_URL: "https://stored-app.example.test",
          KAIOKEN_INFERENCE: "anthropic/claude-sonnet-4-5",
          KAIOKEN_INFERENCE_FALLBACK: "openai/gpt-5.4-mini",
          KAIOKEN_TRANSCRIPTION: "openai/gpt-4o-transcribe",
        },
      },
      managedEnvFile: {
        env: {
  ...
  it("reloads config file changes and notifies clients", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kaioken-managed-config-"));
    ...
    try {
      writeFileSync(
        ...
        `${JSON.stringify({
          config: { KAIOKEN_INFERENCE_FALLBACK: "codex/gpt-5.4-mini" },
        })}\n`,
        "utf8",
      );
      writeFileSync(
  ...
  it("throws on invalid managed config during explicit reload", async () => {
    ...
    try {
      writeFileSync(
        formatBbAppConfigPath(dataDir),
        `${JSON.stringify({ config: { KAIOKEN_INFERENCE: "gpt-4o-mini" } })}\n`,
        "utf8",
      );
