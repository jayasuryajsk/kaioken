...
import {
  ...
  resolveBbAppPackage,
  type KaiokenAppArtifactCommandRunner,
} from "../../src/services/install/kaioken-app-artifact.js";
...
async function writeHostPackage(root: string, readme: string): Promise<void> {
  ...
  await mkdir(join(root, "host-daemon/dist/kaioken-chunks"), { recursive: true });
  await writeFile(join(root, "package.json"), JSON.stringify(packageJson));
  await writeFile(join(root, "dist/kaioken-app.js"), "#!/usr/bin/env node\n");
  await writeFile(
    join(root, "dist/kaioken-host-daemon.js"),
    "#!/usr/bin/env node\n",
  );
  await writeFile(join(root, "dist/kaioken.js"), "#!/usr/bin/env node\n");
  ...
  await writeFile(
    join(root, "host-daemon/dist/kaioken-chunks/chunk-TEST.js"),
    "export {};\n",
  );
}

async function fixture(mode: Mode) {
  const root = await mkdtemp(join(tmpdir(), `kaioken-artifact-${mode}-`));
  roots.push(root);
  const packageRoot = isRepoMode(mode) ? join(root, "packages/kaioken-app") : root;
  const serverEntry =
    mode === "repo-src"
...
describe("kaioken-app artifact service (desktop packaging)", () => {
  it(
    ...
    async () => {
      ...
      expect(listing).toEqual(
        expect.arrayContaining([
          ...
          "package/dist/kaioken-app.js",
          "package/dist/kaioken-host-daemon.js",
          "package/dist/kaioken.js",
          "package/host-daemon/dist/kaioken",
          "package/host-daemon/dist/kaioken-chunks/chunk-TEST.js",
          "package/host-daemon/dist/kaioken-parcel-watcher-child.mjs",
          "package/host-daemon/dist/kaioken-plugin-host-worker.mjs",
          "package/host-daemon/dist/kaioken-provider-bridge-worker.mjs",
...
describe.each(MODES)("kaioken-app artifact service (%s)", (mode) => {
  it(
    ...
    async () => {
      ...
      const runner: KaiokenAppArtifactCommandRunner = async (command, args, cwd) => {
      ...
      expect(listing).toContain("package/dist/kaioken-app.js");
      expect(listing).toContain("package/dist/kaioken.js");
      expect(listing).toContain("package/host-daemon/dist/daemon-bundle.mjs");
      expect(listing).toContain("package/host-daemon/dist/kaioken");
      expect(listing).not.toContain("package/private.txt");
