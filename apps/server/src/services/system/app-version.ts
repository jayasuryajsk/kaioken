...
const NPM_LATEST_URL = "https://registry.npmjs.org/kaioken-app/latest";
const NPM_LATEST_TIMEOUT_MS = 5_000;
const NPM_LATEST_CACHE_TTL_MS = 60 * 60 * 1000;
const UPGRADE_COMMAND = "npx kaioken-app@latest";
    ...
    try {
      ...
      if (!response.ok) {
        logger.warn(
          ...
          "Failed to fetch latest kaioken-app version from npm",
        );
        return null;
      }
