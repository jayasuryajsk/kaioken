...
function installConflict(skillId: string): ApiError {
  return new ApiError(
    ...
    `Skill "${skillId}" already exists with different or unsafe contents; kaioken left it unchanged`,
  );
}
...
export async function installServerRegistrySkill(args: {
  ...
  const extractionRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "kaioken-registry-skill-"),
  );
  ...
  try {
    ...
    if (
      ...
      throw new ApiError(
        ...
        `Skill "${args.skillId}" is already installed in kaioken`,
      );
    }
