...
const processes: KaiokenAppProcess[] = [];
...
async function createTempScript(
  ...
  const root = await mkdtemp(join(tmpdir(), "kaioken-desktop-process-"));
...
describe("kaioken app process", () => {
  ...
  it("uses Electron node mode for packaged runtimes", () => {
    const runtime = resolveBbAppProcessRuntime({
      ...
      isPackaged: true,
      platform: "darwin",
      processExecPath: "/Applications/bb.app/Contents/MacOS/kaioken",
    });

    expect(runtime).toEqual({
      executablePath: "/Applications/bb.app/Contents/MacOS/kaioken",
      kind: "direct",
      mode: "electron-node",
    });
    expect(
  ...
  it("gives a packaged Linux bridge its own AppImage mount", () => {
    expect(
      resolveBbAppProcessRuntime({
        env: {
          APPIMAGE: "/home/user/Apps/kaioken-x86_64.AppImage",
          APPDIR: "/tmp/.mount_bb",
        },
        isPackaged: true,
        platform: "linux",
        processExecPath: "/tmp/.mount_bb/kaioken",
      }),
    ).toEqual({
      appDirPath: "/tmp/.mount_bb",
      executablePath: "/home/user/Apps/kaioken-x86_64.AppImage",
      kind: "appimage",
      mode: "electron-node",
    });
  });
...
`,
  ...
  it("uses the inner executable for an unpacked Linux build", () => {
    expect(
      resolveBbAppProcessRuntime({
        ...
        isPackaged: true,
        platform: "linux",
        processExecPath: "/opt/kaioken/kaioken",
      }),
    ).toEqual({
      executablePath: "/opt/kaioken/kaioken",
      kind: "direct",
      mode: "electron-node",
    });
  });
  ...
  it("requires the host Node executable in desktop dev mode", () => {
    ...
    ).toThrow("KAIOKEN_DESKTOP_NODE_EXEC_PATH is required");

    expect(
      resolveBbAppProcessRuntime({
        env: {
          KAIOKEN_DESKTOP_NODE_EXEC_PATH: "/usr/local/bin/node",
        },
        isPackaged: false,
        platform: "linux",
        processExecPath: "/path/to/electron",
