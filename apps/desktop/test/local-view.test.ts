...
const localViewTestCases: LocalViewTestCase[] = [
  ...
  },
  {
    ...
    viewModel: {
      kind: "info",
      message:
        "A kaioken server is already running on this Mac. Connect via Window ▸ Server.",
  ...
  it.each(localViewTestCases)(
    ...
    (testCase) => {
      ...
      expect(html).toContain(
        '<div class="titlebar-drag-region" data-testid="kaioken-local-view-window-drag-region" aria-hidden="true"></div>',
      );
      expect(html).toMatch(
