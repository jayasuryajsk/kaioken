...
import {
  KAIOKEN_DESKTOP_SERVER_URL_DIALOG_CANCEL_CHANNEL,
  KAIOKEN_DESKTOP_SERVER_URL_DIALOG_SUBMIT_CHANNEL,
...
<body>
  ...
  <p>Point this app at a kaioken server. Leave empty to use only This Mac.</p>
  <form>
...
export function openServerUrlDialog(
  ...
  const result = new Promise<ServerUrlDialogResult>((resolvePromise) => {
    ...
    function finish(dialogResult: ServerUrlDialogResult): void {
      ...
      settled = true;
      ipcMain.removeHandler(KAIOKEN_DESKTOP_SERVER_URL_DIALOG_SUBMIT_CHANNEL);
    ...
    ipcMain.on(KAIOKEN_DESKTOP_SERVER_URL_DIALOG_CANCEL_CHANNEL, handleCancel);
