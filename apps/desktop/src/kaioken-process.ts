...
interface StartBbAppProcessArgs {
  ...
  logLineLimit: number;
  runtime: KaiokenAppProcessRuntime;
}

export interface KaiokenAppProcess {
  childProcess: ChildProcess;
  exit: Promise<KaiokenAppProcessExit>;
  logs: RuntimeLogBuffer;
  pid: number;
  stop(args: StopBbAppProcessArgs): Promise<void>;
}

export interface KaiokenAppProcessExit {
  code: number | null;
...
type KaiokenAppProcessRuntimeMode = "electron-node" | "node";

interface DirectBbAppProcessRuntime {
  executablePath: string;
  kind: "direct";
  mode: KaiokenAppProcessRuntimeMode;
}
...
interface KaiokenAppProcessLaunch {
  args: string[];
  env: NodeJS.ProcessEnv;
  executablePath: string;
}
...
interface CreateBbAppProcessEnvArgs {
  env: NodeJS.ProcessEnv;
  runtimeMode: KaiokenAppProcessRuntimeMode;
}
...
const APPIMAGE_BRIDGE_RELATIVE_PATH_ENV =
  "KAIOKEN_DESKTOP_APPIMAGE_BRIDGE_RELATIVE_PATH";
...
export function resolveBbAppProcessRuntime(
  ...
  const rawNodeExecPath = args.env.KAIOKEN_DESKTOP_NODE_EXEC_PATH?.trim();
  if (rawNodeExecPath === undefined || rawNodeExecPath.length === 0) {
    throw new Error(
      "KAIOKEN_DESKTOP_NODE_EXEC_PATH is required in desktop dev mode. Launch through apps/desktop/scripts/run-electron-dev.mjs.",
    );
  }
...
export function createBbAppProcessLaunch(
  ...
  if (
    ...
    throw new Error("kaioken-app bridge path must be inside the AppImage mount");
  }
...
function waitForProcessExit(
  ...
  return new Promise<KaiokenAppProcessExit>((resolvePromise) => {
...
export function startBbAppProcess(args: StartBbAppProcessArgs): KaiokenAppProcess {
  ...
  if (pid === undefined) {
    throw new Error("kaioken-app child process did not expose a PID");
  }
