...
function resolveDesktopUserDataDir(env, dataDir) {
  const rawUserDataDir = env.KAIOKEN_DESKTOP_USER_DATA_DIR?.trim();
...
function createElectronAppEnv(env, config) {
  ...
  childEnv.KAIOKEN_DESKTOP_NODE_EXEC_PATH = process.execPath;
...
// UI; when it is not, the desktop falls back to starting its own kaioken-app runtime.
...
if (viteReachable) {
  childEnv.KAIOKEN_DESKTOP_APP_URL = appUrl;
}

process.stdout.write(`@kaioken/desktop: instance ${devConfig.instanceId}\n`);
process.stdout.write(`@kaioken/desktop: data ${dataDir}\n`);
process.stdout.write(
  `@kaioken/desktop: server http://127.0.0.1:${devConfig.ports.serverPort}\n`,
);
process.stdout.write(
  `@kaioken/desktop: daemon http://127.0.0.1:${devConfig.ports.hostDaemonPort}\n`,
);
process.stdout.write(
  viteReachable
    ? `@kaioken/desktop: app ${appUrl} (Vite dev server — live reload)\n`
    : `@kaioken/desktop: app (own kaioken-app runtime — no Vite dev server on ${appUrl})\n`,
);
process.stdout.write(`@kaioken/desktop: user-data ${desktopUserDataDir}\n`);

// Extra Chromium/Electron switches for dev automation (e.g.
// KAIOKEN_DESKTOP_ELECTRON_ARGS="--remote-debugging-port=9223" for CDP-driven QA).
const extraElectronArgs = (process.env.KAIOKEN_DESKTOP_ELECTRON_ARGS ?? "")
  .split(" ")
  .map((arg) => arg.trim())
