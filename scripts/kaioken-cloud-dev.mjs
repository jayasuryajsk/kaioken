...
const DEV_BASE_DOMAIN = "kaioken.localhost";
...
function fail(message) {
  console.error(`kaioken Cloud dev: ${message}`);
  process.exit(1);
}
...
function spawnService(args, env = {}) {
  ...
  child.once("error", (error) => {
    ...
    console.error(`kaioken Cloud dev: ${error.message}`);
...
const web = spawnService(
  ...
  {
    KAIOKEN_CLOUD_DEV_APP_URL: CLOUD_URL,
    KAIOKEN_CLOUD_DEV_SERVER_URL_TEMPLATE: `http://{label}.${DEV_BASE_DOMAIN}:${ports.cloudPort}`,
    KAIOKEN_CLOUD_DEV_STATE_PATH: STATE_DIR,
    BETTER_AUTH_SECRET: DEV_SECRET,
...
console.log(`
Local kaioken Cloud is ready at ${BOLD}${CLOUD_URL}/dashboard${RESET}

Claim a handle, generate a pairing code, and run the command shown in the
dashboard against a kaioken started with pnpm dev. Local handles use:
