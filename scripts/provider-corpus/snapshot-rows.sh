...
#
# Requires KAIOKEN_PROVIDER_CORPUS_DIR (defaults to ~/.kaioken/provider-corpus when that
...
#
#   KAIOKEN_PROVIDER_CORPUS_ALLOWLIST=apps/server/test/provider-corpus/allowlists/<ws>.json \
#     scripts/provider-corpus/snapshot-rows.sh compare
#   KAIOKEN_PROVIDER_CORPUS_SNAPSHOT_DIR=$HOME/.kaioken/provider-corpus/snapshots/rows.<ws> \
#     scripts/provider-corpus/snapshot-rows.sh write      # shadow snapshot
#   KAIOKEN_PROVIDER_CORPUS_ROW_CLASSES=apps/server/test/provider-corpus/allowlists/<ws>-row-classes.json \
...
if [[ -z "${KAIOKEN_PROVIDER_CORPUS_DIR:-}" && -f "${HOME}/.kaioken/provider-corpus/manifest.json" ]]; then
  export KAIOKEN_PROVIDER_CORPUS_DIR="${HOME}/.kaioken/provider-corpus"
fi
if [[ -z "${KAIOKEN_PROVIDER_CORPUS_DIR:-}" || ! -f "${KAIOKEN_PROVIDER_CORPUS_DIR}/manifest.json" ]]; then
  echo "KAIOKEN_PROVIDER_CORPUS_DIR must point at a corpus directory with manifest.json" >&2
  exit 2
fi

export KAIOKEN_PROVIDER_CORPUS_SNAPSHOT="${mode}"
cd "${repo_root}"
pnpm exec turbo run test:provider-corpus --filter=@kaioken/server

snapshots_dir="${KAIOKEN_PROVIDER_CORPUS_DIR}/snapshots"
