# kaioken-plugin-slack-bot

The headless "Slack bot" hero plugin — no frontend entry, no dependencies.
Mention the bot in Slack and it spawns a Kaioken thread in your configured project;
...
- **Settings** — `botToken` and `signingSecret` as `secret: true` (stored in
  ...
  `project` picker (stores the Kaioken project id).
...
- **`bb.sdk.threads.spawn`** — project-default environment; Kaioken fills in
  `origin: "plugin"` and `originPluginId: "slack-bot"` automatically, so
  spawned threads are attributed in the thread list.
- **`bb.storage.kv`** — maps Slack `thread_ts` → Kaioken thread id (and back), so
  follow-up mentions land in the same Kaioken thread.
...
1. Install the plugin:
   ...
   ```
   kaioken plugin install ./examples/plugins/slack-bot
   ```
...
2. Create a Slack app (https://api.slack.com/apps → "From scratch"):
   ...
   - **Event Subscriptions**: enable, subscribe to the `app_mention` bot
     ...
     ```
     https://<your-kaioken-server>/api/v1/plugins/slack-bot/http/events
     ```

     Slack must be able to reach your Kaioken server (use a tunnel such as
...
3. Configure and reload:
   ...
   ```
   kaioken plugin config slack-bot set botToken xoxb-...
   kaioken plugin config slack-bot set signingSecret ...
   kaioken plugin config slack-bot set project proj_...
   kaioken plugin reload slack-bot
   kaioken plugin list        # slack-bot should show "running"
   ```

4. Mention the bot in a channel it has been invited to. `kaioken plugin logs
