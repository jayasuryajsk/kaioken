# kaioken-plugin-echo-provider

The **third-party canary** for kaioken's provider plugin API. It is a complete
agent provider — a picker entry, a bridge, a kaioken tool, plugin settings, and
...
- `@get-kaioken/plugin-sdk` (and its published subpaths `/provider-bridge`,
  `/host`, `/app`),
- `zod`,
- node built-ins and the plugin's own files.

Tests may add the published test harnesses (`@get-kaioken/plugin-sdk/testing`,
`@get-kaioken/plugin-sdk/provider-bridge/testing`) and the test runner. **No
`@kaioken/*` workspace package is imported anywhere**, and
`public-sdk-only.test.ts` fails the suite if one ever is. A marketplace
plugin cannot resolve kaioken's private packages; if this example needed one, the
...
| `experimental_nativeSkillRoots` — one workspace-relative skill root (`.echo/skills`) kaioken lists beside its own skills                                        | `server.ts`, asserted in `server.test.ts`          |
...
| `bb.agents.registerTool` with `presentation` — a kaioken tool whose row reads the way the plugin says                                                           | `server.ts`                                        |
...
| The plugin's kaioken tool, called over `item/tool/call` and stamped `server: "kaioken"` with the definition's presentation | `tool`                              |
...
1. **The public testing kit** (`@get-kaioken/plugin-sdk/provider-bridge/testing`):
   ...
   - `provider-bridge.parity.test.ts` replays `recordings/echo-agent/turn-tools`
     — a real recording kaioken made of this plugin's built artifact (record
     mode, `KAIOKEN_PROVIDER_BRIDGE_RECORD_DIR`) — through the bridge the way the
   ...
   - `server.test.ts` loads the plugin into the public fake host
     (`@get-kaioken/plugin-sdk/testing`) and asserts the registered declaration
...
```
kaioken plugin install ./examples/plugins/echo-provider --yes
kaioken plugin config echo-provider set shout true   # optional: prove the settings round trip
```

Then pick "Echo" in the provider picker and send a message. After editing
sources, `kaioken plugin reload echo-provider`.

## Test

```
pnpm exec turbo run test --filter=kaioken-plugin-echo-provider
```

## Re-record

The recording under `recordings/` is never rewritten. To capture a new one,
start a dev kaioken with `KAIOKEN_PROVIDER_BRIDGE_RECORD_DIR=<dir>` exported in the
...
node scripts/provider-recordings/package-cells.mjs --raw <dir> --cells /tmp/cells.tsv \
  ...
  --versions '{"echo-agent":"kaioken-plugin-echo-provider 0.1.0"}'
```
