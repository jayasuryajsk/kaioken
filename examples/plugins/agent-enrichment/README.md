# kaioken-plugin-agent-enrichment

The "agent enrichment" hero plugin: no UI bundle, no background services —
its entire surface is agent-facing. It demonstrates:

- **`bb.cli.register`** — a `kaioken docs` command. Agents run it through bash
  exactly like humans do (`kaioken docs search <query...>`); the handler runs
...
- **`bb.settings.define`** — a boolean (`caseSensitive`) rendered in Kaioken's
  settings UI and editable with `kaioken plugin config agent-enrichment`.
- **`bb.storage.kv`** — caches the last search (`kaioken docs last` prints it;
...
Dependencies: only `zod`, for the tool parameters. When Kaioken runs from a
source checkout the import resolves from Kaioken's own dependencies, so the
...
```
kaioken plugin install ./examples/plugins/agent-enrichment
kaioken plugin list
```
...
kaioken docs last
kaioken plugin config agent-enrichment set caseSensitive true
```
...
After editing sources, `kaioken plugin reload agent-enrichment`.
