...
SKILL = Path('.kaioken/skills/verify-kaioken')
CLI_OWNERS = {
...
def source_files(repo):
    ...
    return sorted({
        ...
        and (Path(name).suffix in {'.ts', '.tsx', '.json', '.css', '.mjs', '.sh'}
             or name == 'scripts/kaioken-dev-app')
    })
...
def collect(repo):
    ...
    add('dev-harness', [name for name in under('scripts/') if 'kaioken-dev-app' in name or 'cloud-dev' in name], ['developer-fixtures'])
...
def main():
    parser = argparse.ArgumentParser(description='Check Kaioken feature-map source drift without starting the app')
