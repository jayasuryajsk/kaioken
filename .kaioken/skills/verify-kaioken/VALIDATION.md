...
| Thread lifecycle | Submitted through the UI; final output was exactly `Kaioken verification complete.`; thread returned idle; response survived reload; rename persisted; archive set `archivedAt`; unarchive cleared it and restored the composer. Fixture Git status stayed clean. |
...
API reads and CLI calls. Later maintenance discovered that inherited `KAIOKEN_CLI`
