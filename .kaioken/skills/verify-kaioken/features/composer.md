...
| Voice input | Select a test microphone, start and stop recording, and compare a known clip with kaioken voice transcribe `<file>`. | Transcript enters the correct composer without an unintended send; denied permission and unavailable service are surfaced. |
