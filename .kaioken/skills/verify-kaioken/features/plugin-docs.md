...
Open the Docs panel; kaioken docs --help. Add a disposable local vault containing Markdown, HTML, a folder, and an image; use a second host only when available.
...
| CLI read/write | List/read/write/mkdir/move/remove a fixture through kaioken docs; compare bytes in UI. | CLI and UI operate on the same vault revision and path. |
