...
- Verify sourceThreadId and checkpoint linkage on the hidden fork. Side chats have parentThreadId=null; they are source-linked forks, not child threads in Kaioken parent/child notification semantics. Source: `plugins/side-chat/server.ts:99`.
- Cleanup is an hourly plugin background schedule (13 * * * *), with a 24-hour age threshold. No documented user CLI maintenance hook exists in this revision; use an owned dated-storage fixture and scheduler harness, and do not imply kaioken side-chat cleanup exists. Source: `plugins/side-chat/server.ts:135`.
