...
| Removed manager commands | Run kaioken manager and its compatibility subcommands on isolated CLI. | Nonzero exit explains that parent threads replace managers; no obsolete manager is created. |
| Guide and help | Use kaioken guide and nested --help for each command family and plugin command. | Discoverable descriptions, flags, and examples match runtime behavior. Do not treat a help listing as a live feature test. |
...
- `status` accepts --json, not --project/--environment flags. To verify in-thread context after the source env helper clears inherited context, set KAIOKEN_PROJECT_ID/KAIOKEN_THREAD_ID/KAIOKEN_ENVIRONMENT_ID to resolved synthetic IDs before the source CLI invocation; keep KAIOKEN_CLI/KAIOKEN_CLI_REEXEC unset. Source: `apps/cli/src/commands/status.ts:50`.
- Open Settings → Remote access (plugin id `connect`). Use `kaioken connect` for command help; this parser rejects `kaioken connect --help` as an unknown flag. Use `off` to disconnect and forget credentials, or the plugin enable toggle to suspend the tunnel while retaining pairing. Source: `plugins/connect/src/cli.ts:74`.
