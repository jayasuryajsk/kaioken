...
- Use the isolated Kaioken data directory for BB-user skills and disposable project .kaioken/.claude/.agents locations for scope tests. Provider-global skill paths still refer to real host directories; a source store does not isolate them. Source: `apps/server/src/services/skills/registry-skill-install.ts:309`.
- Skill Edit opens a new chat composer with skill ID/path and revision-aware CLI instructions. Complete an edit through that chat or kaioken skill update; do not expect an inline text editor. Delete has a cancelable confirmation. Source: `packages/shared-ui/src/components/ui/resource-edit-prompt.ts:15`.
...
- Run scaffolding with a wrapper that applies isolated source environment, clears KAIOKEN_CLI/KAIOKEN_CLI_REEXEC, changes to an owned temporary directory, then executes the absolute built source CLI path. A repo-root wrapper can otherwise scaffold inside the checkout. Source: `apps/cli/src/commands/plugin.ts:1181`.
