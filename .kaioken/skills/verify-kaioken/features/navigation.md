...
| Search threads and message contents | Use Search threads in the quick palette; query a unique title and a different string found only in a message. Compare kaioken thread search `<query>`. | Matching thread/message opens the correct thread and preserves search scope. |
...
| Sections | Create, rename, assign threads to, reorder, collapse, and delete a section. Use kaioken thread section --help for CLI forms. | Assignments and collapsed/order preferences persist; deleting a section does not delete its threads. |
| Grouping, sorting, display options | Use Sidebar display options for each offered grouping and sorting mode; change project order with kaioken project reorder; reload. | Every thread remains reachable exactly once in its applicable group; order persists. |
