...
1. Click **New thread** and select the fixture project if necessary. Fill the
   ...
   ```text
   Reply exactly: Kaioken verification complete. Do not use tools or change files.
   ```
...
3. Wait for a separate assistant response, **Kaioken verification complete.**
   ...
   Set `KAIOKEN_VERIFY_THREAD_ID` from the URL; do not reuse a previous run's ID.
