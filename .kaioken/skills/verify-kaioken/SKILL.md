---
name: verify-kaioken
description: Verify Kaioken user journeys in an isolated source dev app using dev-browser@next and the matching source CLI. Use after Kaioken feature changes or when asked to verify or smoke-test Kaioken. The feature map covers core UI and agent interfaces, every repository plugin, desktop, mobile, and hosted services; select the affected recipes and platform prerequisites.
---
...
```bash
.kaioken/skills/verify-kaioken/scripts/inventory.py
```
...
```bash
.kaioken/skills/verify-kaioken/scripts/test_inventory.py
```

These use temporary Git repositories and verify drift, missing ownership,
catalog consistency, and index coverage without starting Kaioken.
...
dev-browser --version
dev-browser --help
scripts/kaioken-dev-app status
```
...
legacy data from the parent `.kaioken-dev` directory. If `mkdir` finds an existing
dev directory, stop this setup and use another unused checkout; do not delete
or adopt the existing data.

```bash
export KAIOKEN_VERIFY_RUN="$(mktemp -d /tmp/kaioken-verification-XXXXXX)"
export KAIOKEN_VERIFY_BROWSER="verify-kaioken-$(basename "$BB_VERIFY_RUN")"
scripts/kaioken-dev-app status > "$BB_VERIFY_RUN/before-launch.txt"
command -v lsof >/dev/null || exit 1
for KAIOKEN_VERIFY_PORT in $(sed -nE 's/^(App|Server|Host daemon): http:\/\/[^:]+:([0-9]+)$/\2/p' "$BB_VERIFY_RUN/before-launch.txt"); do
  if lsof -nP -iTCP:"$BB_VERIFY_PORT" -sTCP:LISTEN; then
    exit 1
  fi
done
export KAIOKEN_VERIFY_DATA_DIR="$(sed -n 's/^Data dir: //p' "$BB_VERIFY_RUN/before-launch.txt")"
export KAIOKEN_VERIFY_APP_URL="$(sed -n 's/^App: //p' "$BB_VERIFY_RUN/before-launch.txt")"
test -n "$BB_VERIFY_DATA_DIR" && test -n "$BB_VERIFY_APP_URL" || exit 1
mkdir "$BB_VERIFY_DATA_DIR" || exit 1
printf '%s\n' "$BB_VERIFY_RUN" > "$BB_VERIFY_DATA_DIR/verify-kaioken-owner"
git rev-parse HEAD > "$BB_VERIFY_RUN/source-commit.txt"
scripts/kaioken-dev-app current > "$BB_VERIFY_RUN/launch.log" 2>&1
```
...
unset KAIOKEN_CLI KAIOKEN_CLI_REEXEC
...
`scripts/kaioken-dev-app env` deliberately clears the parent thread context,
including `KAIOKEN_THREAD_STORAGE`. Save the evidence location before evaluating
it. It targets the dev server and daemon. In that isolated shell, unset both
`KAIOKEN_CLI` and `KAIOKEN_CLI_REEXEC`, then use `node apps/cli/dist/index.js` for CLI
checks. The launcher builds this entry point through Turbo. If a separate build
is needed, run `pnpm exec turbo run build --filter=@kaioken/cli` first.

The source entry point otherwise reexecutes an inherited `KAIOKEN_CLI`, silently
using the installed client against the dev server. During maintenance,
`pnpm --silent kaioken:dev` also changed literal newlines in arguments into
...
`kaioken` outside it for coordination with the parent Kaioken thread.

Commands such as workflows require thread context. After evaluating the dev
environment, set `KAIOKEN_THREAD_ID`, `KAIOKEN_PROJECT_ID`, and `KAIOKEN_ENVIRONMENT_ID` only
...
```bash
dev-browser --headless -b "$BB_VERIFY_BROWSER" -e "const p = await browser.getPage('kaioken'); await p.goto('$BB_VERIFY_APP_URL'); console.log(await p.snapshot({interactive:true}));"
```

For later steps use `dev-browser --headless -b "$BB_VERIFY_BROWSER"` with a
quoted heredoc, get page `kaioken`, and follow the feature recipe. Read a fresh
...
```bash
test "$(cat "$BB_VERIFY_DATA_DIR/verify-kaioken-owner")" = "$BB_VERIFY_RUN" || exit 1
dev-browser stop "$BB_VERIFY_BROWSER"
scripts/kaioken-dev-app stop
scripts/kaioken-dev-app status > "$BB_VERIFY_RUN/after-stop.txt"
