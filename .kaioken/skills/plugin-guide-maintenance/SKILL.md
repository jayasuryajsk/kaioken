...
The Plugin Guide is kaioken's public Plugin SDK reference. For a public API change,
...
pnpm exec turbo run test typecheck \
  --filter=@get-kaioken/plugin-sdk \
  --filter=@kaioken/plugin-api-map \
  --filter=@kaioken/app \
  --filter=kaioken-plugin-plugin-api-docs
kaioken plugin build plugins/plugin-api-docs
```
...
`scripts/kaioken-dev-app current`; inspect the affected entry and reachable actions
