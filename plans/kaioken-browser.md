...
Add `kaioken browser ...` CLI commands that drive visible in-app browser automation
inside the kaioken desktop app.
...
- `plans/kaioken-settings.md` covers scriptable settings.

Status (2026-08-03): not started. `apps/cli/src/commands/browser.ts` does not
exist, and there is no `kaioken-browser` built-in skill.

## Current State

- The kaioken CLI uses Commander under `apps/cli/src/index.ts` and command modules
...
- `dev-browser` is an external CLI that controls Playwright browsers and can
  connect to Chrome/CDP endpoints, but kaioken's in-app `WebContentsView` is not
  currently exposed as a CDP endpoint or browser-testing primitive.

## Recommendation

Make `kaioken browser ...` the public, reproducible surface. Add dynamic tools later
...
targets, including trusted kaioken UI. A scoped kaioken-owned bridge is safer and gives kaioken
...
```bash
kaioken browser open <url> --visible --json
kaioken browser list --json
kaioken browser snapshot <target-id> --json
kaioken browser click <target-id> --selector 'button[type=submit]'
kaioken browser click <target-id> --x 420 --y 315
kaioken browser type <target-id> --selector '#email' --text 'user@example.com'
kaioken browser eval <target-id> --script-file script.js --json
kaioken browser close <target-id>
```

Prefer stable target ids over implicit "active window" behavior. Commands may
default `--thread` from `KAIOKEN_THREAD_ID` when available, matching existing kaioken CLI
context conventions.
...
- `pnpm exec turbo run typecheck --filter=@kaioken/server-contract`
...
- `pnpm exec turbo run test --filter=@kaioken/server -- browser`
...
- `kaioken browser open --visible` opens a visible in-panel browser tab.
...
- `pnpm exec turbo run test --filter=@kaioken/app -- BrowserTabDeck`
- `pnpm exec turbo run test --filter=@kaioken/app -- useThreadFileTabs`
...
- `pnpm exec turbo run test --filter=@kaioken/desktop -- desktop-browser`
- `pnpm exec turbo run typecheck --filter=@kaioken/desktop`
- Manual smoke test through `scripts/kaioken-dev-app current`
...
- `kaioken browser open https://example.com --visible --json` returns a target id.
- `kaioken browser snapshot <target-id> --json` returns structured page state and/or
  screenshot path/data metadata.
- `kaioken browser click/type/eval/close` operate on that target.
- Commands are usable from an agent shell without browser-specific MCP tools.

Validation:

- `pnpm exec turbo run test --filter=@kaioken/cli -- browser`
...
- Add `apps/server/src/services/skills/builtin-skills/kaioken-browser/SKILL.md`.
- Teach agents to use `kaioken browser ...` for visible browser QA and screenshots.
...
- Mention related commands from the `kaioken terminal` and `kaioken settings` plans once
  they exist.
- Do not instruct agents to use `dev-browser` for kaioken's in-app browser. Keep
...
- The skill references real kaioken CLI commands, not provider-specific tools.
- A user data-dir skill with the same name can override the built-in.

Validation:

- `pnpm exec turbo run test --filter=@kaioken/server -- builtin-skills-copy`
...
- Should `kaioken browser snapshot` return screenshots as files, data URLs, or both?
