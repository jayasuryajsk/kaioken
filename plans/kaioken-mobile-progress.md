# kaioken Mobile — execution log

Running log for the autonomous build of `apps/mobile` (see
`plans/kaioken-mobile-expo.md` for the plan and decisions). Newest entry last.
...
`kaioken/build-expo-react-native-app-thr_qvau3b2d5b` (one commit per phase, plus
a merge of `origin/main`; last commit `7d1b9f30b`); M5 (Phase 8) is deferred. The plan's own "Status" block in
`plans/kaioken-mobile-expo.md` lists what shipped per milestone and what needs
...
the Phase 7 integration entry): first run → Add server (Direct URL or kaioken
...
links (`kaioken://threads/<id>`, the web alias, settings), the workspace panel
...
and 822 unit tests green for `@kaioken/mobile`; `@kaioken/app` / `@kaioken/server` /
`@kaioken/sdk` / `@kaioken/cli` / `@kaioken/integration-tests` typecheck green.
...
that opens the drawer also presses the home row under the touch on a kaioken
connect profile; Direct profiles are fine — Phase 7 integration entry),
inbound "Send to kaioken" share needs the `expo-share-intent` native rebuild,
...
- Backend for e2e: `pnpm --filter @kaioken/integration-tests e2e:mobile-backend`
  (port 41999; fake provider with user questions).
- Metro: `cd apps/mobile && EXPO_PUBLIC_KAIOKEN_SERVER_URL=http://127.0.0.1:41999 EXPO_PUBLIC_KAIOKEN_E2E=1 pnpm dev --port 8082`
  (`EXPO_PUBLIC_KAIOKEN_E2E=1` wipes profiles/preferences on every launch so flows
  start at first run; drop it for manual QA).
- Flows: `cd apps/mobile && pnpm e2e:ios` (Maestro; needs `JAVA_HOME`).
- Checks: `pnpm exec turbo run typecheck lint test --filter=@kaioken/mobile`.
...
- **Data + connectivity layer** (`apps/mobile/src/lib/**`, pure TS, vitest):
  ...
  `sdk/` (`createMobileSdk` on `@kaioken/sdk/browser` + `x-bb-app-surface:
...
- **App shell** (`app/**`, `src/app-shell`, `src/screens`): root layout
  ...
  `appearance.themeId`. E2E reset: `EXPO_PUBLIC_KAIOKEN_E2E=1` wipes local state
  on launch; `kaioken://e2e/reset` in dev/e2e builds.

How verified:

- `pnpm exec turbo run typecheck lint test --filter=@kaioken/mobile`: green
  (17 test files, 124 tests, 0 lint warnings).
- Maestro on iPhone 17 Pro (iOS 26.3) against the harness backend on
  `127.0.0.1:41999` with Metro on 8082 + `EXPO_PUBLIC_KAIOKEN_E2E=1`:
...
- `packages/client-core` (`@kaioken/client-core`): DOM/React-free package holding
  ...
  Deviation: depends on `@kaioken/desktop-contract` (pure) for two length limits.
...
- **Sidebar / list UI** (`src/screens/sidebar`, `threads`, `shell`): the
  drawer is the kaioken sidebar (server switcher → ActionSheet, New thread /
...
- `@kaioken/sdk`: `sdk.projects.sidebarBootstrap()` (typed
  `GET /projects/sidebar-bootstrap`).

How verified:

- `pnpm exec turbo run typecheck lint test --filter=@kaioken/mobile
--filter=@kaioken/client-core --filter=@kaioken/sdk`: green (mobile 32 test files /
  204 tests, 0 lint warnings; client-core 232; sdk 93). `@kaioken/app` typecheck
  green. Prettier clean on every touched file.
- Maestro on iPhone 17 Pro (iOS 26.3) against a fresh harness backend on
  `127.0.0.1:41999` + Metro on 8082 with `EXPO_PUBLIC_KAIOKEN_E2E=1`: all four
...
- `getThreadDisplayTitle` is duplicated from `apps/app/src/lib/thread-title.ts`
  in `src/data/threads/thread-title.ts`; move it into `@kaioken/client-core` when
...
- Maestro on iOS: `back` does not exist (tap `id: BackButton`); the dev
  ...
  private backend (`KAIOKEN_MOBILE_E2E_PORT=42999 … e2e:mobile-backend`).
...
- **Data** (`src/data/thread-detail/`): `useThreadDetailBootstrap`
  ...
  controller on the `@kaioken/client-core` merge/paging helpers: latest window +
...
- **Timeline list** (`src/screens/thread/timeline/`): `rows.ts` projects
  `TimelineRow[]` through `@kaioken/thread-view` (`buildTimelineViewRows`,
...
`EXPO_PUBLIC_KAIOKEN_E2E=1`, iPhone 17 Pro iOS 26.3):

- `pnpm exec turbo run typecheck lint test --filter=@kaioken/mobile
--filter=@kaioken/integration-tests`: green (mobile 57 test files / 365 tests,
...
composer + interactions), kaioken connect / push (Phase 5), the workspace panel
(Phase 6).
...
- Delegation rows carry no child thread id in the wire contract, so there is
  no tap-to-open-child; kaioken child threads surface as system/conversation rows.
...
- The dev showcases `/dev/work-rows` and the thread row host need an active
  ...
  without one. A `kaioken://` deep link reloads the dev client and, under
  `EXPO_PUBLIC_KAIOKEN_E2E=1`, wipes the profile — flows go through the drawer.
- Summary/turn/delegation view rows are re-projected by `@kaioken/thread-view` on
...
- **Interactions** (`src/data/interactions/`, `src/screens/thread/interactions/`):
  ...
  `@kaioken/plugin-interaction-contracts` package (both plugins re-export their
...
with `EXPO_PUBLIC_KAIOKEN_E2E=1`, iPhone 17 Pro iOS 26.3):

- `pnpm exec turbo run typecheck lint test --filter=@kaioken/mobile`: green (81
...
- Independent verification pass (fresh harness backend, iPhone 17 Pro):
  `pnpm exec turbo run typecheck lint test` green for `@kaioken/mobile` (81
  files / 518 tests, 0 lint warnings), `@kaioken/plugin-interaction-contracts`,
  `@kaioken/client-core`, `kaioken-plugin-ask-user-question` (36) and
  `kaioken-plugin-secrets` (8); `@kaioken/integration-tests test:smoke` green (28);
...
- The model pill truncates at 220 px ("Fake Model · Fast Medium" clips).
  ...
  Under `EXPO_PUBLIC_KAIOKEN_E2E=1` any Metro reload (a file save) wipes the
...
- `apps/app/src/components/promptbox/mentions/find-active-trigger.ts`: main's
  256-char trigger scan window applied to the `@kaioken/client-core` copy
...
## Phase 5 — kaioken connect, push notifications, deep links (done 2026-08-19) → M2.5
...
- **Pairing surfaces (web + CLI)**: Settings → Remote access (connected
  ...
  `kaioken connect machine-code [--json]` prints code / server / apex / expiry
  (JSON = the QR payload; `plugins/connect/src/cli.ts`).
  `@kaioken/connect-client` gained zod-free
  ...
  Guide templates (`kaioken-guide-environments`, `kaioken-guide-overview`), the kaioken-cli
  skill, `docs/configuration.md` ("Pairing the kaioken mobile app") and
  `docs/multiple-devices.md` ("Use the kaioken mobile app") describe both.
- **Mobile enrollment** (`src/data/connect/`, `src/screens/connect/`,
  route `/connect`): `parseConnectPairingPayload` (QR JSON, `kaioken://connect?…`
  URLs, bare codes), `resolveEnrollmentTarget` (handle or URL → apex;
  "Self-hosted kaioken connect…" reveals the apex field), `redeemEnrollment` →
  ...
  code). Add server offers "Connect with kaioken connect" above the direct form;
...
- **Push, server side** (`packages/db` migration `0102_push_subscriptions`
  ...
  node), CLI `kaioken notifications push-subscriptions list|add|remove [--json]`,
  env `KAIOKEN_PUSH_NOTIFICATIONS` (default true) + `KAIOKEN_EXPO_PUSH_URL`
  (startup-only; `packages/config`, kaioken-app launcher list, docs/guide/skill).
...
- **Deep links** (`src/lib/links/incoming-link.ts`, `app/+native-intent.tsx`,
  `app.json`): `kaioken://` scheme + universal links (`applinks:getbb.app`,
  ...
  (`ThreadOpenSignalHandler`). Cloud: `@kaioken/connect-db/src/app-links.ts` is
...
- **Connect e2e stub** (`tests/integration/mobile-e2e/connect-stub.ts`,
  `pnpm --filter @kaioken/integration-tests e2e:mobile-connect-stub`): TLS apex
  ...
  backend only with a valid `__Secure-kaioken-connect.desktop_session` cookie,
  ...
  `~/.kaioken-mobile-e2e/connect-stub-certs` installed into the simulator with
  `KAIOKEN_MOBILE_E2E_SIMULATOR=<udid>`. TLS + `*.localhost` are required because
  `@kaioken/connect-client` insists on `<label>.<apex host>` and ATS refuses plain
...
- `pnpm exec turbo run typecheck lint` green for `@kaioken/mobile`, `@kaioken/server`,
  `@kaioken/server-contract`, `@kaioken/sdk`, `@kaioken/cli`, `@kaioken/db`, `@kaioken/config`,
  `@kaioken/templates`, `@kaioken/integration-tests`, `@kaioken/app`, `kaioken-plugin-connect`,
  `@kaioken/connect`, `@kaioken/connect-client`, `@kaioken/connect-db`, `@kaioken/web`,
  `@kaioken/domain`; `turbo run test` green for `@kaioken/mobile` (90 files / 596
  tests), `@kaioken/db` (30 files), `@kaioken/sdk`, `@kaioken/cli` (49 files), `@kaioken/config`,
  `@kaioken/connect` (worker tests incl. the four `.well-known` cases),
  `kaioken-plugin-connect`, `@kaioken/connect-client`, `@kaioken/connect-db`,
  `@kaioken/server-contract`, `@kaioken/domain`, `@kaioken/templates`, `@kaioken/web`; server
  `test/services/push-sender.test.ts` + `test/public/public-notifications.test.ts`
  (13); `@kaioken/integration-tests test:smoke` (28). `drizzle-kit generate`
...
- Review: no accepted-but-ignored route/command fields (the push request is
  ...
  parity for the notifications routes and `kaioken connect machine-code`; no
...
- Maestro on one backend, in this order, all PASS: `phase5-connect` (first
  run → Connect with kaioken connect → EXPIRED-CODE inline error → STUB-PAIR +
  ...
  `phase5-links` (`kaioken://threads/<id>`, `kaioken://projects/<p>/threads/<t>`,
  `kaioken://settings/servers`, `kaioken://settings` with the push row),
  ...
  Screenshots: `/tmp/p5-verify/` (Add server with the kaioken connect row, the
...
- Fix from this pass: Maestro's `hideKeyboard` stopped working on the Add
  server screen once the kaioken connect row lengthened it (the swipe no longer
...
- `openLink kaioken://…` from a thread pushed via `router.push` sometimes lands
...
- **Terminal** (`src/data/terminals/`, `src/screens/terminal/`,
  ...
  `terminals-changed`. `@kaioken/client-core` `TerminalWebSocketTransport` gained
  ...
  (`pnpm --filter @kaioken/mobile terminal:build`, esbuild, deterministic) into
...
with `EXPO_PUBLIC_KAIOKEN_E2E=1`, iPhone 17 Pro `5E5752AC`):

- `pnpm exec turbo run typecheck lint test --filter=@kaioken/mobile
--filter=@kaioken/client-core` green (mobile 112 files / 747 tests, 0 lint
...
- Maestro, all PASS in this order: `phase6-panel` (Info rows, Changed files →
  ...
  Copy line), `phase6-terminal` (panel → Start → full screen → `echo kaioken-42`
...
- Not verified here: Android, a physical device, the terminal socket
  through the kaioken connect gate (`wss://<handle>.getbb.app/ws/terminals/…`;
...
- Maestro: a flow's `env:` block wins over `-e`, so non-default ports need
  a copied flow; under `EXPO_PUBLIC_KAIOKEN_E2E=1` a Metro reload mid-flow wipes
...
Release --no-bundler --device <udid>` with `EXPO_PUBLIC_KAIOKEN_E2E=1` and
  `EXPO_PUBLIC_KAIOKEN_SERVER_URL` in the env (the Xcode bundle phase runs `expo
export:embed`, which inlines them — verified locally: the Release bundle
  contains `EXPO_PUBLIC_KAIOKEN_E2E:"1"` and the 41999 URL), the harness backend
  ...
  `@kaioken/mobile` already run in `ci.yml` (`Checks` builds + typechecks + lints
...
KAIOKEN_E2E_EMBEDDED_BUNDLE=1` → `launchApp`; otherwise the dev-client
  ...
  it; `smoke.yaml` also handles the first-`kaioken://`-link "Open" prompt. The
...
- **Docs**: `docs/platform-support.md` gained a "Mobile app" section
  (platforms, Direct vs kaioken connect, distribution, push prerequisites, what
  ...
  Release sections; `plans/kaioken-mobile-expo.md` status line + "Status
  ...
  release PR, `docs/kaioken-release-process.md`), so no entry was added.
...
- `expo export:embed --dev false` with the CI env: 13.9 MB bundle with the
  inlined `EXPO_PUBLIC_KAIOKEN_E2E:"1"` / server URL. A niced Release build of
  the workspace with the same env from this Mac (Xcode 26.2, `xcodebuild`
  with `-scheme kaioken -configuration Release -sdk iphonesimulator`):
...
- `pnpm exec turbo run typecheck lint test --filter=@kaioken/mobile`: typecheck +
...
- **Share**: "Share link" in the thread "…" menu (`src/lib/share/share-thread.ts`,
  ...
  the thread URL). Inbound "Send to kaioken": `share-intent.ts` loads
...
- **E2E**: `e2e/flows/phase7-plugins.yaml` (harness: Settings → Plugins empty
  ...
  group from the harness catalog) → Skills → filter `kaioken-cli` → detail
...
- Metro caveats hit here: `CI=1` disables file watching (stale bundle); with
  `EXPO_PUBLIC_KAIOKEN_E2E=1` a Fast Refresh caused by another agent's edits
  ...
  persisted profile; `kaioken://e2e/reset` on a running app returns to first run
  — a cold `kaioken://` link lands on the dev-client launcher, and a reset under
...
/settings/*` with an optimistic write into the cached `/system/config` and
  ...
  `summarizeMachineUpdates`, `kaiokenAppRowState`; `useCheckForUpdates` =
...
/hosts/:id/permission-ceiling` through the profile fetch — owner-session
  ...
  `@kaioken/host-daemon-contract` (pure zod) for the provider CLI / usage types.
- **Screens** (`src/screens/settings/`, `src/screens/machines/`, routes
  ...
  account); Updates (kaioken-app row with copy upgrade command + "Check for
  ...
  PATCH, projects on this machine, kaioken agent update status + Retry, provider
...
test --filter=@kaioken/mobile --filter=@kaioken/client-core
--filter=@kaioken/plugin-interaction-contracts`: green (mobile 122 files / 822
  tests, 0 lint findings — the three warnings and the rule-not-found error
  the agents reported were already gone); `typecheck lint` for `@kaioken/app`,
  `@kaioken/server`, `@kaioken/sdk`, `@kaioken/cli`, `@kaioken/integration-tests`: green (the
  144 `@kaioken/app` lint warnings are pre-existing on `main`). Prettier check on
  every changed file: clean except `docs/repository-overview.md` and the
  regenerated `packages/plugin-sdk/bundled-types/kaioken-plugin-sdk.d.ts`, both
...
- Full on-device regression, one fresh harness backend (41999) + Metro 8082
  with `EXPO_PUBLIC_KAIOKEN_E2E=1`, iPhone 17 Pro (`5E5752AC`), Maestro 2.8.0;
...
- **Bug found (not fixed): edge swipe presses the home row on a kaioken connect
...
didActivateInViewWithTouchHandler`) is not reaching the row's `Pressable`
  ...
  a user who pairs through kaioken connect and edge-swipes will open a thread
  they did not tap.
...
Verified: `turbo run typecheck lint test --filter=@kaioken/mobile` (813 tests).
