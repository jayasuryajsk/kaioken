# kaioken Mobile (Expo React Native)

Status (2026-08-19): plan v3, **built through M4** (Phases 0–7) on branch
`kaioken/build-expo-react-native-app-thr_qvau3b2d5b`; M5 (Phase 8) deferred. The
decisions below were confirmed with Sawyer in a grilling session on
2026-08-18; the execution log is `plans/kaioken-mobile-progress.md`. See "Status
...
with `pnpm exec turbo run typecheck lint test --filter=@kaioken/mobile` green at
each step; the per-phase details, verification notes and carried-forward
items live in `plans/kaioken-mobile-progress.md`.
...
- **M1 – Read (Phases 1–4a)**: Direct server profiles (SecureStore, probe,
  ...
  stack shell with connection banner, `@kaioken/client-core` (pure modules
...
- **M2 – Act (Phase 4b)**: the shared native composer (mentions, typeahead,
  ...
  (`@kaioken/plugin-interaction-contracts` shared with the plugins), context
  banner with PR actions, message / thread / git action sheets, fork /
  handoff / edit.
- **M2.5 – Reach (Phase 5)**: kaioken connect enrollment (QR / code, account
  servers, re-pair, session hardening), "Add mobile device" in web Settings →
  Remote access + `kaioken connect machine-code`, push notifications end to end on
  the server (`push_subscriptions`, routes, SDK, CLI, Expo Push sender) and
  in the app (registration, tap routing, badge, per-profile toggle), `kaioken://`
...
| Topic        | Decision                                                                                                                                                                                                                            |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Audience     | Real product: kaioken users must be able to use it through kaioken connect. Dogfood first, no dev-only shortcuts in product paths.                                                                                                            |
| Platforms    | iOS first. Android after M2 (SDK not installed until then). Code stays platform-neutral.                                                                                                                                            |
| Connectivity | Both modes. Direct (Tailscale/LAN/simulator) first for dev and M1 dogfood; kaioken connect is the must-work path (Phase 5).                                                                                                              |
...
| Pairing UX   | QR/code from web Settings → Remote access (+ `kaioken connect machine-code`) for dogfood/TestFlight; GitHub sign-in pairing page + deep link before public release.                                                                      |
...
| Compat       | App targets the current server contract; on mismatch show an "update kaioken" banner and degrade. New routes are additive.                                                                                                               |
| Direct mode  | Allowed with a warning for non-loopback http; https/Tailscale Serve recommended.                                                                                                                                                    |
| Shared code  | Hybrid: extract already-pure modules into `@kaioken/client-core` now; mobile copies the query hooks; revisit after M2.                                                                                                                   |
...
| Design       | kaioken tokens and typography + native navigation, sheets, gestures, keyboard.                                                                                                                                                           |
...
| Identity     | Name "kaioken", bundle id `app.getbb.mobile`, scheme `kaioken://`, package `@kaioken/mobile` in `apps/mobile`. Apple team `9QCU24SXK5`. **Expo/EAS account still to be created** (needed for EAS Build/TestFlight and Expo Push; not for Phase 0). |
| Execution    | Phase 0 in one thread (spike). Then parallel kaioken child threads per work item; Sawyer reviews PRs. No date pressure.                                                                                                                  |
...
Ship a native iOS and Android app for kaioken. The app must look and behave like the
...
The app is a **client only**. It talks to a kaioken server over the existing HTTP +
...
- Reuse the headless packages as-is: `@kaioken/sdk/browser`, `@kaioken/server-contract`,
  `@kaioken/domain`, `@kaioken/thread-view`, `@kaioken/core-ui`, `@kaioken/connect-client`.
- Extract the web app's headless data layer (query keys, cache owners, realtime
  ...
  new `@kaioken/client-core` package behind a `ClientCoreProvider`. This is its own
  phase with its own budget. Rendering is native.
- Two connection modes: **Direct** (LAN / Tailscale / simulator, no auth, same
  as the PWA today) first, then **kaioken connect** (`https://<handle>.getbb.app`)
...
- Tests: pure logic with vitest, screens with Maestro on the iOS Simulator
  against a real kaioken server driven by the existing integration harness (fake
...
- `apps/mobile` (`@kaioken/mobile`) exists: Expo SDK 57.0.14 / RN 0.86.2 /
  ...
  Identity `kaioken` / `app.getbb.mobile` / `kaioken://`, iPad allowed, cleartext +
...
- **Metro resolves workspace packages** from TS source: `metro.config.js`
  applies the `source` export condition only to `@kaioken/*` / `@get-bb/*` (a
  ...
  workspace sources.
- **Runtime spike passed on the simulator** (`app/index.tsx`): `@kaioken/domain`,
  `@kaioken/thread-view`, `@kaioken/server-contract`, `@kaioken/connect-client`,
  `@kaioken/sdk/browser` evaluate under Hermes; `GET /system/config` via the SDK;
...
- **Connect cookie spike passed against `bee.getbb.app`**
  ...
  with `sharedCookiesEnabled` renders the fully authenticated kaioken SPA. **No
...
- E2E skeleton: `tests/integration/mobile-e2e/backend.ts` starts the
  ...
  two threads, prints JSON; `pnpm --filter @kaioken/integration-tests
e2e:mobile-backend` (turbo task builds `@get-kaioken/plugin-sdk` first).
  `apps/mobile/e2e/flows/smoke.yaml` passes end to end with Maestro
  (`pnpm --filter @kaioken/mobile e2e:ios`, sets `JAVA_HOME`).
...
1. The web app is ~290k lines; the timeline row model is projected on the
   **server** by `@kaioken/thread-view` (`apps/server/src/services/threads/timeline.ts`).
...
2. `@kaioken/sdk/browser` is DOM-free (`packages/sdk/src/browser.ts`,
   `transport-http.ts`, `realtime-client.ts`). The root `@kaioken/sdk` export must
...
5. Local server has no user auth; only an Origin guard
   ...
   non-kaioken port are rejected unless configured (`:118-131`).
6. kaioken connect gate (`apps/connect/src/worker.ts`): the machine credential header
...
18. The workspace resolves one `react@19.2.4`; Expo SDK 57 pins `19.2.3`. RN
    ...
    to avoid a second copy (hooks in `@kaioken/client-core` would break).

## Decisions required before Phase 0

1. App identity: bundle id / package name (e.g. `app.getbb.mobile`), app name,
   URL scheme (`kaioken://`), Apple Developer team, EAS org/project. Needed for
   Maestro `appId`, deep links, push.
2. Version alignment: `apps/mobile` uses workspace `react` (19.2.4),
   `@tanstack/react-query`, `jotai`, `zod`; `@kaioken/client-core` declares them as
...
5. Where the e2e backend lives: `tests/mobile-e2e` workspace package (it needs
   `@kaioken/agent-runtime/test`, `@kaioken/host-daemon`, `@kaioken/server`,
...
| Styling      | NativeWind (v5 if the spike passes, else v4) + `react-native-reusables` (`@rn-primitives`) + cva                     | Class vocabulary and variant model close to `@kaioken/shared-ui`. Tamagui rejected (different token model).                                                    |
...
| Data         | TanStack Query 5, jotai (MMKV storage adapter), `@kaioken/sdk/browser`, `@kaioken/client-core`                                 | Same cache/invalidation semantics as web. One `QueryClient` per server profile (query keys are not server-scoped).                                        |
...
1. **Direct** (first): user enters `http(s)://host:port` (LAN with
   ...
   is `127.0.0.1` (emulator case). Direct URLs on non-kaioken ports need the
   Origin guard to accept the RN WebSocket Origin (verify in the spike;
   `KAIOKEN_APP_URL` config is the escape hatch).
2. **kaioken connect** (after M1): the phone enrolls as a connect **machine**
   exactly like the desktop app:
   - Web app Settings → Remote access gets an "Add mobile device" action, and
     the CLI gets `kaioken connect machine-code [--json]` (SDK + CLI parity rule).
   ...
   - Phone scans/enters the code → `redeemMachineCredential`
     (`@kaioken/connect-client`) → stores `{serverUrl, handle, credential(bbcm_)}`.
...
### A3. Shared headless code: `@kaioken/client-core` (own phase)
...
- Rule: no DOM, no `window`, no react-router, no `localStorage`, no
  `@/lib/kaioken-desktop`. A vitest `node` environment guards it.
...
`hooks/queries/system-queries.ts` (`@/lib/kaioken-desktop` → injected desktop
bridge or split), `lib/route-paths.ts:2` (`matchPath` → tiny matcher),
`lib/fixed-panel-tabs-state.ts` (`@kaioken/desktop-contract`, `nanoid`),
...
- queries/mutations behind the provider; (3) `useThreadTimelineController`.
  ...
  `turbo run test --filter=@kaioken/app` must stay green per PR; the 28 mocking tests
...
| In-app browser tab                                         | `react-native-webview` tab                                                                                                                                                                                                                                                                                                                             | Cannot reach host `localhost` unless shared via kaioken connect expose.                                                                                                                                                                                             |
...
- v1: forms for `ask-user-question` and `secret-request` payloads. Move those
  ...
  (`@kaioken/plugin-interaction-contracts` or `@kaioken/domain`) consumed by both the
...
createdAt, lastSeenAt}` (Drizzle migration + regenerated snapshot), routes
  ...
  `kaioken notifications push-subscriptions list|add|remove` CLI, docs surfaces per
  `docs/cli-guide-and-skill.md` (guide templates regen, `kaioken-cli/SKILL.md`,
...
- Sender: triggers are (1) a new pending interaction (`interactions-changed`),
  ...
  the reply, or the error). No APNs/FCM keys in kaioken; the EAS project holds
...
| M2.5 – Reach                                       | kaioken connect enrollment + session, push notifications, deep links                                                                                                                                         | 5           |
...
1. Create `apps/mobile` with `create-expo-app` (blank TS) + `expo-dev-client`,
   name `@kaioken/mobile`, scripts `dev`, `ios`, `android`, `typecheck`, `lint`,
   `test`, `build` (`expo export` or no-op); add `@kaioken/mobile#dev` to
...
3. `metro.config.js`: `unstable_enablePackageExports = true`,
   ...
   `no-restricted-imports` for `@kaioken/sdk` root and `@kaioken/shared-ui`.
...
5. Runtime spike in `App.tsx`: import `@kaioken/domain`, `@kaioken/server-contract`,
   `@kaioken/thread-view`, `@kaioken/sdk/browser`, `@kaioken/connect-client`; call
...
6. Connect spike (no product UI): with a test machine credential, mint a
   ...
   fallback is needed.
7. `tsconfig.json` extends `@kaioken/tsconfig/base.json` + `typecheck-overrides.json`
...
supportsNativeUserQuestion: true })`, seeds with `createProjectFixture` /
   `createReadyThread`, prints `KAIOKEN_SERVER_URL`. Agent-runtime work item: add an
...
9. Maestro: `flows/smoke.yaml` opens the dev-client via `openLink`
   ...
   build; an e2e-only reset entry (`EXPO_PUBLIC_KAIOKEN_E2E=1` deep link) clears
   SecureStore/MMKV; `pnpm --filter @kaioken/mobile e2e:ios` sets `JAVA_HOME`.
...
### Phase 2 – `@kaioken/client-core`, hybrid (3–5 days, can overlap Phase 1)

Decision: hybrid. Extract only the already-pure modules listed in A3 step (1)
into `@kaioken/client-core` (web re-exports from the old paths; no provider
...
M2 when the mobile shapes are known. `turbo run test --filter=@kaioken/app` stays
green per PR.
...
### Phase 5 – kaioken connect, push, deep links (1.5–2 weeks) → **M2.5**
...
1. Connect enrollment UI (QR via `expo-camera`, manual code), session install
   - renewal, re-auth screen, profile list of the account's servers
     ...
     Remote access + `kaioken connect machine-code` CLI + guide/skill updates +
...
3. Deep links: `kaioken://` scheme + universal links → same paths as web via
...
1. Settings screens over `/system/config` + `PUT /settings/*`: general,
   ...
   host is offline.
2. Share sheet target ("Send to kaioken" → new thread with text/image), clipboard,
...
5. TestFlight / Play internal via EAS; `eas update` for JS-only fixes; crash
   reporting decision (none exists in kaioken today).
...
| "Add mobile device" QR/code UI + `kaioken connect machine-code` CLI + guide/skill/config docs                        | `plugins/connect/app.tsx`, `plugins/connect/src/cli.ts`, `docs/cli-guide-and-skill.md` surfaces                                                             | 5     | Uses existing `createMachineCode` RPC.  |
| Push subscriptions + sender + SDK + CLI + docs                                                                  | `packages/db` migration, `apps/server`, `packages/server-contract`, `packages/sdk`, `apps/cli`, guide templates, `kaioken-cli/SKILL.md`, `docs/configuration.md` | 5     | No daemon change → no protocol bump.    |
...
- **Pure logic (vitest, node env)**: everything in `@kaioken/client-core` and
...
- **Live QA**: `scripts/kaioken-dev-app current` + the dev-client on the simulator
  against real providers; physical iPhone via a Tailscale Serve URL, a
  temporary `KAIOKEN_SERVER_BIND_HOST=0.0.0.0` LAN URL, or `kaioken connect expose
...
- **Contract drift**: typecheck of the typed client from `@kaioken/server-contract`;
  label-gated Maestro run in CI on the macOS runner after the Phase 0 probe.
- **Web regression**: every `@kaioken/client-core` extraction PR keeps
  `turbo run test --filter=@kaioken/app` green; the 28 mocking tests are migrated
  explicitly.
...
1. Approve the auth model: machine enrollment + desktop-session cookie via a
   ...
   fails.
2. `@kaioken/client-core` extraction as its own phase (recommended) vs. thin
   copies in mobile.
