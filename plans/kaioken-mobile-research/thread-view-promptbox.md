...
- Event-stream reduction is NOT done in the browser. `@kaioken/thread-view` (`packages/thread-view`, ~16.5k LOC, deps only `@kaioken/domain`, `@kaioken/server-contract`, `zod` — `packages/thread-view/package.json:16-20`) is consumed by `apps/server/src/services/threads/timeline.ts` to turn `ThreadEvent[]` into `TimelineRow[]` (`buildThreadTimelineFromEvents`, `packages/thread-view/src/build-thread-timeline.ts:1314-1380`). The only DOM-ish thing in the package is `Intl.NumberFormat` (`format-helpers.ts:76`); zero `window/document/react` imports. Imports use `.js` suffixes on `.ts` files (NodeNext), and `exports` point at `./src/index.ts` source (`package.json:5-11`) — Metro needs a resolver tweak.
...
- HTTP: `sdk = createBrowserBbSdk({baseUrl, fetch})` (`apps/app/src/lib/sdk.ts`); `@kaioken/sdk/browser` uses global `fetch` + `hono/client` (`packages/sdk/src/transport-http.ts:18-19`) and global `WebSocket` (`realtime-client.ts:163-166`); upload accepts `ArrayBuffer|Blob|Uint8Array` (`areas/projects.ts:118-128`). Package `exports.browser` → `./dist/browser.js`.
...
- Composer `PromptBoxInternal.tsx` (3445 lines): TipTap 3.26 (`useEditor`, StarterKit, Placeholder, `PromptMentionExtension` w/ `ReactNodeViewRenderer`, ProseMirror decorations `editor/prompt-decoration-extension.ts`), DOM paste parsing. Headless pieces: `editor/prompt-editor-serialization.ts` (doc⇄`{text,mentions}`; depends on `@tiptap/pm/model` types), `mentions/types.ts` (`PromptMentionSuggestion` union, `orderCommandSuggestions`), `mentions/find-active-trigger.ts` (regex over `textBetween`), `hooks/usePromptMentions.ts` + `*MentionSuggestions.ts` (pure builders using `@kaioken/fuzzy-match`), `hooks/useCommandSuggestions.ts`, `lib/prompt-draft.ts` (`promptDraftToInput/promptInputToDraft`), `hooks/usePromptDraftStorage.ts` (localStorage, `:59-102`), `effective-prompt-mode.ts`, `lib/permission-mode-options.ts`, `ExecutionControls.tsx` config types (`:15-69`). Voice: `hooks/useVoiceInput.ts` uses `MediaRecorder/getUserMedia/wakeLock` (`:88-268`) → `POST system/voice-transcription` multipart (`lib/api.ts:207`).
...
## Reuse verdicts
- @kaioken/thread-view (packages/thread-view): **reusable-as-is** — Pure TS; deps only @kaioken/domain, @kaioken/server-contract, zod; only global used is Intl.NumberFormat (format-helpers.ts:76). Metro must resolve `exports.source` (./src/index.ts) and `.js`-suffixed imports of .ts files (NodeNext style).
- @kaioken/server-contract + @kaioken/domain: **reusable-as-is** — zod 4 + hono/client only; no DOM/node imports found in src (grep hits are comments). Same Metro source-resolution caveat.
- @kaioken/sdk/browser (createBrowserBbSdk): **reusable-as-is** — Uses global fetch and global WebSocket (transport-http.ts:18, realtime-client.ts:163); pass baseUrl explicitly. Uploads accept ArrayBuffer/Uint8Array (areas/projects.ts:118-128). Pulls @kaioken/templates/generated via areas/guide.ts (data-only). Do not import `@kaioken/sdk` root (node entry pulls @kaioken/config + ws).
- apps/app/src/lib/ws.ts WebSocketManager: **reusable-with-small-changes** — partysocket/ws works on RN WebSocket, but URL derived from window.location (ws.ts:58-60) and HMR singleton via import.meta.hot (ws.ts:275-286); replace URL resolution and singleton creation. Alternatively use @kaioken/sdk realtime-client.
...
- components/thread/timeline/useThreadTimelineController.ts: **reusable-with-small-changes** — Pure React state + sdk calls; imports KaiokenHttpError from @/lib/sdk and useConnectionAwareQueryState; no DOM.
...
## Risks
- Metro/Expo must consume workspace packages from TS source with `exports.source` and NodeNext `.js`→`.ts` import specifiers (packages/thread-view/src/index.ts, @kaioken/domain, @kaioken/server-contract); needs a custom resolver or a build step producing dist for RN.
...
- Is there an auth/session mechanism for non-same-origin clients (fetchWithAppSurface in lib/app-surface.ts; kaioken connect tunnel) that the RN client must replicate for HTTP + WS?
