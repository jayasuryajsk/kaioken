...
**Sidebar row heights:** `--kaioken-sidebar-row-height: 1.75rem`, coarse `2.5rem`.
...
## packages/core-ui
Pure TypeScript, depends only on `@kaioken/domain` (package.json): assertNever, formatEnvironmentDisplay, pending-interaction formatting/presentation, extractErrorMessage/toRecord. No React, no DOM. `@kaioken/thread-view` likewise has no document/window/react-dom refs (grep returned none).
...
## Reuse verdicts
- packages/core-ui (@kaioken/core-ui): **reusable-as-is** — Pure TS + @kaioken/domain only (packages/core-ui/package.json; src/index.ts). No React, DOM, or node builtins in src. Reusable in Hermes.
- packages/thread-view (@kaioken/thread-view): **reusable-as-is** — Pure TS projection/formatting; deps only @kaioken/domain, @kaioken/server-contract, zod; grep found no document/window/react-dom usage.
...
- Is @hugeicons/react-native licensed/available for the free icon set used here (core-free-icons), and does it accept the same IconSvgElement arrays so ICON_MAP can be shared from @kaioken/shared-ui without importing @hugeicons/react?
