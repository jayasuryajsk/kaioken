# Mobile push notifications on self-hosted kaioken servers

Date: 2026-09-01. Branch: `kaioken/add-mobile-push-notifications-thr_nuee4d9q4w`.
...
self-hosted kaioken server therefore cannot call APNs or FCM itself.
...
2. kaioken connect (`apps/connect`, a Cloudflare worker). It already authenticates
   every kaioken server and every phone by an account-scoped credential.

The self-hosted kaioken server needs a network path to one of these two relays.
Nothing else is possible for the store app. A server with no internet route
cannot send pushes in any design.

## How the token flows

1. The app asks iOS for a device token. Apple mints it. It identifies "the kaioken
   app on this phone."
...
- Keys: APNs and FCM keys stay in EAS. kaioken connect holds nothing. The kaioken
  server holds only Expo push tokens.
- Self-hosted: works for every server that can reach `exp.host`. Works in
  Direct mode with no kaioken connect pairing.
...
- Shared risk: Expo rate-limits per project. A flood against one leaked
  token affects every kaioken user. Expo's enhanced push security would stop it,
  but it needs a secret on every sender, which only a publisher-run relay can
  hold.

### Option B. Server sends through kaioken connect (later, if needed)
...
- Option D, self-hosters with their own keys: only with their own app build.
  `KAIOKEN_EXPO_PUSH_URL` already lets a fork point at its own relay.
- Web push with VAPID: browsers only.

## What already exists

Commit `b0362ef65` on `origin/kaioken-mobile-4-push` (2026-08-19, one commit, never
...
- SDK area `sdk.notifications.pushSubscriptions`, CLI
  `kaioken notifications push-subscriptions`, config env vars, docs, guide, skill.
...
`docs/configuration.md`, the kaioken-cli skill, `packages/config` tests,
...
- Drizzle head is `0112_steer_on_enter_default`; the new migration is
  `0113_*`, generated with `pnpm --filter @kaioken/db db:generate`.
- General settings: `appSettings` singleton table
  ...
  `kaioken settings general <key> <value>` in `apps/cli/src/commands/settings.ts`.
...
- Env var pattern: `packages/config/src/env-vars.ts` `defineEnvVar`, defaults
  in `defaults.ts`, launcher list in `packages/kaioken-app/src/launcher.ts:105`.
...
| 6/9 | Server on/off | Boolean general setting `pushNotifications` (default true), read at flush time. Drop `KAIOKEN_PUSH_NOTIFICATIONS`. Keep `KAIOKEN_EXPO_PUSH_URL` startup-only for forks |
...
| 13 | Tap routing | Add `serverUrl` hint from `KAIOKEN_APP_URL` when set; probe profiles as fallback |
...
| 16 | Execution | Two kaioken threads on GPT-5.6, coordinated and reviewed from this thread |
...
3. General setting `pushNotifications` (default true): domain schema and
   default, db accessor, web Settings → General row, `kaioken settings general
   pushNotifications`, SDK types. The sender re-reads it at each flush and
   skips sending when false.
4. Remove `KAIOKEN_PUSH_NOTIFICATIONS` from config. Keep `KAIOKEN_EXPO_PUSH_URL`
   (startup-only) in `packages/config`, the launcher list, docs, guide, skill.
5. Sender changes:
   - Add `serverUrl` to `PushNotificationData` when `KAIOKEN_APP_URL` is set.
...
7. Docs: `docs/configuration.md` (env var and setting),
   ...
   the env var), kaioken-cli guide and skill per `docs/cli-guide-and-skill.md`.
...
1. `eas credentials` shows the APNs key on the kaioken project.
...
3. Direct mode over plain `http://` LAN: the Notifications row refuses with
   the HTTPS hint.
4. kaioken connect mode: same two triggers, same result. Two profiles on the
   phone: tap lands on the right server without probing when `KAIOKEN_APP_URL`
   is set.
5. `kaioken settings general pushNotifications false`: no push. Back to true: push
...
7. Remove a profile on the phone: its row disappears from
   `kaioken notifications push-subscriptions list`. The list shows only suffixes.
...
- Thread 1 (GPT-5.6): PR 1 from this plan. Deliverable: a stacked-ready
  ...
  domain, server-contract, server, sdk, cli, config, kaioken-app, plus the guide
  regeneration.
...
event for a new pending interaction. kaioken connect is precedent for a built-in
...
- Subscriptions in plugin key-value storage. RPC methods
  ...
  `kaioken push-notifications list|add|remove|status`.
- Settings: declarative `expoPushUrl` string with the exp.host default. The
  ...
  `pushNotifications` general setting and `KAIOKEN_EXPO_PUSH_URL` are removed.
